...
1. **`JOBS_PER_INVOCATION`**: 3（現行の並列度）か、`maxDuration` 800 秒を活かして 6 以上にするか。LLM のレート制限とメール送信の制約を実測してから決める
2. **恒久エラーの当日再試行**: ユーザーが Google Ads を接続し直した場合、当日中に再送するか翌日まで待つか。当日再送するなら「設定変更時にジョブを `pending` に戻す」処理が要る

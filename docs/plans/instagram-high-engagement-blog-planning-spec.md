...
- レイアウト:
  ...
  高エンゲージメント率のみ表示中   ← 絞り込み ON のときだけ（目標値は一覧の上に出さない。ダイアログ内の判定基準で読む）
  ┌────┬──────┬────┬────┬────┬──────┬────┬────┬────┬─────────────┐
...
- 項目定義:
  ...
  | 高エンゲージメント率の補足（判定の基準） | 「フォロワー N人（区分）の目標: a〜b%」と、その下に「（フォロワー数は最後に取得した時点の値）」 | — | — | チェックボックスの直下に常に表示（チェックの ON / OFF によらず）。**この補足が無いと、ダイアログの中では何を基準に絞り込むのかが分からない**（2026-09-23 から目標値はここにだけ出す。FR-003）。フォロワー数の但し書きは、フォロワー数を使うこの行に置く（フォロワー数は BR-003 の区分判定の入力で、BR-001 の式には登場しないため、計算式の側には書かない。R-001 / R-002 の分母の誤読を招かない） |
  ...
  | 絞り込み中の表示 | 「高エンゲージメント率のみ表示中」 | — | — | 「高エンゲージメント率」が ON のときだけ一覧の上に出す。チェックがダイアログ内にあり一覧から ON が見えないため（ALT-004）、復元直後を含めて絞り込み中であることを示す。目標値そのものは一覧の上に出さない（ダイアログ内の判定基準と同じ文言を重ねない。2026-09-23 ユーザー指示）。フォロワー数が未取得のときは代わりに下表の案内を出す |
...
- 状態別UI:
  ...
  | 高エンゲージメント率 ON で該当0件 | 既存の0件表示文言のうち**絞り込みあり側**（「表示条件に一致する投稿がありません。…」）。`ig_high` を既存の `hasFilter` 判定（`InstagramTab.tsx:365`）に含めることでこちらに入る。文言自体は変えない。絞り込み中であることは一覧の上の「高エンゲージメント率のみ表示中」で分かる | フィールド構成ダイアログを開き「高エンゲージメント率」のチェックを外す | — |
  | 高エンゲージメント率 ON で復元された直後 | 一覧の上に「高エンゲージメント率のみ表示中」が出る。復元した旨の通知は出さない（§4 Non-goals） | フィールド構成ダイアログを開き「高エンゲージメント率」のチェックを外す | — |
...
Feature: Instagram 投稿のエンゲージメント率と目標判定
  ...
  Scenario: 復元された直後に絞り込み中であることが一覧の上に出る
    ...
    When もう一度 Instagram タブを開く
    Then 一覧の上に「高エンゲージメント率のみ表示中」と表示される
...
- 影響: Instagram タブは絞り込みがダイアログ内（高エンゲージメント率）と上部の行（種別・期間）に分かれたままになる（§6 誤読しやすい罠で明示）。チェックはダイアログを開かないと見えないため、ON のときは一覧の上に「高エンゲージメント率のみ表示中」を出す（§6 項目定義。復元直後に一覧が黙って減ったように見えるのを防ぐ）
...
- 統合・実画面確認: `/analytics?tab=instagram` で列・並べ替え・絞り込み・未取得時の案内・列非表示時の並び順リセットを確認。あわせてフィールド構成ダイアログを開き、(1)「高エンゲージメント率」の直下に判定基準が出る、(2) 計算式が既定で畳まれ、開くと読める、(3) フォロワー数が未取得のときは「絞り込み」節ごと出ない、を確認する（§7「絞り込みの基準がダイアログの中で読める」「計算式は既定で畳まれている」「計算式を開くと読める」）。この表示は props の受け渡しと既存の `details` だけのため、**表示のための単体テストは新設しない**（文言の数値は目標判定の純関数の境界値テストと、一覧側・ダイアログ側で同じ整形関数を使うことで担保する）

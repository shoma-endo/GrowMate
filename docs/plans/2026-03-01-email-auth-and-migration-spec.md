...
- `LiffProvider.getAccessToken()`: LIFF 未ログイン時に `''` を返す（従来は throw）
- `LiffProvider` の `checkAndMaybeLogin` effect: `hasServerSession=true` かつ `isLoggedIn=false` の場合に `/api/user/current` を確認し、`lineUserId` が falsy であれば Email ユーザーと判定して `user` state をセット

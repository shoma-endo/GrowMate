...
**作成日**: 2026-03-01  
**更新日**: 2026-04-10  
**ステータス**: Phase 1 実装完了（2026-04 改訂: 正規ログインはメール OTP のみ）

### 改訂履歴（2026-04-10）

初版（2026-03-01）の前提は **「LINE ログインとメール OTP が併存しうる」** ことと、**二重 `public.users.id` を最優先で防ぐ**ことに寄っていた。そのため **「`email` 一致のみで既存 `public.users` 行へ自動リンクしない」** 等の制約が置かれていた。

**現状のプロダクト**では **新規・正規のログインはメール OTP のみ**（LINE 新規受け入れは廃止）とする。DB 上に **過去の LINE 利用の痕跡**（`line_user_id` 付き行など）が残りうる点は Runbook / 手動移行で扱う。

上記を踏まえ、**チャネル二重を前提にした UI 制約の一部は歴史的記述**となりうる。認証解決の**現行の正**は実装（`UserService.resolveOrCreateEmailUser` と `auth.middleware` の `tryEmailFallback`）および本書 §7.3.2 改訂箇所に従う。
...
2. **Phase 1.5**: **既存 LINE ユーザーをメールログイン可能なアカウントに移行** — 開発者が手動で同一 `users.id` のまま Email ログインを付与し、既存の業務データ（owner 関係等）を維持したうえで Email ログイン可能にする（別 `users.id` へのデータ移行は行わない）

補足:

- **現行方針（2026-04 以降）**: 新規ユーザーの正規ログインは **メール OTP のみ** とする（LINE を正規の新規ログイン経路として追加しない）
- 本仕様では **`1 email = 1 account`**（`public.users` の case-insensitive unique email）を採用する
- **レガシー**として DB に残る LINE 連携行とメール OTP の関係は、**自動のメール一致マージに頼らず** Phase 1.5 / Runbook または実装に定める競合レスポンス（409）で扱う（§7.3.2 参照）
...
2. **正規ログインはメール OTP**（Supabase Email）を用いる
3. **`email` 文字列の一致だけ**を根拠に、検証なしで既存 `public.users` 行へ `supabase_auth_id` を書き込む**自動マージは行わない**（誤結合・取り違えのリスク）。OTP 検証済みセッションにおける解決ルールは §7.3.2 の**現行実装**に従う
4. **Phase 1.5** では、開発者が手動で既存レコードへ Email ログインを付与する手順（Runbook）を用意する
5. 歴史的に **LINE 連携で作成された行**が残る場合があるが、**新規の正規ログイン経路として LINE を増やさない**方針とする（クライアント・Cookie の後方互換は実装に委ねる）
6. **`1 email = 1 account` を採用し、`auth.users` と `public.users` は原則 1:1 で関連付ける**（一意制約により強制）

### 2.2 採用しない案

- 既存ユーザーが**運用承認なし**に、メールアドレスの文字列一致だけで別 `auth.users` を既存業務行に**勝手に統合**するセルフサービス（誤結合の責任境界が曖昧になる）
- `authMiddleware` 全体を将来拡張前提で全面再設計する対応（現時点）
...
- 主目的は「メール OTP によるログイン」と「既存データを壊さない移行手順」の両立
- メール文字列一致のみでの暗黙マージは、**同一人物の証明がコード上明示されない**限り運用トラブルを招きやすい
- `1 email = 1 account` に固定する方が、OTP、セッション解決、運用が単純で事故が少ない
...
- メール OTP ユーザーが `/` 配下の既存機能を利用できること
- **一意制約・競合時**に、無関係なアカウントへ**暗黙にすり替わらない**こと（409 / `email_link_conflict` 等で表出。実装は `resolveOrCreateEmailUser` 参照）
...
- **Phase 1（メールログイン機能開発）**
  ...
  - 新規ユーザーが **メール OTP** で利用開始できる導線（正規経路）
...
### 6.2 Supabase 利用状況（Phase 1 実装後）

- `@supabase/supabase-js` v2.75.0 を使用
- `@supabase/ssr` 導入済み（`src/lib/supabase/server.ts` / `middleware.ts` / `client.ts`）
- Supabase Auth（OTP ログイン・セッション管理）を使用
- Supabase は PostgreSQL + RLS を主用途として利用中
- 保護ページの入場判定は `middleware.ts` が担い、LINE セッション（Cookie）と Supabase Auth セッション（Cookie）の両方を解決する
...
#### 7.3.2 auth.users と public.users の対応付け（現行実装・2026-04）

OTP 検証成功後、`auth.users.id`（= セッションの Supabase Auth UID）に対応する `public.users` を決める。

**採用方針（実装: `UserService.resolveOrCreateEmailUser`）**

1. `public.users` で `supabase_auth_id = auth.users.id` の行を検索する。あればその行を返す。
2. なければ、**新規行を INSERT** する（`email` は正規化済み lowercase、`supabase_auth_id` をセット）。
3. INSERT が **`23505`（一意制約違反）** になった場合:
   - 同一 `auth.users.id` で先行して行が作られていれば、再検索して返す（競合レースの収束）。
   - それでも解決できない場合は **`EmailAuthLinkConflictError`** とし、呼び出し側は **`email_link_conflict`**（HTTP 409 相当）として扱う。  
     典型例: **同じメールが既に別のレコード（例: レガシー LINE 行）に割り当てられており、新しい Auth ユーザーでは別行を作れない**。

**意図的に採用しないもの**

- **`email` の文字列一致だけ**を根拠に、既存行を検索して `supabase_auth_id` を**黙って上書き**する方式（運用上の「誰の行か」の証明をコードに埋め込めない）。

**実装要件**

- `verifyOtp()` の再実行で、同一 `auth.users.id` に対して最終的に一つの `public.users` 行に収束すること
- `signInWithOtp()` だけ通って `public.users` が未作成の orphan `auth.users` については、別途 cleanup Runbook を参照
...
- 現時点の暫定基準として、cleanup 対象は **`last_sign_in_at IS NULL` かつ `created_at < now() - interval '1 day'`** を目安とする
...
サーバー:
  ...
  - auth.users.id を解決
  - public.users を `resolveOrCreateEmailUser` で解決（§7.3.2）
  - メール／auth の一意制約に触れた場合は競合エラー（409）となりうる
  - 途中失敗時も再実行で整合するよう idempotent に扱う
```

#### 7.4.3 public.users 解決ルール

`verifyOtp()` 成功後の解決は **§7.3.2** に従う（`resolveOrCreateEmailUser`）。

- **`email` 一致のみ**を手掛かりに既存行へ暗黙マージする処理は行わない。
- 実装は `SELECT`（`supabase_auth_id`）→ `INSERT` → `23505` 時の再解決／`EmailAuthLinkConflictError` による競合表出。
...
- Cookie 確認、`userId` 解決、role / owner view mode 判定は、認証方式に依存せず同じ `public.users.id` を返す形へ寄せる
...
7. Email セッションで必要なクライアント状態同期（`AuthProvider` の入場判定・遷移制御を含む）
...
### 7.6.3 レガシー行とメール OTP（二重 users.id / データ分断の扱い）

**現行**: 正規ログインはメール OTP のみ。DB に **過去の LINE 利用で作られた `public.users` 行**（同一メールを保持しうる）が残っている場合、**新しい `auth.users` で新規行を INSERT しようとすると一意制約に抵触**し、§7.3.2 の **`email_link_conflict`** として表出する。

| 対策 | 内容 |
|------|------|
| **サーバー側** | `supabase_auth_id` での既存解決 → 新規 INSERT → 失敗時は §7.3.2 のとおり。`email` 一致のみでの暗黙マージは行わない。 |
| **運用** | 同一人物の業務データを一本化するには **Phase 1.5 / Runbook** による手動リンク、または DB 上の整合を取ってから再ログインを試みる。 |

※ 初版で想定した「LINE ログイン済みなら `/login` でメール導線を隠す」は、**LINE を正規経路としなくなった現状**では必ずしも主要な防御ではない。必要に応じて UI を再検討する。

### 7.7 ログイン UI

#### 7.7.1 画面要件

- `/login` に **メール OTP** を主導線として置く
- OTP 入力 UI は 6 桁、数字のみ、ペースト対応、`autocomplete="one-time-code"` を有効にする
- OTP 再送信 UI を設け、再送信制限中は適切に無効化する
- **409 / メール紐付け競合**時は、サポート案内または Runbook 参照のメッセージを返しうる（実装: `ERROR_MESSAGES.AUTH.EMAIL_LINK_CONFLICT` 等）
- レガシー Cookie・後方互換のための LINE トークン読み取りが残る場合は、**認証の正**はメールセッション側のポリシーに従う（`auth.middleware` の `emailLinkConflict` 優先など）
...
新規:
  ...
  （requireUser.ts / requireAdmin.ts は未作成。認証共通層は authMiddleware + resolveUser で代替）
...
- **データ移行の扱い**: 当該ユーザーの既存業務データ（owner 関係・その他 `user_id` 参照）は**そのまま同じ `users.id` に紐づいたまま**である。別テーブルへのデータ移行や別 `users.id` への付け替えは行わない。移行作業は `supabase_auth_id` 設定と、必要に応じた `users.email` 更新で完結する。
...
3. email freeze
   ...
   - 現時点では、**送信 API 側の対象 email ブロック**を第一候補とする
...
### 11.2 Step 2: Supabase Auth 基盤追加（完了）
...
### 11.3 Step 3: 認証解決層の両対応化（完了）

実装:

- `authMiddleware` に Email フォールバックを追加し、LINE / Email 両セッションを解決
  （`accessToken` が falsy の場合 `resolveEmailUserFromSession()` → `public.users` を解決）
- `resolveEmailUserFromSession()` を `src/server/auth/resolveUser.ts` に実装
- role / owner view mode 判定を Email ユーザーでも成立させる
- 既存呼び出し元が同じ `public.users.id` ベースで動作するように調整する
- `requireUser` / `requireAdmin` の独立実装は見送り。`authMiddleware` + `authResult.userDetails.role` による呼び出し元での判定で代替
...
- RBAC、owner 関係、既存業務データが auth provider の追加や変更に引きずられない
...
- `email_link_conflict` → エラー付き `AuthenticatedUser`（`EMAIL_LINK_CONFLICT`、`emailLinkConflict: true`）。**LINE Cookie へのフォールバックより優先**して返す（別 `users.id` で処理が続くのを防ぐ）
...
- `AuthProvider` の `getAccessToken()`: 互換のため常に `''` を返す（Server Action 側で Email セッション解決）
- （歴史的記述）旧実装では `hasServerSession` と `/api/user/current` 連携で Email ユーザーを判定していた。現行は `AuthProvider` が `/api/user/current` で `user` を同期する。
- `ChatClient.tsx`: `(isLoggedIn || !!user) && !liffLoading` でセッション初期化（Email ユーザー対応）

### 14.4 getUserFromAuthHeader の Email フォールバック

`src/server/lib/auth-helpers.ts` の `getUserFromAuthHeader()` にて:

- Bearer トークンが空/なしの場合、Cookie の LIFF トークンを取得して `authMiddleware` に渡す
- `authMiddleware` が Email セッションを解決 → `userService.getUserById` でユーザー取得
- 対象: `GET/DELETE /api/employee`、`POST /api/employee/invite`、`GET /api/employee/invite/status`

### 14.5 middleware の Email ユーザーパス制御

`middleware.ts` にて:

- `line_access_token` Cookie なし + Supabase `supabaseUser` あり → Email ユーザーと判定
- `/admin`、`/analytics`、`/setup/*`、`/setup/google-ads`、`/google-ads-dashboard` は `/unauthorized` にリダイレクト（Phase 1 では Email ユーザーは trial 扱い）
- `/`、`/chat` 等のその他パスは Email ユーザーも通過

### 14.6 仕様書との未実装部分

- Phase 1.5（既存 LINE ユーザー手動移行 Runbook）: 未実施
- `orphan auth.users` 定期 cleanup job: 未実施（手動 Runbook のみ）
- 全デバイス横断の単一セッション制御: Phase 2 以降

...
クライアント処理（ブラウザクライアント: createSupabaseBrowserClient() を使用）:
  ...
  2. ブラウザ側 Supabase クライアントで Auth API を呼び出し:
     ...
     const { error } = await supabase.auth.signInWithOtp({
       email,
       options: {
         shouldCreateUser: true,  // 未登録ユーザーの自動作成を許可
       }
     })
...
> **Note**: OTP 方式では PKCE（`flowType: 'pkce'`）は不要。
> `createBrowserClient` は `signInWithOtp()` でのOTPコード送信リクエストのみに使用する。
> `verifyOtp()` はサーバーサイド（`createSupabaseServerClient`）で実行し、
> セッション Cookie が `cookies.setAll()` 経由で httpOnly Cookie に確実に保存される。
...
| ブルートフォース | Supabase Auth 側のレート制限（API 呼び出し上限）+ OTP 再送信制限（60秒に1回）が適用される |
...
| 対象 | 制限 |
|------|------|
| OTP 送信 | Supabase Auth デフォルト（3600秒あたり30件） + 再送信制限（60秒に1回） |

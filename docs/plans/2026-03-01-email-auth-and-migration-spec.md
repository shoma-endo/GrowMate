...
- **Phase 1.5**: 既存 LINE ユーザーへ Email 認証を安全に追加し、段階的に LINE ログイン導線を停止する運用手順の整備
...
- Phase 1.5 では文言を二段階で切り替える
- Email リンク完了直後: 「今後メールログインをご利用いただけます」
- LINE 導線停止後: 「今後はメールログインをご利用ください」
- ロールアウト対象ユーザーの判定は `email_auth_rollout_scope` に含まれ、かつ `email` / `supabase_auth_id` がリンク済みのユーザーとする
- 既存 LINE Cookie でログイン中の対象ユーザーにも、アプリ内バナーまたは設定画面で同等の案内を表示する
...
## 7. Phase 1.5: 既存 LINE ユーザーへの Email 認証追加と LINE ログイン導線停止

### 7.1 概要

Phase 1.5 は「LINE ユーザーのデータを別 `users.id` へ移すフェーズ」ではない。  
既存 `users.id` に対して Email 認証をリンクし、ロールアウト対象ユーザーから順に LINE ログイン導線を停止して、実質的にメールログインへ移行するフェーズである。
...
流れ:
  ...
  6. 対象ユーザーを rollout scope に追加する
  7. `/login` 上の LINE ログイン導線を非表示にする
  8. 必要に応じて LINE セッションの利用を停止する
```
...
- 段階移行中のみ LINE ログインを互換経路として維持
- ロールアウト対象ユーザーから順に LINE ログイン導線を停止する
- 最終的に対象ユーザーはメールログインを利用する

#### 7.3.2 採用しない

- 別 `users.id` への移行を前提とした設計
- `migrate_user_data(source_user_id, target_user_id)` のような全面データ移行 RPC
- `role='unavailable'` にして旧アカウントを無効化することを前提とした運用
- 複数 `users` 間のデータ統合（例: 誤って作成された重複アカウントの統合）
- ユーザー間のデータ移送を前提とする Stripe / スタッフ / `owner_user_id` / 招待機能の付け替え対応
...
7. 導線停止
   - 対象ユーザーを `email_auth_rollout_scope` の公開対象へ追加
   - `/login` から LINE ログイン導線を非表示にする
   - 必要に応じて LINE 認証許可 Flag を停止する

8. 監査ログ保存
   - 実行者、承認者、実行時刻、source_user_id、email、supabase_auth_id、公開段階を記録

9. ユーザー案内
   - Email リンク完了時に「今後メールログインをご利用いただけます」を通知
   - LINE 導線停止時に「今後はメールログインをご利用ください」を通知
   - 既存 LINE セッション中の対象ユーザーには、ログイン後画面でも同等の案内を表示する
```
...
#### 7.5.4 LINE Cookie が残っている既存ユーザー

- 導線停止後も既存 LINE Cookie でログインできる可能性がある
- 段階的に `authMiddleware` 側で LINE 認証許可を制御する
- 初期段階では UI 導線のみ停止し、安定後に LINE セッションの許可自体を止める
- LINE セッション中であっても、rollout 対象かつ Email リンク済みであればメールログイン移行案内を表示する
...
- `/login` の LINE 導線停止条件を確定
- 必要に応じて LINE 認証許可 Flag を追加

テスト:

- 同一 `users.id` へ Email がリンクされる
- 既存データに変化がない
- 対象ユーザーでは `/login` から LINE 導線が消える
- 対象外ユーザーでは既存 LINE 導線が残る

検証:

- `admin_only` → `internal` → `allowlist` → `all` の順で公開
- UI 導線停止後も想定外のログイン不能が発生しない
- 必要に応じて LINE 認証停止を最終段階で実施する
...
- `allowlist` 以降で対象ユーザーの LINE 導線が非表示になる
- `all` で全ユーザーの主要導線がメール中心になる

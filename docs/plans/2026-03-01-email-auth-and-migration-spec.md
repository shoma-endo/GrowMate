...
クライアント処理（ブラウザクライアント: createSupabaseBrowserClient() を使用）:
  ...
  2. ブラウザ側 Supabase クライアントで Auth API を呼び出し:
     ...
     → PKCE: code_verifier が自動生成され Cookie に保存される
...
2. 新規サーバークライアント（src/lib/supabase/server.ts）:
   ...
   - Cookie ベースでセッションを管理
   - 用途: /api/auth/callback での exchangeCodeForSession()、
           authMiddleware でのセッション検証、Server Actions での認証チェック

3. 新規ブラウザクライアント（src/lib/supabase/client.ts）:
   - クライアントサイドの Supabase Auth 操作専用
   - @supabase/ssr の createBrowserClient を使用
   - PKCE フローの code_verifier 生成・Cookie 保存を自動処理
   - 用途: signInWithOtp()（/login ページ）、signOut()（ログアウト処理）
   - persistSession: true（デフォルト）で Cookie にセッション情報を自動保持
...
> **Note**: `createBrowserClient` は Cookie の読み書きをブラウザのデフォルト動作で自動処理する。
> `signInWithOtp()` 呼び出し時に PKCE の `code_verifier` が自動生成され、
> `sb-<ref>-auth-token-code-verifier` Cookie に保存される。
> サーバー側の `exchangeCodeForSession()` がこの Cookie を参照して検証を完了する。

...
**ブラウザクライアントの初期化パターン（app/login/page.tsx）:**
...
export default function LoginPage() {
  ...
  const [resendCooldown, setResendCooldown] = useState(0); // 残り秒数
  ...
  const handleSendOtp = async () => {
    ...
    setStep('otp');
    startResendCooldown(60); // 60秒の再送信制限タイマー開始
  };
...
**Server Action の詳細仕様（src/server/actions/auth.actions.ts）:**
...
export async function verifyOtpAction(
  ...
  if (!parsed.success) {
    return { success: false, error: parsed.error.issues[0].message, errorType: 'invalid_otp' };
  }

  // 2. サーバー側 Supabase クライアントで OTP 検証
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.auth.verifyOtp({
  ...
  if (error) {
    ...
    // レート制限
    if (error.status === 429) {
      return { success: false, error: '試行回数の上限に達しました。しばらく時間をおいて再度お試しください。', errorType: 'rate_limited' };
    }
    // 無効なコード（不正確な OTP 入力）
    if (error.message.includes('invalid') || error.status === 400 || error.status === 401) {
      return { success: false, error: '認証コードが無効です。再度入力してください。', errorType: 'invalid_otp' };
    }
    // 想定外のエラー
    return { success: false, error: '認証に失敗しました。しばらく時間をおいて再度お試しください。', errorType: 'unknown' };
  }

  // 4. public.users の最終ログイン時刻を更新（失敗してもログインはブロックしない）
  if (data.user?.id) {
    await supabase
      .from('users')
      ...
      .then(({ error: updateError }) => {
        if (updateError) {
          console.error('[verifyOtpAction] last_login_at 更新失敗:', updateError.message);
        }
      });
  }

  // 5. 成功（Cookie は createSupabaseServerClient の cookies.setAll() で自動設定済み）
...
| 項目 | 仕様 |
|------|------|
| 入力制限 | 数字のみ（0-9）。各ボックスは1文字まで |
| 自動フォーカス移動 | 1桁入力後、次のボックスに自動フォーカス |
| ペースト対応 | クリップボードから「123456」をペースト時、各桁を適切なボックスに分配 |
| Backspace 動作 | 空のボックスで Backspace 押下時、前のボックスに移動して削除 |
| `inputmode` | `inputmode="numeric"` でモバイルの数字キーボードを表示 |
| `aria-label` | 各ボックスに「認証コード N 桁目」（N=1〜6）を設定 |
| `autocomplete` | `autocomplete="one-time-code"` でブラウザの OTP 自動入力を有効化 |
| 参考ライブラリ | `input-otp`（shadcn/ui 推奨）を検討。手動実装する場合は上記仕様を遵守 |
...
エラーフロー（LINE アカウント重複時）:
  ユーザーが user@example.com で OTP ログインを試みる
    ...
    ※ §7.1 メール列挙攻撃対策に基づき、LINE 重複・レート制限・その他
       すべてのエラーで上記の同一 UI を表示する。
       「LINE でログイン」等の具体的な回復手段を提示すると、
       当該メールアドレスの状態を推測されるリスクがある。
       「最初に戻る」でログイン方法選択画面（/login）に戻し、
       ユーザー自身に方法を選択させる。
...
```text
作業内容:
  0. 【事前チェック】source_user（LINE ユーザー）の public.users.email が
     target_email と同値の場合、handle_new_auth_user() トリガーの重複防止
     （同一メールの LINE 行が存在 → RAISE EXCEPTION）に抵触する。
     この場合、以下の退避処理を先に実行する:
       a. source_user の email を backup フィールドに退避:
          UPDATE public.users
            SET email_backup = email, email = NULL
            WHERE id = :source_user_id;
       b. 退避完了を確認してからステップ 1 に進む
       c. 移行完了後、email_backup は監査ログとして保持（削除不要）
...
#### 7.3.1 メールアドレス単位のアカウントロックアウトポリシー（将来検討）

Supabase Auth の `verifyOtp()` レート制限は **IP アドレス単位** であるため、複数 IP からの分散攻撃には十分に対応できない可能性がある。

```text
課題:
  - 攻撃者が複数の IP から同一メールアドレスを攻撃した場合、
    IP ごとに 30回/5分 の枠があるため、10 IP × 30回 = 300回の試行が可能
  - 共有 IP（企業プロキシ・VPN）では、他ユーザーの失敗試行が自分の枠を消費する可能性

推奨対策（Phase 1 リリース後に優先度を判断）:
  - 同一メールアドレスに対する verifyOtp() 失敗が 10回に達した場合、
    そのメールアドレスを 15分間ロック
  - ロック中の verifyOtp() 呼び出しはすべて拒否し、ユーザーに
    「セキュリティ保護のため一時的にロックされています。15分後に再度お試しください」と通知
  - 運用監視でアラートを発行し、異常パターン（短時間の大量失敗）を検知

実装方針:
  - Supabase Auth 単体では メールアドレス単位の制限は未サポート
  - アプリケーション層で失敗カウンターを管理する必要がある
  - 選択肢: Vercel KV（Redis 互換）/ Supabase テーブル + TTL で
    メールアドレス × 失敗回数 × 最終失敗時刻 を記録
  - Phase 1 初期リリースでは Supabase Auth デフォルトの IP 制限で運用し、
    不審なアクセスパターンが観測された場合に本対策を実装する

Phase 1 運用監視計画:
  - [ ] verifyOtp() 失敗ログの記録（email, IP, timestamp, エラー種別）
  - [ ] 同一メールアドレスへの失敗試行が 1時間に 20回を超えた場合、
        運用担当者へアラート発行（Sentry / CloudWatch + 運用チャンネル通知）
  - [ ] 分散攻撃パターン（複数 IP から同一メールへの集中試行）を週次レビュー
  - [ ] Phase 1 リリース後 1ヶ月の攻撃パターンを分析し、以下の条件で
        Phase 2（メールアドレス単位ロックアウト）実装を判断:
        ・アラート閾値超過が繰り返し発生する場合
        ・分散 IP からの攻撃が全インシデントの 10% 以上を占める場合
```
...
```text
デプロイ前確認:
  - [ ] メールテンプレートを {{ .Token }} に変更済み（{{ .ConfirmationURL }} ではない。これが OTP / Magic Link の切り替えを決定する唯一の設定）
  - [ ] OTP 有効期限を 3,600秒（1時間）に設定済み（Supabase Dashboard > Authentication > Providers > Email > OTP Expiry。Phase 1 はデフォルト値を維持。最大値は 86,400秒だが、§7.3.1 メールアドレス単位ロックアウト実装後に延長を再検討）
...
■ signInWithOtp の挙動
  ...
  - 【本プロジェクトの設定】Phase 1 では 3,600秒（1時間）のデフォルト値を維持する（§5.6 参照）
    → メールアドレス単位のロックアウト（§7.3.1）実装後に延長を再検討。届かなかった場合は「再送信」ボタンで対応
...
> **Note**: 本セクションの設定は Phase 1 デプロイの前提条件。§10.1 Phase 1 検証項目にチェックリスト（§7.5.5）の完了確認を含む。
...
- [ ] signInWithOtp() エラー時に汎用メッセージが表示され、LINE 重複・レート制限等の詳細が漏洩しないこと（§7.1 メール列挙攻撃対策）
- [ ] verifyOtp() 失敗ログの記録と監視アラートが設定済みであること（§7.3.1 運用監視計画）
- [ ] 同時セッション: PC とスマホから同時ログイン可能で、片方のログアウトが他方に影響しないこと
- [ ] セッショントークンのリフレッシュが各デバイスで独立して動作すること
- [ ] ブラウザ互換性: Safari (macOS/iOS)・Chrome (Windows/Android)・Firefox・Edge での OTP 送信・検証フロー
- [ ] iOS Safari での Cookie 保存（ITP による制限の影響がないこと）
- [ ] モバイルブラウザで OTP 入力時に数字キーボードが表示されること（`inputmode="numeric"`）
- [ ] OTP 送信中のネットワーク切断時にエラーメッセージが表示され、再送信可能であること
- [ ] OTP 検証中のネットワーク切断時にエラーメッセージが表示され、再試行可能であること
- [ ] §7.5 メール到達性のデプロイ前チェックリスト（§7.5.5）を完了済みであること（カスタム SMTP、SPF/DKIM/DMARC、mail-tester 確認）

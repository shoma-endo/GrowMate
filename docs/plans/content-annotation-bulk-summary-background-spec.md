...
10分ごと → GitHub Actions → /api/cron/content-annotation-summary
        ...
        → 1件ごとに「8項目がすべて空・WordPress 連携済み・user_id 一致」を実行直前に再判定（BR-B08）
        → 時間予算(760秒。新規着手の実効打ち切りは経過約640秒)の範囲で、並列3で要約を実行
        → 1件処理するたびに進捗をジョブ行へ保存（BR-B09）
...
- ルール ID: **BR-B04 1回の cron 起動で使う時間は現行の予算を踏襲する**
  ...
  - 例外: 着手できない時点で未処理が残っていれば、ジョブを `pending` に戻し、未処理の位置（カーソル）と件数を保存する（保存粒度は BR-B09）。
- ルール ID: **BR-B05 並列数は3**
...
- ルール ID: **BR-B08 実行直前に対象条件を再判定する（親仕様 BR-01 の継承）**
  ...
  - 例外: なし。起票時のフィルタ結果を根拠に再判定を省略してはならない。
- ルール ID: **BR-B09 進捗は1件ごとに保存し、再開に対して冪等にする**
  - ルール: `processed_count` / `succeeded_count` / `failed_count` / `skipped_count` / `failed_by_code` は**1件処理し終えるたびに**ジョブ行へ書き戻す（予算切れ時にまとめて保存しない）。更新は claim 時に発行した `job_token` を `.eq('job_token', ...)` で条件に付け、別起動に回収済みのジョブへ書き込まない。
  - 理由: 「予算切れ時のみ保存」だと、`maxDuration` によるハードキルや想定外例外で**1起動分（最大約70件）の進捗が丸ごと失われる**。再開時に同じ範囲を再処理するため、BR-B08 が無ければ LLM 課金が二重になり、集計カラムを加算更新している場合は件数が二重加算されて完了メールの意味が壊れる。
  - 例外: なし。
...
| サーバー | ジョブ処理サービス（claim・実行直前の再判定・並列3・時間予算・1件ごとの進捗保存）、cron ルート、`CRON_CONFIGS` 追加 | 12 | 1.5 |
...
| FR-B06 | 進捗を1件ごとに `job_token` 条件付きで保存する | Must | ハードキル・想定外例外での進捗ロスト対策（`spec-review` 指摘 F-06）／BR-B09 | AC-B03 / AC-B14 |
| FR-B07 | ジョブ終了（`completed` / `failed`）時に完了メールを1通送る（冪等） | Must | クライアント合意 2026-09-04（完了時にメール1通）／BR-B06 | AC-B04 / AC-B05 / AC-B06 / AC-B15 |
...
processing --[想定外の例外 / 試行上限超過（attempt_count > 3）]--> failed（完了メール送信。件数は途中まで）
```

- **冪等性・重複実行時の挙動**:
  - 起票: 1利用者につき未完了ジョブは1件（部分ユニークインデックス。BR-B03）。
  - claim: `for update skip locked` で排他。回収されたジョブへの遅延書き込みは `job_token` 条件で弾く（BR-B09）。
  - 進捗: 1件ごとに保存するため、再開時に同じ範囲を処理し直さない。仮に再処理されても BR-B08 の再判定でスキップになり、LLM 課金は発生しない。
  - メール: `notified_at` が非 NULL なら送らない（BR-B06 / AC-B05）。
...
- 状態別UI:
  ...
  | 未完了ジョブあり（`pending` / `processing`）でページを開いた | ツールバー付近に「要約中...（処理済み N / 全 M 件）」 | 再読み込みで最新化 | 自動更新なし（BR-B07） |
...
- UI用語:
  - 「要約中...（処理済み N / 全 M 件）」は `ui-text.md` §3「進行中の状態表現」の**ラベル**として扱う（`<動詞>中...` の形）。説明文ではないので `要約を実行中` とはしない。
...
- 分母 M は「未要約件数」ではなく「起票時に選択した件数」。`mode: 'all'` では要約されない記事も分母に入る（上の「分母の定義」）。
...
機能: AI要約一括のバックグラウンド実行
  ...
  シナリオ: AC-B08 実行中は進捗が見える
    ...
    ならば 「要約中...（処理済み 45 / 全 267 件）」が表示される
  ...
  シナリオ: AC-B14 進捗は1件ごとに保存される
    前提 cron が10件目まで処理した時点で関数が異常終了する
    もし 次の起動が同じジョブを claim する
    ならば 10件目までの成功・失敗・スキップ件数が保存されている
    かつ 11件目から処理が再開される

  シナリオ: AC-B15 失敗で終わったジョブも1通だけ通知する
    前提 試行回数が上限（3回）に達したジョブがある
    もし そのジョブが failed になる
    ならば 途中までの件数を含むメールが1通送られる
    かつ 再度 cron が起動しても同じジョブにメールは送られない
```
...
| 失敗時の再実行 | 着手予算切れは次回起動で続行。想定外の例外は試行回数上限（`attempt_count` > 3）で `failed`。`processing` のまま **20分**以上動いていない行は次回 claim で回収する | 雛形 `claim_gsc_suggestion_jobs` と同型だが、**しきい値は15分から20分へ延ばす**。雛形の cron は `maxDuration` 300秒（余裕10分）だが本仕様は 800秒（13.3分）で、15分のままだと余裕が1.7分しかなく、稼働中のジョブが「スタック」と誤判定されて二重に claim され、同じ記事へ二重課金・件数の二重加算が起きる |
...
| `target_annotation_ids` | uuid[] not null | 起票時に解決した対象（BR-B02） | 新規（BR-B02） |
| `total_count` | integer not null | **起票時に固定した対象ID数**（`target_annotation_ids` の長さ）。要約される見込み件数ではない（§6「分母の定義」） | BR-B07 / AC-B08 |
| `processed_count` | integer not null default 0 | 処理済み位置（カーソル兼用） | 既存 `gsc_suggestion_jobs` 踏襲 |
| `succeeded_count` / `failed_count` / `skipped_count` | integer not null default 0 | 集計。1件ごとに更新（BR-B09） | 親仕様の `BulkSummaryResult` 4区分 |
| `failed_by_code` | jsonb not null default '{}' | 失敗理由コードごとの件数 | 親仕様の `failedByCode` |
| `attempt_count` | integer not null default 0 | claim 回数。**3を超えたら `failed`** | 既存 `gsc_suggestion_jobs` 踏襲（上限3も同値） |
...
- `attempt_count` が3を超える行は claim せず `failed` に落とす。
...
| Resend | 完了メール | 既存 `EmailService` に**新規メソッドを追加**。宛先は `public.users.email` | 送信失敗時は `notified_at` を更新せず、次回の cron 起動で再送を試みる | **未照合**（`resend.com` がブロック。§16） |
| GitHub Actions | 10分間隔の起動 | 新規ワークフロー1本（`CRON_SECRET` を渡して `scripts/invoke-cron.sh` を実行） | 失敗は GitHub Actions の通知で運用担当が検知する（§2） | **未照合**（`docs.github.com` がブロック。§16 / §10 制約条件） |
...
- **リポジトリの visibility を前提にしている**: §11 ALT-002 の代償評価は「public リポジトリのため実行時間の課金が発生しない」を根拠にしている。本レビュー時点では `gh` CLI が実行環境に無く**未確認**。算術では 1日144回 × 30日 = **4,320 job-run/月**で、private であれば分単位切り上げ課金により概算 4,320 分/月となり、Free 2,000 分 / Pro・Team 3,000 分の月次無料枠を超える（既存 `hourly-cron.yml` の 96 job-run/日に上乗せ）。**実装前に visibility と GitHub の課金条件を確認し、private なら起動間隔を見直す**（§16 に確認日・確認方法を記録する）。
...
- 影響: ワークフローが1本増える。空振り起動が1日約144回発生する（pending 0件なら即座に返る）。**この空振りが課金対象にならないことはリポジトリが public であることに依存しており、本レビュー時点では未確認**（§10 制約条件）。private だった場合は本判断（10分間隔）そのものを再検討する。
- 将来変更する条件: 課金前提が崩れた場合、または利用者数が増えて claim の直列化が問題になった場合（`p_limit` の引き上げか間隔短縮を検討）。
- 判断者・判断日: 未確定。
...
| R-B07 | GitHub Actions の課金前提（public リポジトリ）が誤っていた場合、空振り144回/日が課金対象になる | private だった場合、概算 4,320 分/月で無料枠（Free 2,000 / Pro・Team 3,000 分）を超える | 実装前に visibility と GitHub 公式の課金条件を確認する。private なら ALT-002（10分間隔）を再検討する | 実装者 | 未対応（§16 の未照合項目） |
...
- ユニット（新規）:
  ...
  - カーソル前進と再開（AC-B02）、**1件ごとの進捗保存と異常終了後の再開**（AC-B14）
  ...
  - 完了メールの冪等（AC-B05）、メール未登録時のスキップ（AC-B06）、`failed` 終了時の送信（AC-B15）
...
2. ジョブ処理サービス（claim → **1件ごとに BR-B08 の再判定** → 並列3で処理 → **1件ごとに `job_token` 条件付きで進捗保存** → 完了判定）。既存 `GscSuggestionJobService` を雛形にする。
   - `contentAnnotationSummaryService.generateSummary` の `cookieStore` を任意化し、cron からは cookie 無しの `getCookie` を渡す（§9）。
   - 対象記事の取得は既存の `chunkIds(ID_QUERY_CHUNK_SIZE)` を流用し、PostgREST の `db-max-rows = 1000` に触れないようにする。
3. cron ルート追加、`CRON_CONFIGS` へ登録（値は §9 の表）、10分間隔ワークフロー追加（`concurrency` 付き）。`CONTENT_ANNOTATION_BULK_SUMMARY_MAX_DURATION_SEC` の帰属先を決め、`analytics-max-duration.test.ts` を更新する。
4. 完了メール（`EmailService` の新規メソッド + HTML 本文 + `notified_at` による冪等 + `failed` 時の文面）。
...
| --- | --- | --- | --- |
| 着手前 | Q-B01 / Q-B02 / Q-B03 の回答が揃っている。リポジトリ visibility と GitHub Actions の課金条件を確認済み | PO / 実装者 | 未確認 |
| 手順1完了時 | `anon` から RPC を実行できない | 実装者 | 未確認 |
| 手順3完了時 | `workflow_dispatch` で手動起動し、pending 0件で即座に返る | 実装者 | 未確認 |
...
- リポジトリ visibility（public / private）と GitHub Actions の課金条件が未確認（R-B07 / ALT-002 の前提）。

#### 公式ドキュメント照合

- 実施 / 未実施: **実施（一部未確認）**。確認日 2026-09-04、取得手段 WebFetch。
- 照合済み:
...
- **未確認（実行環境の egress proxy にブロックされ取得不可。実装前に照合し、verbatim 引用と確認日をここへ追記すること）**:
  ...
  | GitHub Actions `schedule` | https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule | §10 制約条件（最小間隔・遅延・ドロップ・60日無活動での自動停止） |
  | GitHub Actions の課金 | https://docs.github.com/en/billing （Actions の課金） | §10 / ALT-002 / R-B07（public リポジトリの無料枠条件） |

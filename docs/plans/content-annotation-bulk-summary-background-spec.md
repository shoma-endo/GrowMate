...
- 仕様レビュー・承認の見込み: `spec-review` を4周実施して指摘41件（F-01〜F-41）を反映済み。Q-B01〜Q-B03 は 2026-09-04 に回答を受領し本書へ反映済みで、`approved` に残る条件は §16 承認表の記入のみ。
...
- 判定の中身: 対象ジョブの `user_id` の WordPress 設定（`SupabaseService.getWordPressSettingsByUserId`）を読み、
  ...
  - **WordPress 設定そのものを取得できない場合**（`SupabaseService.getWordPressSettingsByUserId` は `supabaseService.ts:846-877` のとおり **(a) 設定行が無いとき**と **(b) クエリがエラーになったとき**の両方で同じ `null` を返す）。真偽値を返す契約なのでどちらかに倒す必要があるが、**倒し方で利用者に送る次アクションが正反対になる**ため、仕様として次のとおり分ける。
    ...
    - 実装上の注意: 既存 `getWordPressSettingsByUserId` は (a) と (b) を同じ `null` に潰すため、**可否判定関数の中では `maybeSingle()` の `error` と `data` を直接見て両者を区別する**。既存メソッドのシグネチャは変えない（単記事要約・WordPress インポートへ波及するため）。**新規カラム・新規テーブル・新規設定は伴わない。**
...
- 件名（前例 `【GrowMate】コンテンツ評価が完了しました：…` と同型）。**分岐はジョブの終わり方だけでなく「終わり方 × 成功件数」で決める**:

  | ジョブの終わり方 | 成功件数 | 件名 |
  | --- | --- | --- |
  | `completed` | 1件以上 | `【GrowMate】AI要約が完了しました（成功 245 件 / 対象 267 件）` |
  | `completed` | **0件** | `【GrowMate】AI要約を完了できませんでした（成功 0 件 / 対象 267 件）` |
  | `failed` | 1件以上 | `【GrowMate】AI要約が途中で終了しました（成功 120 件 / 対象 267 件）` |
  | `failed` | **0件** | `【GrowMate】AI要約を完了できませんでした（成功 0 件 / 対象 267 件）` |
...
- 本文（HTML。上から順に）:
  1. 見出し: 件名と同じ分岐にする。`completed` かつ成功1件以上なら「AI要約が完了しました」、`failed` かつ成功1件以上なら「AI要約が途中で終了しました」、**成功0件なら終わり方によらず「AI要約を完了できませんでした」**。
  ...
  5. **`failed` で終わった場合だけ**: 「途中までの結果です。もう一度「AIで要約」を実行すると、残りの記事から続けられます。」（背景実行では時間予算切れの継続は cron が行うため、**この案内は `failed` のときだけ出す**。`completed` に出すと BR-B06 が避けた「誤った手動再実行の案内」になる。**成功0件でも `completed` なら出さない**——全件処理は終わっており「続き」は無いため。この場合の次アクションはブロック4の理由ごとの案内が担う）。
...
- 本文の例（**`completed` かつ成功0件** / WordPress の連携が切れている利用者。§6 の状態遷移では全件処理し終えたジョブは全件失敗でも `completed` になるため、この利用者が実際に受け取るのはこの形。実装時はこの粒度を満たすこと）:
  ...
  ブロック5（「途中までの結果です…」）は `completed` なので出さない。リンクはいずれも絶対 URL（`${siteUrl}/setup/wordpress` / `${siteUrl}/analytics`）で組む。
...
- Q-B01 / Q-B02 / Q-B03 は **2026-09-04 に回答受領・反映済み**（§12「クライアント回答（決定事項）」）。監査で出た指摘41件（F-01〜F-41）も反映済み。

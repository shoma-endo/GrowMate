...
| `goal` | ゴール | Claude |
| `prep` | PREP | Claude |
| `opening_proposal` | 書き出し案 | **機械抽出**（本文冒頭から最初の`h2`直前までにある`p`タグの原文。Claude出力は使用しない） |
...
- 対象: 範囲内の空でない`<p>`要素のみ。`div`等の非`p`要素に含まれる文字列は対象外とする。
- 処理: `p`内のインラインHTMLを除去し、パーサーでHTMLエンティティをデコードした原文テキストを取得する。複数段落は空行で連結する。
- 最初の`h2`がない、または範囲内に有効な`p`要素がない場合は空値を保存する。

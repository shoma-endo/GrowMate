...
配置をこの階層にする理由:
- 両画面とも`wp_post_id`・`canonical_url`・`session_id`を含む`AnnotationRecord`を直接保持しているため、活性化条件の判定に`AnnotationFormFields`への新規prop受け渡しが不要になる（`AnnotationFormFields`は経由しない）。

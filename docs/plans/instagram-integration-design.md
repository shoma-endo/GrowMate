...
- **算出方法（§11.3 が正本）**: `率 = 実数 ÷ reach`。提供画面の実数で検算した結果、**分母はリーチで一致**した（いいね率 4.1% ← 21÷523=4.02%／保存率 0.4% ← 2÷523=0.38%。閲覧数 618 で割るとどちらも一致しない）
- **ただし Instagram は算出定義を公開していない**ため、アプリの数値と完全一致する保証はない。**GrowMate 独自計算であることを UI で明示**し、ズレたときに不具合と誤解されないようにする
- **`reels_skip_rate` だけは公式が率で返す**（§3.3）。**独自計算の率と混ぜて表示しない** — 出所が違うものが同じ見た目で並ぶと、片方だけアプリと一致しない理由を説明できなくなる
...
- **率の列（2026-08-05 Q10 回答。GrowMate 独自計算）**
  ...
  - **`reels_skip_rate` と混ぜない**: スキップ率は**公式が率で返す唯一の指標**（§3.3）。独自計算の率と並べる場合は、**ツールチップで出所を書き分ける**（スキップ率＝「Instagram が提供する値」／その他＝「GrowMate がリーチ数から計算した参考値。Instagram アプリの表示と一致しない場合があります」）。同じ見た目で並べると、片方だけズレたときに理由を説明できなくなる

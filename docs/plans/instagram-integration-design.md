...
- ~~**`INSTAGRAM_BETA_USER_IDS` を空にしない**~~（**審査提出前の制約。2026-08-14 の審査通過で失効**）。allowlist の解除は本 Phase の**最終タスク（item6）** であり、審査通過後に行う。審査前に空にすると allowlist 外の paid ユーザーに Instagram カードが出て、連携しても Advanced Access が無いため `error code 100` で落ちる（§3.2 アクセス認証）
   ...
   **決定**: 自動同期（cron）を Phase 2 のスコープから外し、**Instagram タブの「最新化」ボタンだけ**にする。理由は「最初はシンプルな作り（MVP）優先」（CLAUDE.md の MVP 原則にも沿う）。
   ...
   - **タブ初回表示の自動同期（2026-09-16 追加）**: `/analytics` の Instagram タブを開いた時点で `incremental` 同期を自動実行する。cron は引き続き作らない（下の「将来 cron を足す場合」は未着手のまま）。実質「ログイン起点の日次同期」になる。
     ...
     - **Non-goal**:
       ...
       - 一覧の既定期間（直近30日）の変更（今回の要求外。§11.3 の空状態文言で backfill 導線だけ担保する）
...
（「残件なし」は審査提出まわり = Q6/Q7 についての記述。次節の Phase 2 分 Q2/Q3/Q8/Q9 も 2026-08-05 にすべて回答済みで、**本節に未回答の確認質問は無い**）
...
（2026-08-04 時点の記録。**下記4件は 2026-08-05 にすべて回答済み**で、現在ブロッカーではない。）Phase 2 をローカル先行開発する方針に変えたことで、当時は下記4件が実装のブロッカーに昇格した。いずれも「実装者が勝手に決めると後で作り直しになる」もの。2026-07-22 定例の要望（手段を鵜呑みにせず選択肢と推奨案をセットで出す）に従い、選択肢つきで確認する。
...
- ~~Q4. 対象ロール~~: **回答済み（2026-07-23）→ 2026-08-14 に決定変更** — 当初は `canAccessInstagram` 解除後 admin / paid / trial（`unavailable` のみ除外）。**現在は admin / paid**。
  ...
  - **確定（2026-08-14）**: **admin / paid で確定**。2026-07-23 の記録（admin / paid / trial）はこれをもって上書きする。トライアルには Instagram を提供しない
  - **Phase 3 再開時の注意**: §7 は Phase 3（保留中のリール台本チャット）を「既存のトライアル日次制限（`checkTrialDailyLimit`）にそのまま乗せる」前提で書いている。**トライアルが Instagram を使えない以上この前提は成立しない**ため、再開時に trial の扱いを決め直すこと
...
- **一覧が0件のときの文言は3分岐**（2026-09-16 改訂。§4 Phase 2 item3「タブ初回表示の自動同期」に対応）:
  ...
  3. **同期済みで0件** →「表示条件に一致する投稿がありません。期間や種別フィルタを変更するか、「過去の投稿をインポート」で古い投稿を取得してください」。**一覧の既定期間は直近30日なので、30日以内に投稿が無いアカウントは自動同期が成功してもここに来る**。旧文言は期間フィルタにしか触れておらず、この状態から backfill への導線が消えていた

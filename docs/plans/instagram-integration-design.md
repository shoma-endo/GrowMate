...
- **Instagram アプリ画面との突き合わせ（2026-08-05。クライアント提供のリールインサイト画面6枚と照合）**: 「クライアントが実際に見ている画面」を基準に、API で再現できる／できないを確定させた。**画面に出ている＝API で取れる、ではない**（アプリは内部 API を使う）。
  ...
  - **追加すれば取れる（→ §2 スコープに追加済み）**: 再投稿=`reposts`（公式定義「再投稿数から削除された再投稿を引いた数」）/ スキップ率=`reels_skip_rate`（公式定義 "The percentage of views from people who **skipped during the first 3 seconds** of the reel." — **アプリと同じ「率」で返るため加工不要**）
...
**Q8（新規）Adzviser + スプレッドシートで貯めてきた過去の実績は、GrowMate に移しますか。**
1. 移さない — GrowMate は連携以降の投稿だけを扱い、過去分はスプレッドシートを残す（**推奨**。移行の作り込みが不要）
2. 移す — 過去分も一覧で見たい（別途 CSV 取込等の機能が必要。Phase 2.5 として切る）
> 背景: §1 の目的は「スプレッドシート運用の内製化」だが、Phase 2 が取得するのは**直近50件と直近30日のアカウント指標のみ**。過去分の扱いが仕様のどこにも書かれておらず、非スコープにも入っていない。リリース後に「去年の実績が見えない」が必ず出る論点

**Q10（新規 2026-08-05）「いいね率」「保存率」などの率は、GrowMate 側で計算して出しますか。**
1. 実数だけ出す（**推奨**。Instagram アプリを見れば率は分かる。GrowMate は「Instagram では出せない一覧・比較」に集中する）
2. 率も出す — ただし **GrowMate 独自計算である旨をツールチップで明示**する（アプリの数値とズレても説明できる状態にする）
> 背景: 率は API に無く、実数からの割り算になる。提供画面の実数で検算した結果、**分母はリーチ**と見られる（いいね率 4.1% ← 21÷523=4.02%、保存率 0.4% ← 2÷523=0.38%。閲覧数 618 で割ると一致しない）。ただし **Instagram は算出定義を公開していない**ため、完全一致の保証はない。ズレたときに「バグでは？」と言われる余地を残さないための確認
...
- `src/lib/constants.ts`（**変更**: `INSTAGRAM_COLUMNS` 追加、`ANALYTICS_STORAGE_KEYS` に `IG_VISIBLE_COLUMNS` 追加 — §11.3）
...
- **列構成はユーザーが選ぶ（2026-08-05 Q2 回答）— `FieldConfigurator` を再利用する**
  ...
  - **列定義**: `src/lib/constants.ts` に `INSTAGRAM_COLUMNS` を追加（`ANALYTICS_COLUMNS`（`constants.ts:257-277`）と同型）。候補は 種別 / キャプション冒頭 / 投稿日 / リーチ / 視聴数 / いいね / コメント / 保存 / シェア / **再投稿** / 総インタラクション / 平均視聴時間 / 総再生時間 / **スキップ率（リールのみ）**。**既定表示は絞り、残りは `defaultVisible: false`** にして初期表示をシンプルに保つ（クライアント回答の「実装しやすいシンプルな形」に対応）

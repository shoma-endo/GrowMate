...
- **Q-001**: step0a / step0b のプロンプト本文。本文が無いと両ステップが成立しない。回答者: ユーザー。期限: 実装着手前

### 未決定事項（今は決めない）

- **OPEN-001**: step1 の placeholder 文言の微調整（キーワード自動引き継ぎ前提の文言にするか）。今決めない理由: 現行文言でも動作上の問題が無く、実画面を見てからの方が判断が速い。決めるタイミング: step1 の実装時。決める人: 実装担当
- **OPEN-002**: step0b の Web 検索 `maxUses`。今決めない理由: 本設計の初期値 3 で動作し、コスト/精度は実行しないと測れない。決めるタイミング: 初回実測後。決める人: 実装担当

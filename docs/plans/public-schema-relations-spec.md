...
なし。Q-001 / Q-002 はいずれも回答が必要な事項であり、意図的に後ろへ倒した判断は無い。
...
- クライアント確認質問: なし（画面・運用手順・利用者データモデルは変えない）。§11 の Q-001 / Q-002 は仕様承認者宛であり、クライアント宛ではない。Q-001 は残留データを消す決断をするときだけクライアントへ上げる

...
- GA4取込時に全記事を自動評価する処理。2026-08-05 定例で「後回し」に合意済み（`docs/context/client-vision-from-lark.md` §1.9.5）。「実装難易度が最も高いと開発側が判断。評価が走っていないものをソートできれば手動運用で代替可能」という整理による。未評価コンテンツの発見は未評価フィルタで対応する（§10.2。2026-08-26 のサイクル統合以降は「評価未設定」チップ1つが担う）。
- **一覧の並び替え（評価状態・点数・最終評価日時）。** Q-B 回答（2026-08-16）「未評価コンテンツはフィルタだけで足りる」により実装しない。§1.9.5 の「ソート」は未評価コンテンツの発見が目的であり、未評価フィルタで満たす。~~（新設。GSC未評価フィルタと同型）~~ **2026-08-26 訂正（サイクル統合）: GA4専用の未評価フィルタは新設せず、既存のGSC側フィルタを~~「評価未開始」~~**「評価未設定」**（2026-08-26 に再改称。§15.4）へ改称した1つで満たす。**
...
- **全記事への一括サイクル登録・自動登録。**登録は記事単位の手動操作のみとする（BR-10。GSC の `EvaluationSettings` と同じ）。GA4コンテンツ評価**専用**の一覧一括登録UIは作らない。<br>**2026-08-26 訂正（サイクル統合）:** サイクルがGSCと1本になったため、GSC側の一括評価開始（`docs/specs/gsc-bulk-evaluation-start-spec.md`。最大1,000件）を使うと、その記事群がそのままGA4コンテンツ評価の対象にもなる。つまり「GA4だけ一括登録しない」は保たれるが、「GA4が一括で対象になることはない」は**もはや成り立たない**。GA4未連携ユーザーはdue抽出（§8.3）で除外されるため無関係だが、連携済みユーザーが一括開始した場合は毎時のバッチ枠（`MAX_ARTICLES_PER_BATCH`＝20件）で順に消化されていく。
...
- **本文文字数**: `wp_content_text` を正規化（タグ置換で入った~~連続空白の圧縮~~、`&nbsp;` 等のエンティティのデコード）してから `length` を取る。正規化関数は純関数として新設しテストで固定する。
  - **2026-08-26 訂正（レビュー🟡）: 本書のこの記述は受領原文とずれている。**受領原文 §09 は「記事本文の文字数をツールのDBから取得（HTMLタグ・**空白を除いた**本文のみ）」と定めるが、本書は「連続空白の**圧縮**」と書き替えており、実装（`src/lib/content-text.ts:30` の `.replace(/\s+/gu, ' ')`）も圧縮どまりで空白が文字数に算入されている。
  - **影響**: `stripHtml`（`src/lib/utils.ts:8`）がタグ1個を空白1個へ置換するため、**タグの数だけ文字数が膨らむ**。膨張は常に一方向（文字数↑→期待読了時間↑→読了率↓→読了スコア↓）で統計的に相殺されない。日本語主体の記事で約2%、半角スペースの多い英数混じり記事では実測で約16%膨らむ。帯の境界にいる記事では診断コードが変わりうる。
  - **今回は直していない**（2026-08-26）。空白を除去すると**既存の全記事のスコアが動く**ため、Q-H（GA4画面との突合先）の決着と同じタイミングで、再評価の要否とあわせて判断する。**本書の記述を原文へ合わせるのが先**で、実装は追随扱いとする。
...
| 未評価フィルタ UI | `AnalyticsTable.tsx` / `CategoryFilter.tsx` | ~~既存 GSC 未評価フィルタ（`p_has_unstarted_gsc_evaluation`）と同パターンで GA4 版を追加~~ **2026-08-26 撤回（サイクル統合）。**フェーズ3で追加したGA4版（`p_has_unstarted_ga4_evaluation`）と、フェーズ2が同時に行った既存チップの改名（`評価未開始`→`検索順位評価未開始`）を**いずれも撤回し、develop 時点のフィルタへ戻した**（**ラベルのみ 2026-08-26 に「評価未設定」へ改めた。**絞り込み条件は develop と同一。§15.4）（`git show <merge-base>:src/components/CategoryFilter.tsx` と同一構成。したがって本スコープが一覧に追加した未評価発見手段は正味ゼロ）。RPC の引数は `default false` のまま残し、アプリ側から渡さない。**引数を残す理由**: 対象の Supabase プロジェクトには8引数版が適用済み（生成型が8引数で再生成されている）で、機能上無害な未使用引数を消すためだけに本番の関数を再定義するリスクを取らない。ただし `20260818000300` 自体が未マージのため、**未使用引数と死んだ述語を develop へ持ち込まない選択肢（同 migration から削除して型を再生成）も残っている**（§15.4「将来変更する条件」） |
...
期待読了時間（分） = 正規化後の本文文字数 ÷ 500          # 500字/分＝日本語の平均的な黙読速度
    # 「正規化後」＝HTMLタグと空白を除いた本文（受領原文 §09 実装チェック verbatim
    #   「記事本文の文字数をツールのDBから取得（HTMLタグ・空白を除いた本文のみ）」）。
    #   HTMLタグは取込側の stripHtml で落ち、空白は countContentChars が除去する
    #   （2026-08-26 修正。それまで空白を1個へ畳むだけで残しており、文字数が過大 →
    #    期待読了時間が過大 → 読了率が過小 → スコアが実際より低く出ていた）
...
平均エンゲージメント時間（秒） = userEngagementDuration ÷ activeUsers
    ...
    # 2026-08-26 追記: この名前は ÷activeUsers の値にだけ使う。/ga4-dashboard と /analytics 一覧が
    #   出しているのは sum(engagement_time_sec) ÷ sum(sessions) の別物で、表示名は「平均滞在時間」
    #   「滞在時間」とする（§10.7）。同名にすると同じ記事で2つの秒数が同じ名前で出て、
    #   GA4 と突き合わせたとき必ず片方が食い違う
...
>
> ~~**既知の未対応**: 一覧（`/analytics`）と `/ga4-dashboard` の「完読率」…は、**計測できた日だけを母数にした率**を出しているため、期間の一部しか計測されていない場合の値は評価側の全か無か判定とは一致しない。表示用途に限る割り切りとする。~~
>
> **2026-08-26 解消（レビュー🔴4）。3画面で分母と未計測の扱いを揃えた。**「完読率」（2026-08-25 に「読了率」から改称。§10.7 の時間ベース「読了率」とは別指標であり同名衝突を解消するため）の定義は受領原文 §02 の `scroll イベント数(90%到達) ÷ sessions` を正本とし、**期間内に1日でも未計測（NULL）があれば取得不可（`—`）**とする。
>
> **割り切りではなく実害だった。**放置していた「表示用途の割り切り」は、`20260819000300`（実測0を NULL 化）と `20260821000000`（分母を実測日に限定）の組み合わせで**上振れ**へ転じていた。前者で「実測して0回だった日」も NULL になるため、後者の filter が実測0の日を分母から外し、分母に「スクロールが1回以上あった日」しか残らない。PostgreSQL 16 での実測は **0.33% → 30.00%（90倍）**。
>
> | 画面 | 修正前の分母 | 修正後 |
> |---|---|---|
> | 記事詳細（評価パス） | `sessions` / 1日でも未計測なら取得不可 | 変更なし（元から仕様どおり） |
> | `/ga4-dashboard`（3 RPC） | `sum(users) filter (scroll is not null)` | `sum(sessions)`。未計測が混じれば `null` |
> | `/analytics` 一覧 | `users`（全日。未計測でも 0.0% と表示） | `sessions`。`scrollMetricsAvailable === false` なら `—` |
>
> **一覧が揃っていなかった理由**: 一覧は `aggregateGa4PageMetrics`（基本版）を使っており、そこの `scrollMetricsAvailable` は「1日でも実測があれば true」という**逆の意味**だった。全か無かを基本版へ移して評価パスと同じ意味に揃えた。
>
> **分母を `users` から `sessions` へ変えても今は数値が動かない。**取込側が `const users = sessions;`（`ga4ImportService.ts:448`。`totalUsers` は `landingPage` と非互換）としているため。ただし `users` を本物の `activeUsers` にした瞬間に静かに壊れるので、先に明示しておく。

#### 6.2.5 足切り

**本文0字の記事は採点しない**（2026-08-26 追加。レビュー🔴1）。`resolveInitialDisplayStatus` は `countContentChars(wp_content_text) === 0` を `missingMetrics: ['article_content']` として既に判定していたが、`run()` と `computeBaselineScore()` が `low_data` でしか離脱していなかったため素通りして採点されていた。期待読了時間が画像補正だけの数秒になり（本文0字・画像5点なら15秒）、読了率がアンカー上限（50%）を超えて**読了スコア100点＝「完全に読み込まれている」**に張り付く。
**実在経路**: `content_annotations` に行を作る3経路のうち `wp_content_text` を入れるのは WordPress 一括インポートだけで、そこでも本文が取れないと列ごと payload から落ちる。画像だけの投稿・`content` を返さない投稿タイプ・パスワード保護記事が該当する（前2つは `wordpressService.test.ts:176-190` が現に固定している）。チャット由来の2経路は常に NULL。**自己修復もしない**: 再インポートの `hasChanges` は `wp_content_text` を比較対象に含めず、`updateContentCache` は `.eq('wp_post_id', …)` で絞るため `wp_post_id` が NULL の行には永久に届かない。
**NULL 判定だけでは足りない**: `stripHtml` はエンティティをデコードしないため `<p>&nbsp;</p>` は `'&nbsp;'` として保存される（NOT NULL・非空）が、`countContentChars` はデコードしてから空白を畳むので0文字になる。本番調査の条件にはこれを含める。
**`unassessed` を一律に弾かないこと**: `ga4_data` / `engagement_rate` / `active_users` 欠落でも `unassessed` になるが、それらは `computeGa4Score` が `insufficient_data` として正しく扱う。
...
- 内容: **確定済み（2026-08-17 受領）**。`docs/context/ga4-evaluation-engine-spec-20260817.md` §07「システムプロンプト」の本文を verbatim で登録する。役割は**文章化のみ**（読者前提: ~~GA4 用語禁止~~・人数と時間で語る／書き方: 断定・次の一手は1つ・全体200字以内／点数帯と診断コードの意味／JSON のみ出力）。**「GA4 用語禁止」は 2026-08-26 の Q-L (b) 決着で反転した**（下記 §6.3.1.1。ただしクライアント合意が取れるまでは受領原文を登録する）。点数・診断の判定ルールはプロンプトに持たせない（コード側が正本。BR-09）。
- ~~**【重要・未確定】2026-08-22 の用語反転（§10.7）が、この受領原文の【読者の前提】に及ぶかは決まっていない（Q-L。§15.2）。**~~ **2026-08-26 決着: Q-L は (b)（プロンプトを改版し、診断文も GA4 名で書かせる）で確定した**（開発側の方針決定。ユーザー判断）。理由は D9 と同一で、利用者は GA4 を連携しており GA4 の画面をすでに見ているため、同じ数字がラベルと診断文で 2 つの名前を持つと突合できない。**ただしプロンプト本文はクライアント著作物であり（下記「登録経路」）、改版には D9 とは別のクライアント合意が要る。合意は未取得**（§15.2 Q-L / §15.0）。**合意が取れるまでは受領原文どおり verbatim を登録したままにする**（改版案は下記のとおり用意済みで、登録の差し替えだけで切り替わる）。
- **受領原文（`docs/context/ga4-evaluation-engine-spec-20260817.md`）は編集しない**（原文正本の原則。§18 2026-08-17）。改版案は本節にのみ置き、差分として管理する。
...
##### 6.3.1.1 Q-L (b) の改版案（クライアント合意待ち。2026-08-26 作成）

改版は**用語だけ**を対象とし、書き方のルール・点数帯・診断コードの意味・出力契約（5フィールドJSON）には触れない。表記の正本は §10.7、公式名の根拠は §16。

**(1) システムプロンプト【読者の前提】**（受領原文 `:229-232`）

| | 本文 |
|---|---|
| 受領原文 | `【読者の前提】`<br>`- 相手はGA4を見ません。専門用語を一切知りません。`<br>`- 「エンゲージメント率」「セッション」「直帰率」などの用語は出力に使わないでください。`<br>`- 数値は「率」ではなく「人数」と「時間」で語ってください。` |
| 改版案 | `【読者の前提】`<br>`- 相手はGA4を連携しており、GA4の画面も見ます。同じ数字が2つの名前を持つと突き合わせができないため、名前をGA4に合わせます。`<br>`- GA4から取得した指標は、GA4の日本語UIの名前で書いてください（セッション、エンゲージメントのあったセッション数、エンゲージメント率、平均エンゲージメント時間、アクティブ ユーザー数）。言い換えないでください。`<br>`- GrowMateが算出した指標（読了率・読み始めスコア・読了スコア・コンテンツ力スコア）は、GA4に対応する数字が無いため、そのままの名前で書いてください。`<br>`- 率だけで終わらせず、実数と時間もあわせて示してください。渡していない数値を率から換算して書かないでください。` |

**4行目を残す理由**: 受領原文の「率ではなく人数と時間で語る」は用語の話ではなく**読みやすさの要求**であり、D9 / Q-L の反転対象ではない。ただし GA4 名には「エンゲージメント率」のように率そのものが含まれるため、「率を使うな」のままだと (b) と衝突する。「率だけで終わらせない」へ緩めて要求の趣旨を保つ。後段の禁則（渡していない数値を率から換算しない）は §6.3.3 の既存規則をプロンプト側にも明示したもので、新しい制約ではない（`scroll_users` の実測が無い場合に人数を捏造させないため）。

**(2) ユーザープロンプト「# 計測結果（算出済み）」のラベル**（受領原文 `:270-276`）

§6.3.2 の入力契約では既に GA4 名へ反転済みだが、**受領原文のラベル自体は言い換え語のまま**である。LLM は与えられた名前をそのまま返すため、ここを直さないと (1) だけでは効かない。**変数名（`{{sessions}}` 等）は verbatim のまま変更しない**（§6.3.3）。

| 受領原文のラベル | 改版案 |
|---|---|
| `訪問した人数: {{sessions}}人` | `セッション: {{sessions}}` |
| `読み始めた人数: {{engaged_users}}人（全体の{{engagement_rate}}%）` | `エンゲージメントのあったセッション数: {{engaged_users}}（エンゲージメント率 {{engagement_rate}}%）` |
| `実際に読まれた平均時間: {{avg_time_display}}` | `平均エンゲージメント時間: {{avg_time_display}}` |
| `この記事を読み切るのに必要な時間: {{expected_time_display}}` | 変更なし（GrowMate の算出値） |
| `読了率: {{read_rate}}%` | 変更なし（GrowMate の算出値。§10.7） |
| `最後までスクロールした人数: {{scroll_users}}人（全体の{{scroll_rate}}%）` | 変更なし（GA4 の `scroll` イベント実測であって GA4 の指標名ではないため。§6.2.4） |

**(3) 出力形式の説明文と出力例**（受領原文 `:289` / `:297` / `:300`）

few-shot の出力例は文体を最も強く支配するため、(1)(2) を直しても例が言い換え語のままだとそちらへ引きずられる。

| 箇所 | 受領原文 | 改版案 |
|---|---|---|
| `:289` `situation` の説明 | `いま何が起きているか。人数と時間で説明する。80字以内` | `いま何が起きているか。実数と時間で説明する。80字以内` |
| `:297` 出力例 `situation` | `312人が訪れましたが、読み始めたのは134人だけです。残り178人はページを開いてすぐ戻っています。` | `312セッションのうち、エンゲージメントのあったセッションは134です。残り178はページを開いてすぐ戻っています。` |
| `:300` 出力例 `target` | `読み始めスコアを46点から80点へ（読み始める人を190人に）` | `読み始めスコアを46点から80点へ（エンゲージメント率を61%に）` |

`:300` の括弧内を人数から率へ変えたのは、受領原文自身の「1文40字以内」に収めるため（「エンゲージメントのあったセッションを190に」だと41字で超える）。**スコア名「読み始めスコア」は据え置き**（§10.7 の GrowMate 算出行）。

**(4) 触らないもの**: 【書き方のルール】（`:234-241`）・【点数の意味】（`:243-244`）・【診断コードの意味】（`:246-252`）・出力JSONのキーとフィールドの字数制限。用語の話ではないため。
...
**変数名は受領原文 verbatim であり変更しない**（テンプレートを verbatim 登録するため。**2026-08-26 の Q-L (b) 決着後も変数名は据え置く** — 改版対象は変数を囲む日本語ラベルだけである。§6.3.1.1 (2)）。**UI・メールの表示名は §10.7 に従う**ので、変数名と表示名は一致しない。とくに `engaged_users` は名前が `users` だが実体は `engagementRate × sessions` の丸め（＝エンゲージメントのあったセッション数の推定）であり、表示名は「エンゲージメントのあったセッション数」とする（§10.7 / §6.3.2）。
...
| ~~**当日の `syncUser` が失敗した実行**~~ **`syncUser` が失敗した実行**（結末の判定より前。下記「取込失敗の扱い」） | ~~**進めない（ただし当日限り）**~~ **進める（2026-08-26 変更。評価は実行せず、取込できなかったことを通知する）** | 取込済みデータが揃っていないため、この実行の結果は記事の実力を表さない。次の毎時実行で取込から再試行する |
...
- **2026-08-26 変更（レビュー🔴6）: 取込に失敗している間は評価そのものを実行しない。クールダウンは進め、取込できなかったことをユーザーへ通知する。**<br>**旧仕様の何が問題だったか**: 見送りを「当日限り」にしていたため、`ga4_next_evaluation_date` が過去になった瞬間に抑止が外れ、**古い取込データでスコアを出してスコア付きの「コンテンツ評価が完了しました」メールを送っていた**。数週間前の残存データで診断文まで生成され、ユーザーには取込が壊れていることが伝わらない。<br>**クールダウンを進める理由**: 進めないと毎時同じ記事を掴み続け、通知も毎時になる。進めても、再連携後は記事詳細の「今すぐ評価を実行」で次回予定日を待たずに評価できる（§10.4）。<br>**判定に `syncUser` の `reason` を使わない理由**: `{ok:false, reason:'not_connected'}` が返るのは `gsc_credentials.ga4_property_id` が無いときだけで（`ga4ImportService.ts:109-111`）、due 抽出RPCが `ga4_property_id is not null` で絞っている以上そこへは実質来ない。実際に起きるトークン失効は `ensureAccessToken` が例外を投げて catch 側へ落ちるため、`syncFailed` 全体で判定する。。
- ~~**無制限リトライを避けるため、見送りは `ga4_next_evaluation_date` が当日である間に限る。**~~ **2026-08-26 撤回（上記）。**クールダウンを進めるため無制限リトライにはならず、当日限定の条件自体が不要になった。旧記述: `ga4_next_evaluation_date < today_jst` になってもなお取込が失敗する場合は、通常どおりクールダウンを進める（結末は `import_failed` として記録し、§10.4 の画面で確認する）。毎時実行のため、見送りは最大でも24回に有界化される。**この上限を置かないと、GA4 側の障害が続く間ずっと同じ記事集合が毎時 due として抽出され、R-17 の 1,000 行枠を占有し続ける。**
...
このまま due に含めると、そのユーザーの記事は毎回 `syncUser` が `{ ok:false, reason:'not_connected' }` を返し、`withholdForSyncFailure`（§6.6.4）でクールダウンも進まないまま**毎時20件（`MAX_ARTICLES_PER_BATCH`）の実行枠を空回りで消費し続ける**。連携済みユーザーの評価が後回しになり、`data.failed > 0` でCronが慢性的に赤くなる。GSC側の一括評価開始（最大1,000件。`docs/specs/gsc-bulk-evaluation-start-spec.md`）と組み合わさると規模がさらに大きくなる。
...
- ~~`unassessed` / GA4未評価を**フィルタ**できるようにする（GSC未評価フィルタ `p_has_unstarted_gsc_evaluation` と同型）。~~<br>**2026-08-26 廃止（サイクル統合。ユーザー判断）。**サイクルを登録した時点でGSC検索順位評価とGA4コンテンツ評価が同時に始まるため、**系統別の「未開始」という状態が存在しなくなった**。実装されていたGA4版（`p_has_unstarted_ga4_evaluation`）は `ga4_content_evaluations` の行の有無＝「結果があるか」を見ており「開始したか」ではなかったため、サイクル登録済みでも本評価が走る2サイクル目まで true のまま残り、その間ユーザーに取れるアクションが無かった。<br>**2026-08-26 追加（フィルタ条件の開示）: チップの直下に既定で畳んだ「絞り込まれる条件」を置く**（`CategoryFilter.tsx`。`details` / `summary` は `ContentEvaluationCard.tsx:143` と同じ既存パターン）。**目的は「絞り込んで出てこなかった記事」にユーザーが気づけるようにすること。**チップのラベルからは「サイクル未設定だけが対象」「設定済みで結果が出ていない記事（初回の計測待ち・Google Analytics 4未連携・セッション30未満）は含まれない」が読み取れず、下記の限界が黙って効いてしまうため。あわせて代替の確認手段（既定で表示される「コンテンツ評価状態」列。`ANALYTICS_COLUMNS` の `ga4_evaluation_status`。ラベルは 未評価／評価可能／データ蓄積中／評価済み 等）を同じ場所で案内する。**既定で畳むのは通常の絞り込み操作の邪魔をしないため**で、常時見えるのは summary の1行だけ。**フィルタ自体の条件は変えていない**（表示のみ）。<br>**撤回後の仕様: 一覧の状態フィルタは「評価未設定」1つ**（~~「評価未開始」~~ から 2026-08-26 に改称。§15.4。条件は `gsc_article_evaluations` に行が無い＝サイクル未登録。RPC 引数は既存の `p_has_unstarted_gsc_evaluation` をそのまま使う）。**これは develop 時点の構成そのもの**（ラベルを除く。上記）で、フェーズ2がチップを `検索順位評価未開始` へ改名しGA4版を足していたのを撤回した形になる（新規の追加ではない）。`p_has_unstarted_ga4_evaluation` は `default false` のまま SQL に残し、アプリ側から渡さない（**migration は追加しない**。理由は §5.1）。<br>**旧 deep link `?ga4_evaluation=not_started` の扱い（正確に）**: 絞り込みが効かない理由は `app/analytics/page.tsx` が当該パラメータを読まなくなったこと。`AnalyticsTable.tsx` の `pushFilterQuery` も当該キーを落とすが、**同関数はフィルタ操作・ページ送り・期間変更のいずれかが起きるまで呼ばれない**ため、素の deep link で開いた直後は URL 文字列としてパラメータが残る。`hasUrlFilterParams` の判定対象外で、ページング用 href も Instagram 用 href も空の `URLSearchParams` から組み立てるため伝播せず、**ユーザーに見える影響は無い**。マウント時の強制クリーンアップは実装しない（CLAUDE.md「要件に無い機能を先回りで作らない」）。
...
4. **診断文**: `headline`（見出し）と `situation` / `cause`（何が起きているか・なぜか）。~~**注意: 診断文の語彙は §10.7 の適用外かどうかが未確定である（Q-L。§15.2 / §6.3.1）。**~~ **2026-08-26 決着: Q-L は (b)。診断文も §10.7 の適用対象とし、GA4 から取得した指標は GA4 名で書かせる**（§6.3.1.1 に改版案）。**ただしプロンプト改版のクライアント合意は未取得であり、合意・登録が済むまでは同じカード内でラベル（GA4 名）と診断文（言い換え語）が混在したままになる。**この混在は「決着済みだが未反映」であって未確定ではない。実装者はプロンプト登録以外の手段（診断文の後処理・置換など）で解消しない。
...
| GA4 平均エンゲージメント時間 | 平均エンゲージメント時間 | GA4 の生指標。~~**この名前が指す数字の分母（÷`activeUsers` か ÷`sessions` か）は Q-H で未確定**~~ **→ 2026-08-26 訂正: 分母は未確定ではない。**受領仕様 §02 が verbatim で `userEngagementDuration ÷ activeUsers` と定めており（§6.2.1）、Q-H で未確認なのは**その値が「ページとスクリーン」画面と一致するか**だけである（軸が `landingPage` と `unifiedPagePathScreen` で違うため。§16 / §15.2）。<br>**この表示名は ÷`activeUsers` の値にだけ使う**（記事詳細の評価カード・評価履歴・通知メール・LLM入力）。**`/ga4-dashboard` と `/analytics` 一覧が出している `sum(engagement_time_sec) ÷ sum(sessions)` は別物**なので「平均滞在時間」「滞在時間」と呼ぶ（下行）。GA4 API 側もこの2つを `averageEngagementTime` と `averageEngagementTimePerSession` として別指標に分けている（§16 verbatim） |
| GrowMate 集計値 `sum(engagement_time_sec) ÷ sum(sessions)` | 平均滞在時間（一覧の列見出しは「滞在時間（平均）」・ランキング列は「滞在時間」） | **GA4 名を当てない**（2026-08-26 決定）。分母が `sessions` で、上行の「平均エンゲージメント時間」（÷`activeUsers`）とは別の数字になるため、同じ名前を付けると**同じ記事で2つの秒数が同じ名前で表示され、GA4 と突き合わせたとき必ず片方が食い違う**。D9 の目的（GA4 名にして突合できるようにする）を裏切るので、D9 の適用対象から外す。<br>2026-08-22 の用語反転で `SummaryCards.tsx` / `TimeseriesTab.tsx` / `Ga4DashboardClient.tsx` / `AnalyticsClient.tsx` の4箇所を「平均エンゲージメント時間」へ改名してしまっていたのを、2026-08-26 に develop の表記へ戻した。**RPC・計算式・列は変更していない（表示層のみ）** |
...
**辞書・コードの追随が必要（~~2026-08-22 時点で未了~~ → 2026-08-26 に一部解消。下表の「状態」列）:** 先行実装（commit `26eea718`）と `ui-text.md` は上表の GA4 公式表記と一致しない。**追随が済むまで、突合の目的はこれらの箇所について達成されない。**修正対象は §17 に記載する。実測（2026-08-22）で確認した不一致は次のとおり。

| 箇所 | 現行 | 上表（正） | 状態 |
|---|---|---|---|
| `ui-text.md:40` | エンゲージのあったセッション | エンゲージメントのあったセッション数 | **解消済み**（辞書は追随済み） |
| `ui-text.md:46` | セッション / ユーザー | セッション / アクティブ ユーザー数 | **解消済み**（現在は `:47`。2026-08-26 に `訪問数` / `総訪問数` を「使わない表記」へ追加し、`目視` → `✅` へ上げた） |
| `ui-text.md:48` の根拠文 | 「Search Console・GA4 とも日本語 UI は『表示回数』」 | GA4 側に単独の「表示回数」指標は無い（§16）。表示回数は GSC 専用行 | 未了 |
| `ContentEvaluationCard.tsx:269,274` | ファネルのラベル「訪問」「読み始め」 | 「セッション」「エンゲージメントのあったセッション数」（§10.3-3）| **解消済み** |
...
- 機械検査が効くのは `ui-text.md` の「機械チェック」列が ✅ の行だけであり、`目視` 行（~~`セッション / ユーザー`~~ **2026-08-26 に ✅ へ**・`表示回数`・`キーイベント` 等）・SQL・メール本文組み立て関数は対象外である。これらは `quality-gate` の手動確認へ回す（§13 / AC-17）。
- **プロンプト本文（システム／ユーザー）は 2026-08-26 の Q-L (b) 決着で適用対象に入った**が、本文は DB（`prompt_templates.content`）にあり `/admin/prompts` から手動登録するため、リポジトリ内の機械検査は永久に届かない。**クライアント合意と登録差し替えが済むまでは診断文が言い換え語のまま出る**（§6.3.1.1 / §10.3-4）。確認は §14 手順9 の目視に依存する。
...
```gherkin
  ...
  Scenario: 系統別の「未開始」フィルタが存在しない
    ...
    Then チェックボックスは「評価未設定」と「改善提案あり」の2つで、
...
- E2E: 未評価フィルタ、記事詳細の「コンテンツ評価」タブ表示、評価実行、評価中表示、評価結果と根拠表示、前回結果保持、データ不足・失敗・Google連携確認表示、移設後の `/analytics/[annotationId]` で既存3タブが変わらないことの確認（AC-14）。**2026-08-26 追加（目視）: 完読率が未計測の記事で、ファネルに「最後まで」を出さず、代替文が「1人あたり平均で全体の N% まで読まれています」＝**読了率**で出ること**（完読率ではない。受領原文 §08 の 12% は直前の文の読了率を指す。§18）。`tests/` に `.tsx` が0件でコンポーネントテスト基盤が無いため自動テストでは担保できない。
...
- E2E（フェーズ3）: ~~「概要」タブでサイクルを登録すると次回評価予定が表示されること、「検索順位評価サイクル設定」と横並びに並ぶこと、~~ **2026-08-26 訂正:** 概要タブは「検索順位・コンテンツ評価サイクル設定」**1枚だけ**になったこと、その1枚で登録すると両タブの次回評価予定が表示されること、コンテンツ評価タブ側には設定導線が無く次回予定のみ読み取り表示されること（GA4未連携時はその行も出ないこと）、**GSCの評価サイクルが未登録でもGA4が評価可能な状態なら「今すぐ評価を実行」が表示され、隣の「評価サイクルを開始」と文言で区別できること**（J1 の回帰防止）、**`/ga4-dashboard` のサマリーカードが5枚で、同じ数値のカードが並ばないこと・時系列タブの折れ線が重ならないこと**（2026-08-26 追加。`.tsx` のコンポーネントテスト基盤が無いため目視で拾う）、**`/analytics` の状態フィルタが「評価未設定」「改善提案あり」の2つだけであること・チップ直下の「絞り込まれる条件」が既定で畳まれており開くと3項目が出ること（§10.2）・`?ga4_evaluation=not_started` を付けても絞り込みが変わらないこと**（AC-10 の回帰シナリオ。`.tsx` のコンポーネントテスト基盤が無いため自動テストでは担保できず、手動確認で拾う）、**他タブがカード名を引用している4箇所（`ContentEvaluationCard.tsx` ×3・`EvaluationHistoryTab.tsx` ×1）の文言が実際のカード見出しと一致していること**（改名時の追随漏れ防止）。
...
9. **確定した文章化プロンプト（評価エンジン仕様 §07 の verbatim。§6.3.4 の5フィールド契約と一致）を `/admin/prompts` から GA4 評価用テンプレートとして登録する**（§6.3.1。画面改修は不要）。<br>**2026-08-26 追記（Q-L (b)）: 用語を GA4 名へ揃えた改版案を §6.3.1.1 に用意してある。クライアント合意が取れている場合は verbatim ではなく改版案を登録する。**合意前は verbatim を登録し、合意後に同画面から本文を差し替える（コード変更・再デプロイは不要。プロンプト本文は DB にあり `prompt_templates` から読むため）。差し替え後は1記事を再評価し、診断文が GA4 名で書かれることを目視で確認する。
...
|---|---|---|---|---|
| **D9** | クライアント承認ゲート | **開発側は承認（2026-08-26）／クライアント合意は未取得** | 承認済みのクライアント仕様（評価エンジン仕様 §08「ツール内でGA4用語を全面禁止」）を覆す変更。**2026-08-26 にユーザー（開発側）が反転を承認**し、判断根拠も「利用者も GA4 の管理画面で用語を見ているため一致させたい」と明示された。**実装・辞書・本仕様書は反転側で確定**。残るのは**クライアントへの提示と合意のみ**で、開発側の判断待ちは無い。合意が得られない場合は差し戻す | §15.3 D9 |
| **Q-L** | クライアント確認（実装判断の前提） | **方針は (b) で確定（2026-08-26）／クライアント合意は未取得** | ~~用語反転が文章化システムプロンプトへ及ぶかが決まっていない~~ → **及ぶ。(b)（プロンプトを改版し診断文も GA4 名で書かせる）で確定**（ユーザー判断）。改版案は §6.3.1.1 に用意済み。**プロンプト本文はクライアント著作物のため、改版には D9 とは別の合意が要る（未取得）。**合意・登録が済むまでは記事カード・メール本文でラベル（GA4 名）と診断文（言い換え語）が混在する。**差し替えはコード変更を伴わない**（`/admin/prompts` の本文差し替えのみ） | §15.2 Q-L / §6.3.1.1 |
| **Q-H** | 外部入力・実測 | **未確認（数値突合待ち）。2026-08-26 に射程を訂正** | §10.3 の「生の数値を GA4 の画面と一字一句一致させる」が**達成可能か**が未確認。<br>~~「平均エンゲージメント時間」の分母（÷`activeUsers` か ÷`sessions` か）も公式ドキュメントから取得できていない~~ **→ 訂正。分母は受領仕様が決めている**（§02 verbatim「= userEngagementDuration ÷ activeUsers ※GA4「ページとスクリーン」の平均エンゲージメント時間と同じ定義」）。**取得できていないのは GA4 日本語 UI の定義文であって、採用すべき式ではない**（§16）。§6.2.1 の本文は当初から ÷activeUsers で確定と書いており、**この行だけが未確定であるかのように書かれていた。**<br>**残る射程**: 受領仕様は同じ §02 で「※必ず landingPage 次元で取得する」とも定めるが、「ページとスクリーン」画面の集計軸は `unifiedPagePathScreen`（§16 verbatim）。**軸が違えば行のまとまりが変わるため、landingPage 軸 ÷activeUsers の値がその画面と一致する保証が無い。**一致しなければ「一字一句一致」は構造的に達成できず、Q-H は実測案件からクライアント合意案件（D9 と同じ扱い）へ変わる | §15.2 Q-H / §6.2.1 |
| **D12** | クライアント承認ゲート | **開発側は承認（2026-08-26）／クライアント合意は未取得** | 一覧の「未評価」の定義を**「評価サイクルが未登録」へ限定**した変更が未合意。`client-vision-from-lark.md` §1.9.5 の原語は「**評価が走っていない**ものをソートできれば手動運用で代替可能」で、新定義は**サイクル登録済みだが評価が一度も走っていない記事**（初回計測待ち・GA4未連携・`low_data`）を対象外にする。この発見手段は「全記事自動評価を作らない」という Non-goal の**代替手段**として §3.2 が明示していたもの。合意が得られない場合は、評価状態列での絞り込みを別途設計する必要がある | §15.3 D12 / §3.2 / §2.3 |

~~**この3件が閉じるまで、本スコープの仕様レビューは完了扱いにしない。**~~ **この4件が閉じるまで、本スコープの仕様レビューは完了扱いにしない。**

**2026-08-26 追記（「誰が閉じるか」を分ける）:** 同日のユーザー判断で **D9・Q-L・D12 の3件は開発側の意思決定としては閉じた**（D9＝反転を承認／Q-L＝(b) を採用／D12＝新定義を承認）。**残っているのはクライアントへの提示と合意だけ**であり、開発側の判断待ちは1件も無い。したがって:

- **実装は止まらない。**3件とも実装は反転側・新定義側で確定しており、追加の実装判断を要しない。
- **合意が得られなかった場合の差し戻し範囲は変わらない。**D9＝表示文言の実装やり直し、Q-L＝プロンプト本文の差し替え（コード変更なし。§6.3.1.1）、D12＝評価状態列での絞り込みを別途設計（§3.2）。
- **本節の「未解決」は以後「クライアント未合意」の意味である**（Q-H だけは実測待ちで性質が異なる）。**ユーザー（開発側 PO）の承認をクライアント合意として記録しない。**両者は別の主体であり、混同すると承認ゲートが機能しなくなる。
...
| Q-H | 「GA4 画面と一字一句一致」の突合先はどの画面か（「ページとスクリーン」＝÷activeUsers か「ランディングページ」レポート＝÷sessions か） | GA4 公式 predefined-reports（2026-08-17 取得。§16）では、Pages and screens が `userEngagementDuration/activeUsers`、Landing page レポートが `userEngagementDuration/sessions` と**画面によって定義が異なる**。landingPage 軸で取得する本実装の生値がどちらの画面と一致すべきかを数値で確定する必要がある。**Q-G は互換で決着（÷activeUsers）したため、有力仮説は「ページとスクリーン」一致だが、数値での確認が未了**（2026-08-17 の実測プロパティはデータ0行で突合不能だった） | §10.3 の突合要件（評価エンジン仕様 §08「GA4の画面と一字一句一致させる」）の検証方法 | 開発チーム実測（データのあるプロパティ・期間で） → クライアント確認 | フェーズ2の実データ検証前 | **未確認（数値突合待ち）。2026-08-26 に射程を訂正した。**<br>~~「÷activeUsers か ÷sessions か」を選ぶ質問~~ ではない。**分母は受領仕様 §02 が verbatim で ÷activeUsers と定めており、`averageSessionDuration` は名指しで排除されている。**§6.2.1 も当初から ÷activeUsers で確定と書いている。<br>**実測で見るのは1点だけ**: 受領仕様が必須と指定する `landingPage` 軸で `userEngagementDuration ÷ activeUsers` を計算した値が、「ページとスクリーン」画面（集計軸は `unifiedPagePathScreen`。§16 verbatim）の同じ URL の行と**一致するか**。軸が違うため一致しない可能性がある。<br>**手順**: (1) データのあるプロパティ・期間を選ぶ（2026-08-17 の実測は rows 0件で空振りした）、(2) Query Explorer で `dimensions=[landingPage]`, `metrics=[userEngagementDuration, activeUsers]` を取る（Q-G で受理は実測済み）、(3) 自分で除算する、(4) GA4「エンゲージメント > ページとスクリーン」の同期間・同URLの値と比べる。<br>**不一致だった場合**: 「軸は landingPage にせよ」と「ページとスクリーン画面と一致させよ」が両立しないことをクライアントへ示し、どちらを採るか問う（実測案件 → 合意ゲートへ変わる） |
...
|---|---|---|---|---|---|---|
| ~~**一覧の状態チップの文言を「評価未開始」のまま据え置く**~~ **一覧の状態チップを「評価未設定」へ改称する**（2026-08-26。D12 の付随論点。**同日中に (a) 据え置き → (c) 改称へ反転した。**下記「反転の経緯」） | (a) `評価未開始`（現行＝develop と同一） / (b) `評価未登録` / (c) `評価未設定` / (d) `評価サイクル未設定` | **(c) を採用（ユーザー判断）。**チップの実際の条件は「サイクル行が無い」であり、「未開始」は**サイクル登録済みで初回評価がまだ走っていない記事**（D12 が対象外にした集合）にも日本語として当てはまるため、ラベル単体では境界が読み取れない。`設定` を採ったのは、同機能の**状態**表現が記事詳細で既に「まだ評価サイクルが**設定されていません**」（`EvaluationSettings.tsx:491`）であり、`登録` は利用者向け文言に一度も出てこないため。`開始` はボタン（「評価サイクルを開始」）の**動作**の動詞で、状態語ではない。<br>**反転の経緯**: 当初は下記の理由で (a) 据え置きを採ったが、ユーザー判断で (c) へ反転した。据え置きの根拠だった「develop と一字一句同じ」は D12 の提示条件 (a) の**補強**にすぎず、条件の本体（撤去したフィルタは本ブランチが追加したもの／絞り込み条件は develop と同一）は改称後も成立する。D12 の提示条件 (a) は「画面は一切変わらない」と言わない形へ書き換えた。<br>~~**(a) を採用。**理由は2つ。**(1) develop の一覧と一字一句同じであること自体が D12 の提示条件 (a)「クライアントが現に見ている画面は変わらない」の根拠になっている。**改名するとこの条件を取り下げることになり、承認を得たい相手に「知らないうちに画面の文言も変わっている」という論点を新たに渡す。**(2) 本スコープが一覧に追加する発見手段は正味ゼロ（§2.3）であり、文言だけを変えると「何かが増えた」という誤った印象を与える。~~ | **(a) 却下**: 実際の条件と読みが食い違うラベルを、develop と同一であることだけを理由に残すのは本末転倒。**(b)「未登録」却下**: `登録` は利用者向け文言に一度も出てこない語であり、新しい語彙を1語増やすことになる。**(d)「評価サイクル未設定」却下**: チップとして長く、一覧画面には他にサイクルの概念が出てこないため `サイクル` が浮く。<br>**改称しても残る限界（重要）**: (b)(c)(d) のいずれを採っても、**サイクル登録済みで初回計測待ち・GA4未連携・`low_data` の記事が絞り込めない事実は変わらない。**ラベルはその境界を読み取れるようにするだけで、機能の穴は塞がない。塞ぐなら §3.2 が示すとおり**評価状態列での絞り込みを別途設計する**（D12 が合意に至らない場合の (d) と同じ）。**2026-08-26 追加: 穴を塞ぐ代わりに、穴の存在をチップ直下で開示することにした**（既定で畳んだ `details`。§10.2）。絞り込んで出てこなかった記事にユーザーが気づけないのが実害であり、**気づけるようにするだけなら機能を足さずに済む**ため（`CLAUDE.md`「要件に無い機能を先回りで作らない」）。 | 一覧（`/analytics`）のチップと解除ボタンの `title`（`CategoryFilter.tsx:100` / `AnalyticsTable.tsx:812,817`）、`ui-text.md` の辞書、D12 の提示条件 (a)。**RPC 引数 `p_has_unstarted_gsc_evaluation` と URL パラメータ `gsc_evaluation=not_started` は改称しない**（DB とブックマーク済み URL に触れないため。表示名と内部名の不一致は §6.3.3 と同じ割り切り）。 | **D12 がクライアント合意に至らず (d)「評価状態列での絞り込み」を別途設計する場合**、状態列フィルタと並ぶことになるため、そのタイミングで両者の呼び分けを設計しなおす | ユーザー判断 / 2026-08-26 |
...
- **2026-08-26 追記（Q-H の射程を切り分ける）**: この引用が記録している **Pages and screens の dimension は `unifiedPagePathScreen`** であり、受領仕様が必須と指定する `landingPage` とは**別の軸**である。したがって Q-H は「式を選ぶ質問」ではない（式は受領仕様が ÷activeUsers に固定済み）。**軸の違いにより、landingPage 軸 ÷activeUsers の値がこの画面の行と一致しない可能性が残る**という点だけが未確認であり、それが Q-H の全射程である。あわせて、Landing page レポート側の指標名が `averageEngagementTimePerSession`（＝per session）であることは、**÷sessions の値に「平均エンゲージメント時間」という無印の名前を付けてはならない**根拠にもなる（§10.7 / `ui-text.md`）。
...
| 2026-08-26 | **`verify:doc-paths` の恒常的な失敗3件を解消した。**本セッション中、この3件を毎回「本件と無関係の既知の失敗」と呼んで `git commit --no-verify` で回避してきたが、**実際には無関係ではなかった**。`docs/specs/gsc-bulk-evaluation-start-spec.md` が参照していた `app/gsc-dashboard/{page.tsx, EvaluationSettings.tsx, components/OverviewTab.tsx}` は、**本ブランチのフェーズ1（§5.5 のルート移設）が `/analytics/[annotationId]` へ移した**もので、追随漏れは本ブランチ自身の責任範囲だった（merge-base では `app/gsc-dashboard/` が存在することを実測確認）。<br>同仕様の該当12箇所を実態へ更新し、あわせて**認可に関する記述の食い違い3箇所**も訂正した（フェーズ1で `registerEvaluation` に `canWriteGa4` が入り、移設先が proxy のプレフィックスマッチで保護されたため、同仕様の「`trial` でも単記事開始が可能」という前提と、それを根拠にした Non-goal が失効していた）。詳細は `gsc-bulk-evaluation-start-spec.md` §16 の変更履歴。<br>**これにより `npm run verify:doc-paths` が green になり、`--no-verify` なしでコミットできるようになった。**今後この種の追随漏れは pre-commit で普通に検知される。 | `npm run verify:doc-paths` が「陳腐化した参照はありません」で成功。`npx tsc --noEmit`・`npm run lint`・`npm run test`・`npm run knip`・`npm run build`・`npm run verify:ui-text` もすべて成功 |
| 2026-08-26 | **未解決3件（D9 / Q-L / D12）について開発側の意思決定を確定した（ユーザー判断）。**<br>**(1) D9 承認**: UI 用語を GA4 の名前へ戻す反転を承認。判断根拠は「利用者も GA4 の管理画面で用語を見ているため一致させたい」で、これは §15.3 D9 の提示条件 (g)（`client-vision-from-lark.md:88` `:114` でクライアント自身が GA4 用語で指標を列挙している）と同じ向き。**クライアントへの提示・合意は未実施のまま**であり、承認者欄は「開発側承認済み／クライアント未共有」へ改めた。**ユーザー（開発側 PO）の承認をクライアント合意として記録しない**方針を §15.0 に明記した（両者を混同すると承認ゲートが機能しなくなるため）。<br>**(2) Q-L は (b) で確定**: 用語反転を文章化プロンプトへも及ぼす。**改版案を §6.3.1.1 として新設**し、(1) システムプロンプト【読者の前提】4行／(2) ユーザープロンプト「# 計測結果（算出済み）」のラベル3行／(3) 出力形式の説明文と出力例2箇所 を受領原文との対比表で示した。**受領原文（`docs/context/ga4-evaluation-engine-spec-20260817.md`）は編集していない**（原文正本の原則）。**改版の射程についての発見**: 【読者の前提】だけを直しても効かない — ユーザープロンプトのラベル（「訪問した人数」「読み始めた人数」）と few-shot の出力例が言い換え語のままだと LLM はそちらへ引きずられるため、3点セットで直す必要がある。**変数名（`{{sessions}}` 等）と【書き方のルール】・点数帯・診断コード・出力JSON契約は据え置き**。プロンプト本文は DB（`prompt_templates.content`）にあり `/admin/prompts` から手動登録するため、**この改版はコード変更・再デプロイを伴わない**（§14 手順9 に差し替え手順を追記）。クライアント合意までは verbatim を登録したままにする。<br>**(3) D12 承認**: 一覧の「未評価」を「評価サイクルが未登録」へ限定する定義を、対象外になる集合（サイクル登録済みで初回計測待ち・GA4未連携・`low_data`）を明示したうえで承認。クライアントへは未共有。<br>~~**(4) 状態チップの文言は「評価未開始」で据え置き**~~ **→ 同日中に反転。下行を参照（据え置きの根拠にした「develop と一字一句同じ」は提示条件の補強にすぎず、条件の本体は改称後も成立する、と整理しなおした）。**旧記述: （ユーザーからの「未登録がいいか」という提案への回答。§15.4 に判断行を追加）。「未開始」がラベル単体では境界を示さないという指摘は正しいが、**改名しても初回計測待ちの記事が絞り込めない事実は変わらず**、解消するなら §3.2 のとおり評価状態列での絞り込みを別途設計するのが筋。据え置きの決め手は、**develop と一字一句同じであること自体が D12 の提示条件 (a)「クライアントが現に見ている画面は変わらない」の根拠になっている**こと。改名する場合の第一候補は「未登録」ではなく**「未設定」**（記事詳細が既に「まだ評価サイクルが設定されていません」と書いており、`登録` は利用者向け文言に一度も出てこない） | 開発側の意思決定は3件とも確定。**クライアント合意（D9 / Q-L / D12）と Q-H の実測は未了のまま**で、§15.0 の4件は開いたまま。コード変更なし（文書のみ） |
| 2026-08-26 | **一覧 `/analytics` の状態チップを「評価未開始」→「評価未設定」へ改称した（ユーザー判断。同日の据え置き決定を反転）。**改称の理由は、チップの絞り込み条件が「`gsc_article_evaluations` に行が無い＝サイクル未登録」であるのに対し、「未開始」は**サイクル登録済みで初回評価がまだ走っていない記事**（D12 が対象外にした集合）にも日本語として当てはまり、ラベル単体では境界が読み取れないため。`登録` ではなく `設定` を採ったのは、同機能の**状態**表現が記事詳細で既に「まだ評価サイクルが設定されていません」（`EvaluationSettings.tsx:491`）であり、`登録` は利用者向け文言に一度も出てこないため（`開始` はボタンの**動作**の動詞で状態語ではない）。<br>**改称に伴って直した前提**: 据え置きの根拠にしていた「develop と一字一句同じ」は **D12 の提示条件 (a)** にも書いてあったため、**「クライアントが現に見ている画面は変わらない」→「絞り込み条件は develop と同一。ただしラベルは変わったので『画面は一切変わらない』とは言わない」**へ書き換えた（改称の理由も条件文に入れた）。§2.3 / §3.2 / §5 / §10.2 の「develop 時点の構成そのもの」という記述にも同じ但し書きを足した。<br>**改称しない内部名**: RPC 引数 `p_has_unstarted_gsc_evaluation` と URL パラメータ `gsc_evaluation=not_started` は据え置く（DB とブックマーク済み URL に触れないため。表示名と内部名の不一致は §6.3.3 の変数名と同じ割り切り）。<br>**改称しても残る限界（明示）**: (b)(c)(d) のどれを採っても、サイクル登録済みで初回計測待ち・GA4未連携・`low_data` の記事が絞り込めない事実は変わらない。ラベルは境界を読み取れるようにするだけで機能の穴は塞がず、塞ぐなら §3.2 の「評価状態列での絞り込み」を別途設計する。<br>変更は `CategoryFilter.tsx:100` / `AnalyticsTable.tsx:812,817` の表示文言と、それを指すコメント・テスト名・`ui-text.md`・仕様書の記述 | 実装・文書とも反映済み。**D12 のクライアント合意は引き続き未取得**（§15.0）。判断の記録は §15.4 |
| 2026-08-26 | **「評価未設定」フィルタの絞り込み条件を、チップ直下に既定で畳んだ形で開示した（ユーザー判断）。**前行の改称でラベルの読みは正したが、**「設定済みで結果が出ていない記事（初回の計測待ち・Google Analytics 4未連携・セッション30未満）は絞り込めない」という限界そのものは残る。**実害は限界の存在ではなく、**絞り込んで出てこなかった記事にユーザーが気づけないこと**なので、機能を足さず開示で対応した（`CLAUDE.md`「要件に無い機能を先回りで作らない」）。<br>**置き場所を記事詳細の評価サイクル設定ダイアログではなく一覧のフィルタ直下にした理由**: 実際に困るのは「一覧で絞り込んだのに出てこない」瞬間であり、サイクルを登録している瞬間ではない。`CategoryFilter` は `FieldConfigurator` の `dialogExtraContent` として表示項目設定ダイアログ内に描画されるため、チップと注意書きが同じダイアログに並ぶ。<br>**代替の確認手段を同じ場所で案内する**: 「コンテンツ評価状態」列（`ANALYTICS_COLUMNS` の `ga4_evaluation_status`。`defaultVisible !== false` なので既定で表示される）に 未評価／評価可能／データ蓄積中／評価済み 等が出る。**情報は元から同じ画面にあり、欠けているのは絞り込みだけ**という事実を可視化した。<br>**UI は既存パターンに合わせた**: `details` / `summary` は `ContentEvaluationCard.tsx:143`（「不足項目を確認」）と同型。常時見えるのは summary の1行だけで、通常の絞り込み操作を邪魔しない。**フィルタの条件・RPC・URL は変更なし（表示のみ）** | 実装・文書とも反映済み。§10.2 / §15.4 に記載、E2E の目視項目に追加（`.tsx` のコンポーネントテスト基盤が無いため自動テストでは担保できない） |
| 2026-08-26 | **PR全体の総合レビュー（6観点・並列サブエージェント）で確定した🔴4件とテストの穴3件を修正した。**観点は 算出エンジン／認可・秘密情報／migration 13本／バッチ・冪等性／既存機能への回帰／テスト実効性。**認可の🔴は0件**で、むしろ本PRは既存の穴（`ga4Dashboard.actions.ts` はロール検証が無く trial がデータを取得できた等）を複数塞いでいることが確認された。<br>**(1) 評価が「実行中」で永久固着する（migration `20260826000100`）**: `returns table(evaluation_run_id uuid)` の OUT パラメータが `ga4_content_evaluation_history` の同名列と衝突し、リース期限切れの回復パスが実行時に必ず ambiguous エラーで落ちていた。パース時ではないためこの分岐を通らない限り露見しない。評価が途中で死ぬと `status='evaluating'` が残り、以後その記事への `start` は毎回中断。`cancelRun` は自分の runId を要るため孤児化したリースを解除できず、**アプリ側に復旧経路が無かった**（本番DBへの直接SQLが必要）。UPDATE 対象にエイリアスを付けて列を修飾する。適用済みの既存4本は触らず新規 migration で `create or replace`。**データ移行は不要**で、修正後は次の `start` で自動復旧する。あわせて `error_code='evaluation_run_expired'` が初めて実際に書かれるようになった（それまで到達不能で、表示ラベルと型の union はデッドコードだった）。PostgreSQL 16 で再現→修正→回復まで実測し、実行中ガード・service_role ガードの非退行と再適用の冪等性も確認した。<br>**(2) 本文0字の記事に読了スコア100点（§6.2.5）**: 詳細は同節。<br>**(3) 完読率が3画面で3通りの定義（§6.2.4 の注記・migration `20260826000200`）**: 詳細は同節。実測 0.33%→30.00%（90倍）。<br>**(4) GA4を取込めていないのに評価メールが飛ぶ（§6.6.4）**: 詳細は同節。<br>**(5) テストの穴3件**: ミューテーション44件中22件が生存し、**すべてサービス／アクションのオーケストレーション層に集中**していた（純関数層はほぼ全て killed）。バグの所在とテストの穴が一致している。塞いだのは ①`user_id` 境界（`FakeQuery.eq()` が引数を捨てており、`.eq('user_id')` を外しても全テストが通っていた。Service Role 経由で RLS がバイパスされる以上これが唯一の防御層）②スコアの配線（`toMatchObject` + `expect.any(Number)` のため `p_read_score` と `p_engage_score` の取り違えも `p_content_score` へのセッション数混入も通っていた。実値を手計算して固定）③`narrative_failed` と失敗カウンタ（`result.failed` が非0になる入力が無く、**全記事が失敗してもジョブが緑になる**状態だった）。修正後、生存していた6ミューテーションがすべて killed になることを再実行して確認した | 実装・文書とも反映済み。**保留**: 🔴2（`activeUsers` の日次和）は **Q-H が「÷sessions」で決着すればバグ自体が消える**ため Q-H 待ち。🟡（バッチ締切のコメント矛盾・文字数への空白算入・`/ga4-dashboard` のラベル逆転・`checkCompatibility` 未使用）は未着手 |
| 2026-08-26 | **総合レビューの🟡のうち、クライアント合意に依存せず小さく閉じるものを処理した。**<br>**(1) GA4取込の単一障害点を解消**: `resolveScrollEventNameForProperty`（`ga4ImportService.ts:146`）に try/catch が無く、例外がそのまま `syncUser` を抜けて **baseReport すら取得されずその回の取込が丸ごと失敗**していた。完読率という任意指標のために、`/ga4-dashboard`・一覧の全列が依存する共有経路を落としていたことになる。失敗時は未計測（null）として続行する。<br>**(2) 記事開始の締切を155秒へ統一**: 定数のコメント（`:29-31`「最悪所要135秒の猶予を残せる時点でなければ開始してはいけない」）と実装が矛盾していた。進捗ゼロの間だけ155秒・1件でも進めば280秒に戻す作りだったが、**280秒側も同じ理由で155秒を超えて開始してはいけない**（279秒開始なら最悪414秒で maxDuration 300秒を超える）。常に155秒で締める形へ統一し、`BATCH_TIME_LIMIT_MS` は結末ログの観測用途に残した。処理窓は 280→155秒に縮むが、毎時実行で `MAX_ARTICLES_PER_BATCH`=20 が先に効くため実質的な取りこぼしは増えない。逆に maxDuration による中断は `status='evaluating'` の残骸を作る。<br>**(3) 完読率が取れないときの代替文言を読了率へ**: 受領原文 §08 の「取得できない場合は人数を出さず『1人あたり平均で全体の12%まで読まれています』と表記する」の 12% は**直前の文の読了率**を指すが、実装は完読率（scroll率）を渡していた。入った場合に読了率と食い違う文がLLMへ渡る。`score.readRate` を渡す形へ直した。<br>**(4) `/ga4-dashboard` を `PAID_FEATURE_REQUIRED_PATHS` へ追加**: 本PRでこの画面にGA4評価のメディア別スコアを載せたのに、proxy のリストに無く trial でも画面枠まで到達していた（データは Server Action が弾くので漏洩は無いが `/unauthorized` へ飛ばない）。<br>**(5) `/analytics` ページ本体に `canAccessGa4` を追加**: 本PRは他の12箇所すべてにサーバー側ゲートを足しており、GA4評価4列を新設したこの画面だけが proxy の前方一致1箇所に依存していた（`CLAUDE.md`「UIだけでなくサーバー側でも検証する」）。<br>**(6) `list_due_ga4_content_evaluations` を他のRPCと同じ堅牢化パターンへ**（migration `20260826000300`）: 同PRの他7本は `security definer` + `set search_path = public` + `auth.role()` ガードで揃えているのに、この1本だけ外れていた。**本機能のロール規約（`u.role in ('admin','paid')`）を担保する唯一のSQL**である。抽出結果は変わらない。<br>**(7) `constants.ts` の宣言の無いJSDoc残骸を削除。**<br>**(8) §4.1.1 の文字数記述を原文へ訂正**（下記）| 実装・文書とも反映済み。ローカル PostgreSQL で (6) の拒否・通過・冪等を実測。**未着手の🟡**: `/ga4-dashboard` のラベル逆転（D9 とセット）、未読提案の通知条件から `suggestion_summary IS NOT NULL` が消えた件（揃える向きの判断が要る）、`retryNarrative` の現在値／履歴値の食い違い、`/api/gsc/dashboard` の `q` 埋め込みと生エラー返却（いずれも本PR以前からの既存）、`checkCompatibility` 未使用（§3.4 / §4.1.1 が「将来の再検証手段として残す」と既に明記しているため今回は触らない） |
| 2026-08-26 | **`/ga4-dashboard` の指標ラベルを「セッション」へ統一し、同じ数値を2つ見せていた重複表示を削除した（ユーザー判断。D9 の適用）。**<br>**発見: ラベルが逆だった。**GA4 の `sessions` を「総訪問数」と呼び、その代用値でしかない `users` の方が「セッション」を名乗っていた（`SummaryCards.tsx`）。`users` は取込側が `const users = sessions`（`ga4ImportService.ts`。`totalUsers` は `landingPage` 軸と非互換で、CVR の分母を確保するための代用）としているため **`sessions` の完全なコピー**で、2枚のカードは常に同じ数値だった。時系列タブでも `sessions`（緑）と `users`（青）が**同じY軸に**プロットされ、後から描かれる青が緑を完全に覆っていた（凡例で片方を消しても見た目が変わらない状態）。`MediaContentScorePanel` に至っては同一パネル内で `sessions` を「訪問数」（`:37`）と「セッション」（`:70` の散布図）の2通りに呼んでいた。<br>**対応**: `sessions` を指す表示をすべて「セッション」にし、`users` 由来のカード1枚・折れ線1本・ツールチップ1行を削除した。**表示層だけで閉じており migration は不要。**`totalUsers` は CVR の分母として4経路（一覧・サマリー・ランキングRPC・時系列RPC）で使われ続けるため、列・RPC・型には触っていない。<br>**再発防止**: `ui-text.md` の該当行は「使わない表記」に**「訪問した人、訪問した人の数」しか載せておらず**、実際の逸脱形の `訪問数` / `総訪問数` が無かった。さらに `目視` 指定で機械チェックの対象外だった。その理由として書かれていた「『セッション』は認証・チャットでも使うため機械チェックしない」は**検査の仕組みの誤解**で、`dictionary_pairs`（`scripts/check-ui-text.sh:93-121`）は「使わない表記」列だけをルール化するため、正表記が他文脈で使われても検査に影響しない。逸脱形2語を追加したうえで `✅` へ上げた（辞書 22→26語、誤検出0）。ラベルを戻すミューテーションで実際に検出されることも確認した | 実装・文書とも反映済み。**やらないと決めたもの**: `active_users` を返す RPC 変更（本PRで追加した列で、実データは「過去90日を再取込」の実行後になるため、見せるかは再取込後の実データで判断）、`users` 列そのものの改名・削除（CVR の分母として現役。`active_users` は NULL 可なので差し替えると未計測時に 0.0% が出て BR-02 に反する）、**ランキング列「滞在時間」→「平均エンゲージメント時間」（同じ数字がソート欄・見出し・一覧で3つの名前を持つが、ダッシュボードは `÷ sessions`・評価パスは `÷ activeUsers` で計算が違い、分母の違うものに同じ GA4 公式名を付けると「GA4 画面と一致する」という誤った主張になる。Q-H とセットで処理する）**、`ga4-evaluation-display.ts` の「訪問データ」（`sessions` を指すラベルではなく欠損理由の表示のため D9 の対象外） |
| 2026-08-26 | **未読改善提案の通知条件から `suggestion_summary IS NOT NULL` を落としていたのを戻した（本PRが持ち込んだ回帰。残る🟡の再評価で判明）。**<br>**判断の誤りは「事実」ではなく「揃える向き」にあった。**削除時のコメントは「`get_filtered_content_annotations` の `p_has_unread_suggestion` の EXISTS 句と揃える」と書いており、RPC（`20260818000300`）が `suggestion_summary` を見ていないのは**事実として正しい**。誤っていたのは、**通知側を RPC に合わせた**こと。この通知の文言は「N件のコンテンツに**改善提案があります**」で、提案文が存在するという主張そのものなので、NULL の行を数に含めると文言が嘘になる。RPC 側は「未読の評価結果がある記事」を絞る**別目的**の述語であり、**一覧フィルタと通知で条件が違うのは意図的**である（次に「不整合だから揃えよう」と再発しないよう明記する）。<br>**運用に支障が出る仕組み（4点とも実コードで確認）**: (1) 提案文は別ジョブが後から書く。`gsc-evaluate` が history 行を INSERT した瞬間に `outcome` / `outcome_type` / `is_read=false` が揃い削除後の4条件を満たすが、`suggestion_summary` を書くのは毎時の `gsc-suggestions` で `JOBS_PER_INVOCATION = 1`（`gscSuggestionJobService.ts:9`）＝**全ユーザー合計で1時間に1件**。一方 `gsc-evaluate` は1回で最大10ユーザー×期限到来記事の全件を積むので待ち行列は構造的に長い。(2) 生成に3回失敗した行は `suggestion_next_retry_at = null` に固定され**永久に NULL のまま**残る。理論値ではなく `docs/runbooks/gsc-suggestion-timeout-recovery-2026-07-12.md:16-18` に **171件中8件（約4.7%）**の実績がある。(3) その行は `evaluation-history-view.ts:89-93` の `canMarkAsRead` が `Boolean(suggestion_summary)` を要求するため **UI から既読にできない**（`markSuggestionAsRead` 自体に summary 条件は無く、サーバー制約ではなく UI ゲート）。(4) トーストは `duration: Infinity`（`GscNotificationHandler.tsx:94`）。**帰結**: 恒久 NULL 行が1件でもあると「1件のコンテンツに改善提案があります」が出続け、開くと「AI改善提案を生成できませんでした」と表示され、**消す手段が UI に存在しない**。<br>**同じクエリが一覧のベルアイコンも供給している**（`getAnnotationIdsWithUnreadSuggestions` → `app/analytics/page.tsx:232` → `AnalyticsTable.tsx:932`）ため、1行戻すとトーストと一覧が同時に正しくなる。<br>**テストを新設**（`tests/unit/server/services/gscNotificationService.test.ts`。このサービスのテストは**0件だった**）。フィルタ呼び出しを記録するモックで、`suggestion_summary` の NULL 除外・BR-06 の `user_id`・既読／エラー／改善済みの各条件・記事単位への畳み込み・失敗時0件を固定した。**戻した1行を外すとテストが落ちる**ことをミューテーションで確認済み | 実装・文書とも反映済み。**据え置き（根拠つき）**: `retryNarrative` の現在値／履歴値の混在（発生に3条件のANDが要り、DBは汚れずメールにも出ない。`context.article.url` は現在値必須のため history 全置換はできず設計判断が要る）、`/api/gsc/dashboard` の `q` 埋め込みと生エラー返却（マージベースと一字一句同じ既存。`.eq('user_id')` はトップレベルANDのままで漏洩には至らない）、文字数への空白算入とランキング列「滞在時間」（Q-H とセット）、`canMarkAsRead` の緩和とトーストの dismiss 永続化（通知が真実になれば消せないトーストは発生しない） |
| 2026-08-26 | **Q-H の射程を訂正し、それ待ちにしていた🟡2件を処理した（受領仕様の読み直しで判明）。**<br>**訂正の中身**: §15.0 / §15.2 の Q-H 行は「『平均エンゲージメント時間』の分母（÷`activeUsers` か ÷`sessions` か）も公式ドキュメントから取得できていない」と書いていたが、**分母は受領仕様が決めていた**。`ga4-evaluation-engine-spec-20260817.md` §02 は verbatim で「平均エンゲージメント時間（秒） = userEngagementDuration ÷ activeUsers ※GA4「ページとスクリーン」の平均エンゲージメント時間と同じ定義 ※averageSessionDuration は使わない（GA4画面と数値が合わなくなる）」と定めており、**÷sessions は名指しで排除されている**。取得できていないのは GA4 日本語 UI の**定義文**であって採用すべき式ではない。§6.2.1 の本文は当初から「÷activeUsers で確定」と書いており、**Q-H 行だけが未確定であるかのように書かれていた**（同じ論点を2箇所で要約し直した結果のズレ。§15.0 が「本節が唯一の正本」と定めているのに、その正本側が本文と食い違っていた）。<br>**Q-H に残る射程**: 同じ §02 が「※必ず landingPage 次元で取得する」とも定める一方、「ページとスクリーン」画面の集計軸は `unifiedPagePathScreen`（§16 の predefined-reports verbatim）。**軸が違えば行のまとまりが変わるため、landingPage 軸 ÷activeUsers の値がその画面と一致する保証が無い。**Q-H の問いは「どちらの分母か」ではなく **§08 の「一字一句一致させる」が構造的に達成可能か**であり、実測で見るのは1点（landingPage 軸の計算値がページとスクリーン画面と一致するか）だけになった。不一致なら実測案件からクライアント合意案件（D9 と同じ扱い）へ変わる。<br>**🟡(1) ダッシュボードの指標名を戻した**: 2026-08-22 の用語反転で `SummaryCards.tsx` / `TimeseriesTab.tsx` / `Ga4DashboardClient.tsx` / `AnalyticsClient.tsx` の4箇所を「平均滞在時間」→「平均エンゲージメント時間」へ改名していたが、これらが出しているのは `sum(engagement_time_sec) ÷ sum(sessions)` で**分母が違う**。結果、**同じ記事で記事詳細（÷activeUsers）とダッシュボード（÷sessions）が同じ名前で違う秒数を出す**状態になっていた。develop では「平均滞在時間」「滞在時間」と名前が分かれていたので衝突していない。**D9 の目的は「GA4 名にすれば突合できる」ことなので、突合すると合わない数字に GA4 名を付けたのは D9 の適用として誤り。**develop の表記へ戻した（ユーザー判断。第一候補だった「セッションあたりの平均エンゲージメント時間」は長く、日本語 UI の実在名としても未確認のため不採用）。**表示層のみ。RPC・計算式・列は変更なし。**Q-H の結論は待たなくてよい（÷sessions がこの名前を名乗れないことは受領仕様で既に確定しているため）。<br>**🟡(2) 文字数から空白を除いた**: 受領仕様 §09 実装チェックは verbatim で「記事本文の文字数をツールのDBから取得（**HTMLタグ・空白を除いた本文のみ**）」と定める。HTMLタグは取込側の `stripHtml` で落ちていたが、`content-text.ts` の `normalizeContentText` は `\s+` を半角スペース1個へ**畳むだけで除去していなかった**ため空白が文字数に入っていた。文字数 → 期待読了時間 → 読了率 → コンテンツ力スコアと伝播し、**スコアが実際より低く出る**（日本語主体の記事では差は小さいが、英数字やコードブロックを含む記事では効く）。`countContentChars` 側で除去した（`normalizeContentText` の他用途には影響しない。呼び出し元は `countContentChars` とテストのみ）。副次的に、**空白だけの本文が0字と数えられるようになり**、`ga4ContentEvaluationService.ts:206` の「本文0字は採点しない」ガード（🔴1）が実際に効くケースが増えた。テストを3件追加し、除去処理を外すミューテーションで3件とも落ちることを確認した。**据え置きの根拠だった「Q-H とセット」は、Q-H が分母を変えうるという前提に立っていたもので、その前提が消えたため単独で処理した。** | 実装・文書とも反映済み。**クライアントへの説明が要る**: 空白除去により**既存の全記事のスコアが動く**（方向は上振れ。文字数が減る→期待読了時間が減る→読了率が上がる）。D9 / D12 / Q-L の提示資料に1項目として載せる。**Q-H は引き続き未確認**で、実測できるプロパティが無ければ「軸の不一致により一致しない可能性」自体をクライアントへの質問として同じ資料に載せる |
| 2026-08-26 | **受領原文（HTML 正本）と実装を §01〜§09 まで全項目突き合わせ、残っていた1件の追随漏れを直した（ユーザーから HTML 仕様書を「正本」として再提示された）。**<br>**照合の結論: 計算エンジンは完全一致。**確認したのは — 期待読了時間（文字数÷500・画像×3秒）／平均エンゲージメント時間（÷`activeUsers`）／読了率（÷期待読了時間）／読み始め率（`engagementRate`・`landingPage` 軸）／完読率（scroll÷`sessions`）の各式、足切り30、点数帯5区分（0-19/20-39/40-59/60-79/80-100）、アンカー2組の数値、`score()` の線形補間と丸め、幾何平均（仕様例 100×20→45 も一致）、4×3マトリクス**12セル全部**、完読率併用の2ルール（40%以上×読了40未満→流し読み型／15%未満×読了40未満→冒頭離脱型で確定）、診断コード6つと診断名、資産価値スコア（単純平均）／実効スコア（セッション加重平均）、プロンプト変数26個、§09 実装チェック12項目。§04 の注記も検算が合う（9,000字で読了率6%→1分5秒、3,000字なら1分48秒で80点）。**13項目目の「用語言い換え表をUI全体に適用」だけは D9 で反転済み（クライアント未合意）。**<br>**「読了率を人数に換算しない」禁則も守られている**ことを確認した。ファネルの「最後まで」は scroll の実測値だけを出し、実測できない記事では枠ごと出さない（`ContentEvaluationCard.tsx:237`）。「エンゲージメントのあったセッション数」は `round(engagementRate × sessions)` だが、`engagementRate` はセッション加重平均（`ga4-metrics-aggregation.ts:131`）で、**評価パスは1日でも欠測があれば `null` に倒す**（同 `:167`）ため、定義の逆算であって推定値ではない。<br>**直した1件（同じ誤りを2箇所に持っていて片方だけ直していた）**: `ContentEvaluationCard.tsx:252` の代替文言が**完読率**を引用していた。受領原文 §08 は「取得できない場合は人数を出さず『1人あたり平均で全体の**12%**まで読まれています』と表記する」と書いており、この 12% は**直前の文の「読了率12%」**を指す。読了率は平均時間の比なので「全体の何%まで読まれたか」が成り立つが、完読率は90%到達イベントの割合なので同じ文に入れると意味が変わる。**しかも分岐条件が `measuredScrollUsers === null`（完読率の実測が無いとき）なので、完読率が無いときに完読率を出そうとしている構造だった**（多くの場合 `scrollRate` も null で「表示していません」へ落ちるため、実害は「本来出すべき読了率の一文が出ていない」ことが主）。`latest.readRate` を出す形へ直した。**同じ誤りのプロンプト側は同日の🟡処理 (3) で先に直しており、カードが追随していなかった。**次に `scrollRate` へ戻されないよう、根拠をコメントに書いた。<br>**既知の見え方（直さない）**: `readRate` は 1 を超えうる（期待読了時間より長く読まれた場合）ため「全体の120%まで読まれています」と出る記事がある。受領原文はこのケースに触れておらず、**要件に無い分岐を先回りで作らない**（`CLAUDE.md`）方針で文言を足さない。<br>**あわせて `retryNarrative` 据え置きの但し書きを補強**: 「発生に3条件のAND」のうち**記事更新の条件は当初の見積もりより緩い**。`wordpressImport.actions.ts:269-279` の更新条件はタイトル・抜粋・canonical_url・post_type・カテゴリ・画像点数のいずれかの差分で成立し、そのとき `wp_content_text` も一緒に書き換わるため、**WordPress 側でタイトルを直しただけでも本文キャッシュが更新される。**ただし一括インポートに cron は無く手動のみ（`app/api/cron/` に WordPress 経路は無い）ため、**据え置きの結論自体は変えない** | 実装・文書とも反映済み。`tests/` に `.tsx` が0件でコンポーネントテスト基盤が無いため、本件は自動テストで担保できない。§13 の E2E 目視項目へ追加した |

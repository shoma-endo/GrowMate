...
| 項目 | 内容 |
|---|---|
| ステータス | **承認済み（2026-08-16）**。確認質問（Q-A / Q-B / Q-C / Q-D / Q1〜Q4 / Q7 / Q8）と D1 / D3 / D4 は承認とは別に引き続き確定待ち（§15）。Q-E / Q-F / D5 は 2026-08-15 に決着（§15.1 / §15.3） |
| 作成日 | 2026-08-12 |
| 対象 | GA4評価機能の初期実装 |
| 承認者 | **承認済み（ユーザー、2026-08-16）**。本文（2026-08-15 の新ルート移設方針・認可再設計・RLS自己参照のみを含む）を承認。個別合意の記録: Q6 合意済み（2026-08-13）、Q-E / Q-F / D5 決着済み（2026-08-15）。**D1（見積合意）と D3 / Q-C（UIたたき台・挙動保存移設の合意）は本承認とは別に確定待ち**（§15.3） |
| 最終更新 | 2026-08-16 |
| 作成者 | GrowMate 開発チーム |
...
この文書は、会議内容・既存コード・Google公式仕様をもとにした実装前の仕様書である（2026-08-16 承認済み）。繁田さんのプロンプトで決まる評価観点・出力項目は、確定後に本書へ反映する。§15.2 の確認質問と §15.3 の D1 / D3 / D4 が未確定の間は `spec-to-pr` へ進まない（§17）。
...
**期限について:** 個別の期日はまだ設定されていない。全件の回答期限は「D1（見積合意）の提示時」に確定させる。それまでは無期限に開いた状態であることを明示し、未回答のまま `spec-to-pr` へ進まない（§17）。ただし **Q-D（migration 着手前）は、D1 とは別に着手工程に紐づく期限を持つ**（下表の期限欄を正とする。Q-E / Q-F は 2026-08-15 に回答済み・§15.1）。
...
- 実装前に本書をレビューし、§15.2 の未確定事項（Q1〜Q4・Q7・Q8・Q-A〜Q-D。Q-E / Q-F は回答済み・§15.1）と §15.3 の開発側確定事項（D1 見積合意、D3 UIたたき台の合意（Q-C と同時）、D4 期間上限・ページング方式、**D5 `GlobalToastBridge` の扱い**）を確定したうえで、テーブル項目・API契約・画面文言を固定する。**フェーズ1の着手には D5、リリースには Q-E / Q-F が必要である（§15.3 依存関係表）。****未回答のまま `spec-to-pr` を実行しない。****Q-A が未回答の間は追加指標の取込とAC-11を実装対象にしない。**

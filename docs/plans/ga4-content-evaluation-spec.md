...
| 項目 | 内容 |
|---|---|
| ステータス | **承認済み（2026-08-16）**。2026-08-16 に Q1〜Q4 / Q7 / Q8 / Q-A / Q-B / Q-C が回答され D3 も決着（§15.1）。残る確定待ちは **D1（見積合意）／D4（打ち切り対処方式）／Q-D（リモートDB照会）／繁田さんのシステムプロンプト最終契約**（§15.2 / §15.3）。Q-E / Q-F / D5 は 2026-08-15 に決着 |
| 作成日 | 2026-08-12 |
| 対象 | GA4評価機能の初期実装 |
...
この文書は、会議内容・既存コード・Google公式仕様をもとにした実装前の仕様書である（2026-08-16 承認済み）。繁田さんのプロンプトで決まる評価観点・出力項目は、確定後に本書へ反映する。**`spec-to-pr` の実行ゲート（§17）: D1（見積合意）が未合意の間は実行しない。フェーズ2の実装着手には、加えて D4・Q-D の確定とシステムプロンプト最終契約（§15 繁田確認 #1〜5）の受領が必要。**
...
| 2. 手動評価MVP | 1記事単位の評価・保存・表示を提供する | GA4取込拡張（`pagePath` 軸の `screenPageViews` / `engagementRate`。§4.1.1）、現在状態projection・履歴・settingsのmigration/RLS、所有者検証trigger、`start_`/`finish_ga4_content_evaluation` RPCとDBテスト、一覧RPCへの成功履歴JOINと未評価フィルタ、評価サービス、手動API、DB Kill Switch、stale回復、打ち切り検知（D4 次第）、評価用エンドポイントの認可ガード、一覧の評価状態列、記事詳細の「コンテンツ評価」タブ、情報階層の統合レイアウト再設計（Q-C）、実データ検証 | 121〜172h ＋ 統合レイアウト（未見積） |

MVP合計は **175〜257時間（22〜33人日）＋ 統合レイアウト再設計（未見積。D1 提示時までに見積る）**。2026-08-16 の回答反映による差分は、GA4取込拡張 12〜20h の算入（Q7）、一覧の並び替え 4〜6h の削除（Q-B）、統合レイアウト再設計の追加（Q-C。未見積）である。**取込拡張 12〜20h は `landingPage` 軸への追加を前提とした概算であり、Q-A 回答（記事自体のPV）により `pagePath` 軸の別クエリが必要になったため、§4.1.2 の後方互換対応と合わせて D1 提示時に精査する。**フェーズ0では、GSC・GA4の全体的な共通化、無関係な既存サービスの再設計、画面仕様の変更を行わない。
...
| GA4取込拡張 | `pagePath` 軸での `screenPageViews` / `engagementRate` 取得追加（Q-A / Q7 回答 2026-08-16）、`checkCompatibility` 経路の新設と実測、`ga4_page_metrics_daily` の追加指標 migration、§4.1.2 の後方互換対応 | 12〜20h（`landingPage` 軸前提の概算。D1 提示時に精査） |
| UI | 一覧の評価状態列・未評価フィルタ、記事詳細の「コンテンツ評価」タブ（状態・主操作・点数・診断・提案・根拠・履歴） | 20〜30h |
| UI（統合レイアウト） | 記事詳細の情報階層を 2026-08-13 合意たたき台の統合レイアウトへ再設計する（Q-C 回答 2026-08-16「まとめで全てやる」） | 未見積（D1 提示時までにたたき台を基に見積る） |
...
| `/analytics/[annotationId]` | 1記事の詳細。フェーズ1で `/gsc-dashboard?annotationId=...` から移設（既存は概要・検索クエリ分析・評価履歴の3タブ） | フェーズ1: 既存3タブを挙動保存で移設（§5.5 / AC-14）。フェーズ2: 「コンテンツ評価」タブを追加し、**情報階層を 2026-08-13 合意たたき台の統合レイアウトへ再設計する**（Q-C 回答 2026-08-16。§10.5）。既存3タブの機能・データは維持し、配置・情報階層はたたき台に従う |
...
|---|---|---|---|---|---|---|
| D1 | MVP見積（フェーズ0 30〜45h + フェーズ1 24〜40h + フェーズ2 121〜172h = **175〜257h ＋ 統合レイアウト再設計（未見積）**）を着手前に合意する | §3.4 の各フェーズ内訳。2026-08-16 の回答反映で、GA4取込拡張 12〜20h を算入（Q7。ただし `pagePath` 軸化に伴い提示前に精査）、並び替え 4〜6h を削除（Q-B）、統合レイアウト再設計を追加（Q-C。**提示前にたたき台を基に見積る**）。<br>**フェーズ1（24〜40h）は利用者に見える変化を伴わない**（既存3タブの挙動保存移設・redirect・認可の多層化）。フェーズ0（30〜45h。同じく挙動不変）と合わせると **54〜85h（MVP の約1/3）が、クライアント第1優先要求（`client-vision-from-lark.md` §1.9.5「**優先**: ①未評価コンテンツのソート ②評価機能の追加。」）の価値提供より前に置かれる**。この配分そのものを見積提示時に明示して合意する（開発側の都合による前倒し作業であることを隠さない） | 着手判断 | 開発チーム → クライアント | 未設定 | 未合意 |
...
- 仕様書HTML束: `docs/plans/_html/ga4-content-evaluation-spec/` は **2026-08-16 の回答一括反映を反映済み**である（`core.yaml` の 4分類・pagePath 軸・175〜257h＋統合レイアウト未見積・ゲート再定義）。**本文をさらに改訂した場合は、`core.yaml` の `source_label` / 行数参照と `source_refs` のアンカー行番号がずれるため、spec-to-html の手順で貼り直して `npm run spec-html:refresh` で再生成する。**なお `docs/plans/_html/` は `.gitignore:69` で除外されており、リポジトリの成果物には含まれない。
...
- 実装前の確定状況（2026-08-16 更新）: Q1〜Q4 / Q7 / Q8 / Q-A / Q-B / Q-C / Q-E / Q-F は回答済み（§15.1）、D3 / D5 は決着済み（§15.3）。**`spec-to-pr` の実行ゲート: D1（見積合意）が未合意の間は実行しない。**D1 合意後はフェーズ0・フェーズ1に着手できる。**フェーズ2の実装着手には、加えて D4（打ち切り対処方式）・Q-D（`page_views` 列のリモート照会）の確定と、システムプロンプト最終契約（繁田確認 #1〜5。出力JSONフィールド名・点数の意味）の受領が必要である。**

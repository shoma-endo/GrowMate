...
| 2026-08-19 | **記事カードの視覚階層をクライアント提供のモックに合わせた。** 表示項目は §10.3 の1〜7から変えていない（データの追加・削除なし）。変更したのは見せ方のみ: 記事タイトルをカード見出しに出す／コンテンツ力スコアを大きな数字＋点数帯のピルにする／読み始め・読了はラベル＋数値＋バーを1行に並べる／人数ファネルは数字を大きくラベルを下に置く／診断見出しを点数帯の色で出す／NEXT ACTION と狙いを独立した箱にまとめる。点数帯の色は文言（要改善・良好など）と併記して色だけに頼らない（§10.3 の「状態は色だけで表現せず」）。内容幅は `max-w-3xl` に絞った（全幅だとバーが伸びきって大小差が読み取れない） | 見せ方のみの変更。仕様の表示項目は不変 |

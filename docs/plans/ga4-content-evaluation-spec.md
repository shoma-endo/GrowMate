...
- ~~**GSC との違いは「初回計測」の概念を持たないこと。**~~ **D10（2026-08-24）で反転。GSC と同じくベースライン（初回計測）を持つ。**サイクル登録時（`onRegister`）に即座に評価を1回実行してベースラインを取得し、**基準日から1サイクル後が初回評価**となる（GSC は基準日から2サイクル後。§6.6.2の`initialMeasurementDate`/`initialEvaluationDate`計算はGSCと同じ`addDays`連鎖を踏襲するが、起点がサイクル登録時点である点がGSCの「次サイクルを待って計測」と異なる）。<br>**ベースラインの保存先（D10確定）**: コンテンツ評価は**単発で完結する**設計（前回との比較は §6.4 の前回差分として任意に付くだけで、前回が無くても評価は成立する）ため、GSCのように次評価との「順位の変動」を主目的にしていない。ベースラインの評価結果は `ga4_content_evaluation_cycles.last_seen_content_score`（§7.7）にのみ保存し、`ga4_content_evaluation_history` には行を追加しない。理由: (1) 前回差分（§6.4）やメディア全体スコアの幾何平均（§6.2.3 / §10.6）にベースライン値が混入すると集計が歪む、(2) 記事詳細「コンテンツ評価」タブの評価状態（§10.4 / `ga4_content_evaluations`）はベースラインで更新しない（サイクル設定の状態カードのみで完結させる。GSCの`hasCompletedInitialMeasurement`判定と同じ考え方）。画面のスケジュールプレビューはGSCと同じ3枠「基準日／初回計測日／初回評価日」とする（§10.8）。
...
| `last_seen_content_score` | integer | NULL 可, CHECK 0〜100 | **ベースライン（初回計測）のコンテンツ力スコア。GSCの`last_seen_position`と同じ役割・同じ用途（D10確定。2026-08-24）。「登録済み・未計測」と「計測済み」の判定に使う（`EvaluationSettings.tsx`の`hasCompletedInitialMeasurement`相当）。`ga4_content_evaluation_history`には残さない（§6.6.2）** |
...
```gherkin
  Scenario: サイクルを新規登録する
    ...
    And 登録と同時にベースライン評価が1回実行され、last_seen_content_score が記録される（D10確定。ga4_content_evaluation_history には行を追加しない）
...
| **D10** | **コンテンツ評価サイクルの登録時にベースライン（初回計測）を取得するか。** (a) GSC と同じくベースライン概念を導入する（採用）／(b) 現行どおりベースラインなし・単発完結のまま | 2026-08-24 のユーザー依頼「GSC検索順位の評価と同様に設定した際に評価スタート（初動評価はゼロになる）し、1ヵ月で評価を行う」。GSCは基準日から2サイクル後が初回評価（3枠プレビュー）だが、依頼の「1ヵ月で評価」と両立するには、**登録時に即座にベースラインを取得し、基準日から1サイクル後を初回評価とする**解釈が必要（GSCの「次サイクルを待って計測」とは起点が異なる）。ベースラインの保存先は別途論点: 履歴（`ga4_content_evaluation_history`）に残すと前回差分・メディア全体スコアの幾何平均が歪むため、**サイクル行（`ga4_content_evaluation_cycles.last_seen_content_score`）にのみ保持し、履歴には残さない**（ユーザー確認済み。2026-08-24） | §6.6 揃え方の正本表、§6.6.2、§7.7 列定義、§10.8 プレビュー・状態カード | 開発チーム → ユーザー | フェーズ3実装着手前 | **確定（2026-08-24、ユーザー確認）: (a) を採用。**保存先はサイクル行にのみ保持し履歴に残さない方式で確定 |

...
| 評価種別由来（設定操作ではない） | 初回計測あり（プレビュー3枠）。サイクル日数を取込期間に使う | **初回計測あり（プレビュー3枠。登録時にベースラインを取得。D10確定 2026-08-24）。評価期間は90日固定で、サイクル日数は取込期間に使わない**（§6.6.2 / §6.6.3） | **ダイアログのプレビュー3枠はGSCと揃える（D10）。**~~状態カード分岐（3枚/2枚）もGSCと揃える~~ **状態カードは常に2枚へ簡略化（2026-08-25。§10.8参照）。理由はGSCとの差異ではなく、GA4側の3枚目がフロント独自計算の不誠実な推測値だったため**。評価対象期間の扱いはGSCと異なる（コンテンツ評価は単発完結のため90日固定のまま）。ベースラインは`ga4_content_evaluation_cycles`にのみ保持し、`ga4_content_evaluation_history`には残さない（§7.7） |
...
```gherkin
  Scenario: サイクルを新規登録する
    ...
    And スケジュールプレビューは GSC と同じ「基準日／初回計測日／初回評価日」の3枠で表示される（D10確定。2026-08-24）

...
| 項目 | 内容 |
|---|---|
| ステータス | レビュー指摘反映済み・確認質問回答待ち（Q-A / Q-B / Q-C / Q-D / Q-E / Q-F / Q1〜Q4 / Q7 / Q8 / D1 / D3 / D4 / D5） |
| 作成日 | 2026-08-12 |
| 対象 | GA4評価機能の初期実装 |
| 承認者 | **未承認**。Q6（評価テーブルのRLSは自己参照のみ）は合意済み（2026-08-13。当時の表現「owner-only」は共有モデル廃止済みのため §15.1 に補記）。**2026-08-15 に記事詳細を `/analytics/[annotationId]` へ移設する方針へ反転し（§15.4）、2026-08-13 合意たたき台（新ルート前提）と再整合。挙動保存移設と統合レイアウトの残差分は D3 / Q-C で合意待ち**、見積は合意待ち（D1） |
| 最終更新 | 2026-08-15 |
| 作成者 | GrowMate 開発チーム |
...
- フェーズ2: 移設後の記事詳細 `/analytics/[annotationId]` に「コンテンツ評価」タブを追加し、そこで評価の実行・結果・履歴を表示する。
...
- 記事詳細画面の情報階層の再設計（2026-08-13 合意たたき台の統合レイアウト化）、レスポンシブ・アクセシビリティの新規要件定義。フェーズ1の移設は挙動保存とし、評価タブは移設後の画面の作りに従う。
...
| 2. 手動評価MVP | 1記事単位の評価・保存・表示を提供する | 現在状態projection・履歴・settingsのmigration/RLS、所有者検証trigger、`start_`/`finish_ga4_content_evaluation` RPCとDBテスト、一覧RPCへの成功履歴JOINと未評価フィルタ・並び替え、評価サービス、手動API、DB Kill Switch、stale回復、打ち切り検知（D4 次第）、評価用エンドポイントの認可ガード、一覧の評価状態列、記事詳細の「コンテンツ評価」タブ、実データ検証 | 113〜158h |

MVP合計は **167〜243時間（21〜31人日）**。GA4取込拡張（12〜20h）は Q-A 回答待ちのため合計に含めていない。取込追加が確定した場合は §4.1.2 の後方互換対応と合わせて別途積む。フェーズ0では、GSC・GA4の全体的な共通化、無関係な既存サービスの再設計、画面仕様の変更を行わない。
...
フェーズ1での扱いは次の2案があり、**本書では確定しない（D5）**。
...
いずれを選んでも利用者から見える挙動は変わらない（現状が到達不能であるため）。**D5 の確定前にフェーズ1へ着手しない。**
...
| 画面 | 責務 | MVPでの変更 |
|---|---|---|
| `/analytics` | コンテンツ一覧、カテゴリ/未評価フィルタ | GA4評価状態・点数・パターン・最終評価日時の列と未評価フィルタを追加（フェーズ2） |
| `/analytics/[annotationId]` | 1記事の詳細。フェーズ1で `/gsc-dashboard?annotationId=...` から移設（既存は概要・検索クエリ分析・評価履歴の3タブ） | フェーズ1: 既存3タブを挙動保存で移設（§5.5 / AC-14）。フェーズ2: 「コンテンツ評価」タブを追加。既存3タブの**内容と操作**は変更しない（`GscDashboardClient.tsx` の `TabsList` グリッド列数指定は4タブ化のため変更する） |
| `/gsc-dashboard`（旧URL） | — | フェーズ1で恒久 redirect のみ（§5.5）。ページ実体は削除する |
| `/ga4-dashboard` | サイト全体のGA4集計、ランキング、時系列 | 変更しない（§3.2 対象外） |

記事詳細の情報階層の再設計（統合レイアウト化）は §3.2 のとおり引き続き別チケットで扱う。
...
フェーズ1で移設した記事詳細 `/analytics/[annotationId]` にタブを1つ追加する。既存タブの順序・内容・GSC固有の操作（`OverviewTab` の「最新化」、`SuggestionDataReadiness`、`EvaluationSettings` の評価周期・評価時刻設定と評価開始導線）は変更しない。
...
クライアント文脈 §1.8 に従い、§10.2〜§10.4 をUIたたき台としてフェーズ2のUI実装前に合意する。2026-08-13 に合意したたたき台は新ルートの統合詳細画面を前提としており、2026-08-15 の改訂でルートはたたき台どおり `/analytics/[annotationId]` に戻った。ただしフェーズ1の移設は**挙動保存**であり、たたき台の情報階層（統合レイアウト）は含まない。フェーズ2のUI実装前に、①評価タブUI（§10.2〜§10.4）、②移設直後の画面がたたき台と異なること（既存3タブ＋評価タブ、情報階層は現行のまま。再設計は別チケット）の2点を合意する（D3 / Q-C）。
...
- フェーズ1移設テスト: 既存3タブの挙動保存 E2E／特性テスト（AC-14）、redirect のステータス（308）・遷移先・`annotationId` 以外のクエリが失われないこと（AC-15。`annotationId` 自体の残留有無は期待値に含めず、実測結果を §5.5 に記録する）、`revalidatePath('/analytics/[annotationId]', 'page')` 更新後の再検証動作。**`GlobalToastBridge` のテストは D5 の確定後に定める**（現状の遷移分岐は到達不能であり、(a)(b) いずれを選ぶかで対象が変わる。§5.5「GlobalToastBridge の扱い」）。
...
3. **フェーズ1（ルート移設）を単独でリリースする。** 旧URLからの redirect（AC-15。308 応答の `Location` を実測し、`annotationId` の残留有無を §5.5 へ追記する）、既存3タブの挙動保存（AC-14）、`trial` の遮断（AC-12）を確認する。`trial` の到達遮断は §15.4 の決定に基づく意図的変更のため、リリース時の告知対象とする（R-11。**告知範囲と経過措置の要否は Q-E / Q-F の回答が前提**。告知では旧タブの再読み込みを促す）。
4. §10.2〜§10.4 のUIたたき台を合意する（D3 / Q-C）。合意前にフェーズ2のUI実装へ着手しない。
...
8. 評価サービス・手動API・一覧の評価列と並び替え・記事詳細の「コンテンツ評価」タブをKill Switch無効状態でデプロイする。評価タブとその Server Action / Route Handler で認可が検証されていること（読み取り＝`canAccessGa4` / 書き込み＝`canWriteGa4`。AC-12 / §3.3）を確認する（既存3タブの挙動保存はフェーズ1で確認済み。AC-14）。
9. 許可された運用手順でステージングのDB設定を `enabled=true` に変更し、実データで評価結果とエラー状態を検証する。
10. 一般ユーザーへ段階展開する。

Q-A が「取込追加をMVPに含める」で確定した場合は、手順5（事前確認）の前に「GA4取込へ `engagementRate` / `screenPageViews` を追加する。Compatibility を確認し、`page_views` 列のリモート実在（Q-D）を確認したうえで `ga4_page_metrics_daily` のmigrationを適用し、§4.1.2 の後方互換方針に従って取込を有効化する」を挿入する。
...
**期限について:** 個別の期日はまだ設定されていない。全件の回答期限は「D1（見積合意）の提示時」に確定させる。それまでは無期限に開いた状態であることを明示し、未回答のまま `spec-to-pr` へ進まない（§17）。ただし **Q-D（migration 着手前）と Q-E / Q-F（フェーズ1のリリース前）は、D1 とは別に着手・リリース工程に紐づく期限を持つ**（下表の期限欄を正とする）。
...
| Q-B | `/analytics` の未評価コンテンツは、絞り込み（フィルタ）だけで足りるか。並び替え（評価状態・点数・最終評価日時）も必要か | §1.9.5 は「優先: ①未評価コンテンツの**ソート**」と記録。現状 `AnalyticsTable` に並び替え機構がない | UI 仕様確定（暫定で並び替えを実装対象とした） | クライアント | D3 の UI 合意と同時 | 未回答 |
| Q-C | 2026-08-13 合意のたたき台は新ルートの統合詳細画面であり、2026-08-15 の改訂でルートはたたき台どおり `/analytics/[annotationId]` に戻した。ただしフェーズ1の移設は**挙動保存**（既存3タブそのまま＋フェーズ2で評価タブ追加）であり、たたき台の統合レイアウトは含まない。①この段階的な進め方（移設 → 評価タブ → 情報階層再設計は別チケット）でよいか。②情報階層再設計を別チケットのままとしてよいか | §10.5 参照。旧 Q-C（タブ追加案への再合意可否）は 2026-08-15 の新ルート回帰で前提が変わったため本内容へ差し替えた | **Yes**（UI着手可否） | クライアント | D3 の UI 合意と同時 | 未回答 |
...
| Q-E | 現在 `/gsc-dashboard` に直URLで到達できている `trial` ユーザーが、フェーズ1以降は記事詳細（既存3タブ）へ到達できなくなる。**事前告知のみで実施してよいか。それとも経過措置（一定期間の到達維持など）が必要か** | §3.3「既存挙動の意図的な変更」・§15.4 trial 行・R-11。**これは新規機能の提供範囲ではなく、既存利用者から見える挙動の変更である。**`client-vision-from-lark.md:147` に verbatim で「勝手な判断をされると、前提認識が崩れるのでやめていただけますか？」「やるなら事前に許可をとっていただきたいです。」があり、§15.4 は判断者を「ユーザー決定 / 2026-08-15」と記録するがクライアント（繁田さん）の合意を経たかは本書から判別できない。R-11 の対策は「リリース時に告知」であって事前合意ではない | **Yes**（フェーズ1のリリース可否。移設・redirect・認可の実装自体はブロックしない） | クライアント（繁田さん） | フェーズ1のリリース前 | 未回答 |
| Q-F | 現時点で `trial` ロールのユーザーが `/gsc-dashboard` を実際に利用している事例はあるか | 利用実績の有無で告知範囲と経過措置の要否が変わる（Q-E の判断材料）。**利用実績があった場合、リリース直後に「昨日まで見えていた画面が見えない」問い合わせが発生し、R-11 の想定（旧ブックマーク経由の `/unauthorized`）より影響範囲が広がる** | Q-E の判断材料 | クライアント（繁田さん）＋開発チーム | Q-E と同時 | 未回答 |
...
|---|---|---|---|---|---|---|
| D1 | MVP見積（フェーズ0 30〜45h + フェーズ1 24〜40h + フェーズ2 113〜158h = 167〜243h）を着手前に合意する | §3.4 の各フェーズ内訳。GA4取込拡張12〜20hはQ-A回答待ちのため別枠。**提示時に §15.4 の「未評価ソートの分離先行リリース」判断と、§15.2 の全質問の回答期限を同時に論点化する**。<br>**2026-08-15 改訂で挿入したフェーズ1（24〜40h）は利用者に見える変化を伴わない**（既存3タブの挙動保存移設・redirect・認可の多層化）。フェーズ0（30〜45h。同じく挙動不変）と合わせると **54〜85h（MVP 167〜243h の約1/3）が、クライアント第1優先要求（`client-vision-from-lark.md` §1.9.5「**優先**: ①未評価コンテンツのソート ②評価機能の追加。」）の価値提供より前に置かれる**。この配分そのものを見積提示時に明示して合意する（開発側の都合による前倒し作業であることを隠さない） | 着手判断 | 開発チーム → クライアント | 未設定 | 未合意 |
| D3 | 新ルート `/analytics/[annotationId]` 上の評価タブUI（§10.2〜§10.4）と、移設直後の画面が挙動保存であること（Q-C）を合意する | 2026-08-15 の新ルート回帰で、2026-08-13 合意たたき台の前提（新ルート）と再整合した。残差分は情報階層（統合レイアウトは別チケット） | フェーズ2のUI実装 | 開発チーム → クライアント | 未設定 | 未合意 |
| D5 | `GlobalToastBridge.tsx` の `/gsc-dashboard` 参照（`:20` パス判定・`:25` 遷移先）のフェーズ1での扱いを **(a) 変更せず別チケットへ送る** / **(b) パス判定のみ新ルートへ変え else 側の遷移分岐を削除する** から決める | 当該分岐は現状**到達不能**である（`gsc-dummy-open` 購読0件・`gsc-dummy-update` dispatch 0件・`:28` が payload を同期削除。実測根拠は §5.5「GlobalToastBridge の扱い」）。**2026-08-15 改訂が「挙動保存の唯一の意図的な逸脱」としていた前提は成立しないため撤回した。**利用者から見える挙動はどちらを選んでも変わらない。判断が分かれるのは「到達不能な既存コードをこの移設のスコープで削除してよいか」であり、挙動保存の原則の外側にある | §3.4 フェーズ1完了条件の grep 0件に例外を設けるか否か（(a) なら必要）、§13 フェーズ1移設テストの対象 | 開発チーム | フェーズ1着手前 | 未確定 |
...
|---|---|---|---|
| 繁田さんのシステムプロンプト | 評価観点・出力JSON契約が確定していること | `prompt_templates` に登録され、§6.3.4 の出力契約が確定している | 出力スキーマ・CHECK制約・AC-01 が固定できず、フェーズ2の評価サービスに着手できない（§15.5 R-01） |
...
| D3（UIたたき台の合意。Q-C と同時） | §10.2〜§10.4 の案と挙動保存移設に合意していること | §10.5 の合意ゲートを通過 | フェーズ2のUI実装に着手できない |
...
| Q-E / Q-F（trial の到達遮断の事前合意） | `trial` の記事詳細到達を遮断してよいこと、経過措置の要否が確定していること | §15.2 の Q-E / Q-F が回答済みになる | 事前許可なしの既存挙動変更として `client-vision-from-lark.md:147` の要求に反する恐れがある。**移設・redirect・認可の実装自体はブロックしないが、フェーズ1のリリース判断はブロックする** |
...
| 存在しない annotationId は null detail 描画を踏襲する（§5.5） | (a) `notFound()` を導入し 404 化 / (b) 現行踏襲（null detail で描画継続） | (b) を採用。フェーズ1は挙動保存が原則であり、`notFound()`・`not-found.tsx` はリポジトリに前例0件（2026-08-15 実測）で新規UI導入になる | (a) は挙動保存の範囲を超え、404 画面の設計・文言の新規決定が必要になる | 実在しない ID でも空の詳細画面が表示される（現行同等） | 情報階層再設計チケットで 404 設計を扱うとき | 開発チーム / 2026-08-15 |
| 未評価ソートを分離して先行リリースしない（§3.4） | (a) フェーズ0 → フェーズ2 を一括でリリース / (b) 一覧の並び替え・未評価フィルタだけを先行リリース | クライアント第1優先（§1.9.5「①未評価コンテンツのソート」）は「評価が走っていない記事を見つけて評価する」ための手段である。評価機能が無い状態では全記事が未評価となり、先行リリースしても優先要求を満たさない。並び替えには評価テーブルとの JOIN（§5.1 一覧RPC）が必要で、フェーズ2のDB migrationを分割しても依存は消えない | (b) は DB migration と一覧RPC 再作成を2回に分けることになり、`get_filtered_content_annotations` の DROP/再作成を重複して行う | 第1優先要求の利用者価値がフェーズ2完了まで出ない。**2026-08-15 改訂のフェーズ1挿入（24〜40h）により、この判断の前提だった工数配分が変わった。**利用者に見える変化を伴わない先行作業はフェーズ0（30〜45h）＋フェーズ1（24〜40h）＝**54〜85h**（MVP 167〜243h の約1/3）に増えている。この点は D1 提示時に論点として提示する | クライアントが「評価機能より先に一覧の並び替えだけ欲しい」と明示した場合 | 開発チーム / 2026-08-14（影響欄は 2026-08-15 のフェーズ1挿入を反映して更新） |
...
- フェーズ2の変更対象候補: `src/server/services/`、`src/server/actions/` または Route Handler、`src/types/`、`supabase/migrations/`（評価テーブル・settings・trigger・開始/完了RPC）、`src/components/AnalyticsTable.tsx`（評価状態列・未評価フィルタ・**並び替え機構の新規実装**）、`app/analytics/[annotationId]/`（「コンテンツ評価」タブ追加。`GscDashboardClient.tsx` 相当の `TabsList` グリッド列数と既存タブ文言の修飾。既存3タブの取得処理と表示は変更しない）、評価用 Server Action / Route Handler の入口での `canAccessGa4` 検証（§3.3 / BR-07 / AC-12）、`get_filtered_content_annotations` の再作成（未評価フィルタ・並び替えパラメータ・評価テーブルJOIN・返却フィールド追加）、`.agents/skills/growmate-ui-ux/ui-text.md`（「評価」行の修飾ルール更新と `/gsc-dashboard` 表記の差し替え。§10.3）。
...
- 別チケットへ送るもの: 一覧への戻り先クエリ引き継ぎ（§3.2）、記事詳細の情報階層の再設計（統合レイアウト化）、存在しない annotationId の `notFound()` 導入（§15.4）、レスポンシブ・アクセシビリティの新規要件、定期Cron・非同期ジョブ、GSC `dataState` の明示指定と記録、インポート直後の自動評価、`src/server/lib/gsc-status.ts` への `webmasters.readonly` 欠落判定の追加（§9.1.1 / §15.4）。
...
- 仕様書HTML束: `docs/plans/_html/ga4-content-evaluation-spec/` は **2026-08-15 改訂を反映済み**である（`core.yaml` のサマリー・フェーズ1・308・trial 遮断・167〜243h・新ルート、`diagrams/ui-layers-compare.json` の「2026-08-15 に新ルート移設へ反転」。2026-08-15 実測）。**本文をさらに改訂した場合は、`core.yaml` の `source_label` / 行数参照と `source_refs` のアンカー行番号がずれるため、spec-to-html の手順で貼り直して `npm run spec-html:refresh` で再生成する。**なお `docs/plans/_html/` は `.gitignore:69` で除外されており、リポジトリの成果物には含まれない。
...
- 実装前に本書をレビューし、§15.2 の未確定事項（Q1〜Q4・Q7・Q8・Q-A〜Q-F）と §15.3 の開発側確定事項（D1 見積合意、D3 UIたたき台の合意（Q-C と同時）、D4 期間上限・ページング方式、**D5 `GlobalToastBridge` の扱い**）を確定したうえで、テーブル項目・API契約・画面文言を固定する。**フェーズ1の着手には D5、リリースには Q-E / Q-F が必要である（§15.3 依存関係表）。****未回答のまま `spec-to-pr` を実行しない。****Q-A が未回答の間は追加指標の取込とAC-11を実装対象にしない。**

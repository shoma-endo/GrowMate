...
- 文章化プロンプト（評価エンジン仕様 §07 のシステム／ユーザープロンプトを1本に統合した本文）をDBのプロンプトテンプレートから読み込み、`user`ロール1メッセージでLLMへ渡す処理。
...
| プロンプト管理 | `PromptService` + `prompt_templates`。変数展開は `PromptService.replaceVariables`（`promptService.ts:522-535`。`{{key}}` 形式） | 文章化用テンプレート `ga4_content_evaluation` を1本追加（評価エンジン仕様 §07 のシステム／ユーザープロンプトを統合した本文）。変数は §6.3.3 の表 |
...
- 欠損値とデータ期間を明示する。
- **算出済みの判定結果のみ**をプロンプトへ変数展開して渡す（§6.3.3）。指示部と記事データ部は1本のテンプレート内で節を分けて記述する。
...
- LLM へのロール: `user` の1メッセージ。既存の `gsc_insight_*`（`gscSuggestionService.ts:267`）と同じ1テンプレート＝1メッセージ構成に揃える。
...
- 内容: **確定済み（2026-08-17 受領）**。`docs/context/ga4-evaluation-engine-spec-20260817.md` §07 の「システムプロンプト」と「ユーザープロンプト（変数埋め込み）」を、この順で連結した1本の本文として登録する（各本文は verbatim。分割していた2テンプレートは 2026-08-19 に統合。§18）。役割は**文章化のみ**（読者前提: GA4 用語禁止・人数と時間で語る／書き方: 断定・次の一手は1つ・全体200字以内／点数帯と診断コードの意味／JSON のみ出力）。点数・診断の判定ルールはプロンプトに持たせない（コード側が正本。BR-09）。
- 登録経路: **既存の `/admin/prompts` 画面から admin が手動登録する**（migration の seed にはしない。プロンプト本文はクライアント著作物でありコードと別ライフサイクルで改版されるため）。登録先は `/admin/prompts` の「GA4コンテンツ評価」タブ（`ga4_` プレフィックスで分類。2026-08-19 追加。§10.1）。登録タイミングは §14 手順（Kill Switch 有効化の前）。テンプレート未登録時の評価実行は、スコア算出は行い**文章化のみ失敗**として扱う（§6.5 部分成功）。
...
| 5 | 文章化プロンプト | `prompt_templates.content`（§6.3.1） | テンプレート有効時 | アクセストークン・Service Role キーを注入しない |
...
記事一覧（/analytics, get_filtered_content_annotations）
  ...
  -> [LLM] 文章化プロンプト（指示部 + 算出済み変数を展開した記事データ部。§6.3.3）を1メッセージで送信し、5フィールドJSONを検証（Zod）
...
Feature: GA4コンテンツ評価
  ...
  Scenario: 確定した判定を文章化する
    ...
    And 文章化用プロンプトが有効である
    When 文章化が実行される
    Then LLMへ文章化プロンプトと §6.3.3 の算出済み変数のみが渡される
...
9. **確定した文章化プロンプト（評価エンジン仕様 §07 のシステム／ユーザープロンプトを連結した1本。§6.3.4 の5フィールド契約と一致）を `/admin/prompts` の「GA4コンテンツ評価」タブから `ga4_content_evaluation` として登録する**（§6.3.1）。
...
1. ~~システムプロンプトの最終版・出力 JSON~~ → **確定**（評価エンジン仕様 §07 verbatim。出力は5フィールド。§6.3.1 / §6.3.4）。登録は1テンプレートに統合（2026-08-19。§18）。
...
| 2026-08-19 | **文章化プロンプトを2テンプレートから1テンプレートへ統合（開発側決定）。** 受領仕様 §07 はシステム／ユーザーの2本立てだが、(1) 既存の `gsc_insight_*` は1テンプレート＝1メッセージ（`gscSuggestionService.ts:267`）で構成が不揃いだったこと、(2) 評価履歴のプロンプト追跡列が単数形で `prompt_template_id` / `prompt_version` / `prompt_content_sha256` がシステム側しか記録せず、**ユーザー側テンプレートの改版が追跡できない穴**があったこと、から `ga4_content_evaluation` 1本へ統合し `user` ロール1メッセージで送る。本文は §07 の両プロンプトをこの順で連結（各 verbatim）。migration `20260819000000` で旧2件（本文未登録のもののみ）を削除。§6.3 / §6.3.1 / §8 / §10.1 / §14 手順9 を同期 | 追跡の穴を解消。プロンプト登録は1件のみでよい |

...
- ~~**点数閾値による一覧化・フィルタUI（「70点以下の一覧化」）。** Q2 回答（2026-08-16）「なくていい」により実装しない（§6.4）。~~ → **D11（2026-08-24）で反転。**「改善が必要な記事を見つけたい」という依頼が再度届き、フェーズ2で実装済みの散布図（§10.6）はこの目的を満たさない（記事単位の絞り込みではなく分布の俯瞰であり、目的が異なる）ため、一覧のフィルタとして実装する（§10.2 / §6.4）。
- LLMによる記事本文の自動編集・公開。
...
点数の算出根拠・上限下限・データ不足時の扱いは §6.2 でコード側に確定した（旧「プロンプトで確定する」は 2026-08-17 失効）。クライアント文脈 §1.9.2 の「70点以下の一覧化」は ~~Q2 回答（2026-08-16「なくていい」）により実装しない~~ **D11（2026-08-24）で反転し実装する。**

- **点数閾値フィルタ（D11）**: 一覧（`/analytics`）に「コンテンツ力スコア60点未満」の絞り込みチェックボックスを追加する。閾値は §6.2.2 の点数帯定義（0-19 深刻／20-39 要改善／40-59 改善の余地あり／**60-79 合格ライン**／80-100 良好）の「合格ライン」の下限をそのまま流用し、新規の任意閾値を作らない。UI・RPCとも既存の未評価フィルタ（`p_has_unstarted_ga4_evaluation`）と同型（§10.2）。順位・散布図（下記）は分布の俯瞰を担い、本フィルタは個別記事の絞り込みを担う。両者は目的が異なるため、散布図の追加は本フィルタの代替にならない。
...
- **コンテンツ力スコア60点未満をフィルタできるようにする（D11。2026-08-24 追加）。**「改善が必要な記事を見つけたい」という依頼に対する回答。閾値は §6.2.2 の点数帯「合格ライン（60-79）」の下限を流用し、新規の任意閾値は作らない。RPCに `p_ga4_content_score_below` を追加し、未評価フィルタと同じチェックボックス形式で `CategoryFilter.tsx` に配置する。未評価（`unassessed` / `R_LOWDATA`）は対象外とする（コンテンツ力スコアを持たないため）。
...
| Q2 | 「70点以下の一覧化」は Must の UI フィルタか、任意か | ~~**「なくていい」。点数閾値のフィルタ・一覧化 UI は実装しない**~~ → **反転（2026-08-24、ユーザー依頼。D11）**。「改善が必要な記事を見つけたい」目的の再依頼を受け、コンテンツ力スコア60点未満（§6.2.2「合格ライン」下限を流用）のフィルタを一覧に追加する | 2026-08-16 →<br>2026-08-24 反転 | §3.2 / §6.4 / §10.2 / §15.3 D11 |

...
- フェーズ3（2026-08-19 追加）: **コンテンツ評価サイクル設定**（記事単位。基準日・サイクル日数・実行時刻）、**定期評価バッチ**（毎時 Cron。次回評価予定日時に達した記事のみ）、**評価完了の Resend メール通知**（結果と記事詳細URLを本文に含める）。**評価設定は既存の「検索順位評価サイクル設定」（GSC。`EvaluationSettings.tsx`）と同型に揃え、意図した差分は通知先（GSC＝アプリ内 / コンテンツ評価＝メール）だけとする（§6.6）。**記事詳細の「概要」タブで横並びに配置する（§10.8）。
...
- 記事詳細の「概要」タブで「コンテンツ評価サイクル設定」を登録・更新でき、次回評価予定日時が表示される（AC-19）。
...
| 未評価フィルタ UI | `AnalyticsTable.tsx` / `CategoryFilter.tsx` | ~~既存 GSC 未評価フィルタ（`p_has_unstarted_gsc_evaluation`）と同パターンで GA4 版を追加~~ **2026-08-26 廃止（サイクル統合）: 追加したGA4版（`p_has_unstarted_ga4_evaluation`）を撤去し、既存フィルタを「評価未開始」へ改称して1つに統合した。RPC の引数は `default false` のまま残し、アプリ側から渡さない**（本番適用済み関数の再定義を避けるため。migration は追加しない） |
...
- ~~`unassessed` / GA4未評価を**フィルタ**できるようにする（GSC未評価フィルタ `p_has_unstarted_gsc_evaluation` と同型）。~~<br>**2026-08-26 廃止（サイクル統合。ユーザー判断）。**サイクルを登録した時点でGSC検索順位評価とGA4コンテンツ評価が同時に始まるため、**系統別の「未開始」という状態が存在しなくなった**。実装されていたGA4版（`p_has_unstarted_ga4_evaluation`）は `ga4_content_evaluations` の行の有無＝「結果があるか」を見ており「開始したか」ではなかったため、サイクル登録済みでも本評価が走る2サイクル目まで true のまま残り、その間ユーザーに取れるアクションが無かった。<br>**統合後の仕様: 一覧の状態フィルタは「評価未開始」1つ**（条件は `gsc_article_evaluations` に行が無い＝サイクル未登録。RPC 引数は既存の `p_has_unstarted_gsc_evaluation` をそのまま使う）。`p_has_unstarted_ga4_evaluation` は `default false` のまま SQL に残し、アプリ側から渡さない（本番適用済み関数の再定義を避けるため。**migration は追加しない**）。旧 deep link `?ga4_evaluation=not_started` は `AnalyticsTable.tsx` の `pushFilterQuery` が毎回削除するため効かない。
...
> | 箇所 | 旧 | 新 |
> |---|---|---|
> | カード見出し | 検索順位評価サイクル設定 | **検索順位・コンテンツ評価サイクル設定** |
> | カード説明文 | N日ごとに検索順位の変動を自動的に追跡・評価します | **N日ごとに検索順位の変動とコンテンツの状態を自動で評価します** |
> | 開始ボタン | 検索順位評価を開始 | **評価を開始** |
> | ダイアログ見出し（新規） | 検索順位評価サイクルの開始 | **検索順位・コンテンツ評価サイクルの開始** |
> | ダイアログ見出し（更新） | 検索順位評価基準日の変更 | **評価基準日の変更** |
> | 入力ラベル | 検索順位評価基準日 | **評価基準日** |
> | 状態カード | 現在の検索順位評価基準日 | **現在の評価基準日** |
> | 未設定時 | まだ検索順位評価サイクルが設定されていません。「検索順位評価を開始」ボタンから設定してください。 | **まだ評価サイクルが設定されていません。「評価を開始」ボタンから設定してください。** |
> | 保存後トースト | 検索順位評価基準日を更新しました／検索順位評価を開始しました | **評価基準日を更新しました**／**検索順位・コンテンツ評価を開始しました** |
>
...
**2026-08-26 訂正（2段階で反転した。この宣言はもう成立しない）。**
...
**本スコープには次の3件が未解決のまま残っている。**いずれも仕様書の修正では解消せず、クライアントまたは実測でしか閉じられない。
...
**この3件が閉じるまで、本スコープの仕様レビューは完了扱いにしない。**

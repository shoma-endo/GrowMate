...
| 最終更新 | 2026-08-19 |
| フェーズ3（2026-08-19 追加） | **コンテンツ評価サイクル設定＋定期評価バッチ＋メール通知**を追加する（§3.4 / §6.6 / §7.7 / §8.3 / §9.5 / §10.8 / §10.9）。**評価設定は既存の「検索順位評価サイクル設定」（GSC）と同型に揃え、意図した差分は通知先だけとする**（GSC＝アプリ内 / コンテンツ評価＝Resend メール。§6.6）。**本フェーズは 2026-08-16 のクライアント合意2件（Q4＝メール配信 Non-goal、定期Cron の対象外）を反転させる**ため、§15.1 Q4 行・§3.2 の該当 Non-goal を更新し、反転の共有と見積合意を **D6** として §15.3 に起票した。**D6 は 2026-08-19 に共有済み（ユーザー報告）。Q-J / Q-K も同日確定。設計上の着手ゲートは解消済みだが、`spec-to-pr` の実行は保留**（同日ユーザー決定: **フェーズ2の手動評価を先に本番出荷する**。§14 手順11 までフェーズ3を回さない。§17）。audit 第7回（🔴2 / 🟡18 / 🟢4）は反映済み（§19）。spec-review の再実行はユーザー判断で不要。開発側判断 D8（手動評価とサイクルの関係）は未決だがリリースはブロックしない |
| 作成者 | GrowMate 開発チーム |
| 対象リリース | 未定（D1 合意後に確定） |
| 関連 Issue / PR | 未起票 |
...
| BR-13 | 通知メールの本文にも §10.7 の用語言い換えを適用し、GA4 用語を出さない。欠損値を 0 として書かない（BR-02 と同じ） | なし | §9.5 / §10.9 | AC-21 / AC-17／§13 UI用語検査 |
...
| メール通知 | `emailService.sendGa4ContentEvaluation` 追加、`sanitizeEmailHtml` の共通化（現在 2ファイルに重複。§17）、本文組み立て（用語言い換え適用・欠損の非0表示・記事詳細URL）、冪等な送信記録（BR-12）、失敗時の `console.error` とログ記録（§9.5 / §10.9） | 8〜12h |
...
| 3 | 計測結果（セッション数・エンゲージのあったセッション数とエンゲージメント率・平均エンゲージメント時間・読み切りに必要な時間・読了率・最後までスクロールした人数と率） | スコア算出エンジンの中間値（§6.2.1） | 常時 | 生 API レスポンス全文は注入しない |
...
生の数値（平均エンゲージメント時間、エンゲージメント率、セッション）は **GA4 の画面と一字一句一致させる。名前も GA4 のまま出す**（§10.7。2026-08-22 方針転換。利用者は必ず一度は突き合わせるため、値だけ合っていて名前が違うと突合できない。突合先の画面は §4.1.1 の実測結果で確定し §15.2 で確認する）。
...
| 種別 | ツール内の表記 | 根拠 |
|---|---|---|
| GA4 `sessions` / `activeUsers` | セッション / ユーザー | GA4 の生指標 |
| GA4 `engagementRate` | エンゲージメント率 | GA4 の生指標 |
| GA4 エンゲージのあったセッション | エンゲージのあったセッション | GA4 の生指標 |
...
| GSC / GA4 `impressions` | 表示回数 | Search Console・GA4 とも日本語 UI は「表示回数」 |
| 直帰率 | 表示しない | 評価の入力に使っておらず、対応する数字が GrowMate 側に無い（表記ではなく表示可否の判断） |
| **GrowMate が算出**（読了率・読了スコア・読み始めスコア・コンテンツ力スコア・資産価値スコア・実効スコア） | 現行のまま据え置き | GA4 に対応する数字が存在しない。読了率は「平均エンゲージメント時間 ÷ 期待読了時間」で GrowMate 固有 |
| 検索順位 | 検索順位（据え置き） | GSC 正式名は「掲載順位」だが、`検索順位評価サイクル` `検索順位スナップショット` など既存機能の名称として定着済み。GA4 由来でもないため今回の対象外 |

適用範囲は**ツール内の UI 全体**とし、`ui-text.md` の用語辞書を正本とする（§17）。`scripts/check-ui-text.sh` は同辞書からルールを生成するため、辞書を直せば機械チェックも追従する。**フェーズ3の通知メール本文も適用対象とする**（BR-13。画面外だがユーザーが読むテキストである）。

**失効した記述**: 「GA4 用語の残存をコード検索で 0 件にする」検査は、GA4 用語こそ正となったため意味を失った。代わりに `ui-text.md` の辞書が「言い換え語（訪問した人・読み始め率・実際に読まれた時間・入口になった記事・検索結果に出た回数）を書かない」方向で検査する。§3.4 の「UI用語言い換え」+4〜6h は、置換の向きが変わっただけで工数は同等。
...
- GSC 側の表示文言に GA4 用語が残っている場合の置換は、**フェーズ2 の「UI用語言い換え（UI 全体適用）」のスコープ**であり（§10.7 / AC-17）、フェーズ3では扱わない。
...
- **用語言い換えを適用する**（BR-13）。「セッション」「エンゲージメント率」等の GA4 用語を本文に出さない（§10.7）。
- **欠損を 0 と書かない**（BR-02）。サイト内順位や完読率が取れていない場合は、その行ごと出さない。「0位」「0%」と書かない。
- **人数を率から換算しない**（§10.3 の3と同じ禁則）。人数ファネルはメールには載せない（実測の有無で出し分けが要り、文面が複雑になるため。詳細はアプリで見る）。
...
| 可用性・信頼性 | 外部 API または LLM の一時障害で、既存の正常結果が失われない。評価機能の停止が既存の一覧・記事詳細（`/analytics/[annotationId]` の既存3タブ）・既存の取込経路（GSC は `gsc-evaluate` Cron、GA4 は `/api/ga4/sync` のユーザー起動）へ波及しない | AC-04、AC-06、§13 サービステスト | 確定。SLA 目標値は設定しない（フェーズ2までは手動実行のみで可用性が業務停止に直結しない）。**フェーズ3の毎時バッチは、1回の失敗を次の毎時実行で回復する設計とする**（中断・失敗した記事は `last_evaluated_on` を進めない、または次サイクルで再評価される。§8.3 / §6.6.4）ため、単発の失敗に対する SLA も設定しない。取込経路の実態は §8.2 |
...
```gherkin
  ...
  Scenario: メール本文に GA4 用語を出さない
    Then 本文に「セッション」「エンゲージメント率」「平均エンゲージメント時間」「ランディングページ」等の GA4 用語が含まれない
...
- UI用語検査: ツール内 UI 全体（新設・変更ファイルおよび既存画面）の表示文言に GA4 用語が残存しないこと（AC-17。§10.7 の対応表をコード検索で機械確認。原文正本の原則で既存画面も対象）。**フェーズ3では通知メールの本文組み立て関数も検査対象に含める**（BR-13 / AC-21）。
...
- クールダウンの分岐テスト（フェーズ3。BR-11）: §6.6.4 の表の全行（結末10値 ＋ 取込失敗 ＋ ロール不許可・時間予算切れ・件数上限切れ）で `last_evaluated_on` が進む／進まないことを固定する。とくに `evaluated` / `narrative_failed` / `insufficient_data` / `low_data` / `import_failed` / `evaluation_failed` / `unknown_error` / `needs_reauth` は進み、`evaluating` / `already_running` / `evaluation_disabled` / `unassessed` / `eligible` と中断事由では進まないこと。**取込失敗の分岐**（当日は進めない／`next_evaluation_date` が当日より前なら進める）も含める。**この分岐を落とすと失敗記事が毎時 LLM を呼び続けるため、回帰テストとして必ず置く。**
...
6. フェーズ2のDB マイグレーションを適用する。（Kill Switch 設定行の作成は §8.2 で廃止。既に作成済みの環境は `20260819000200_drop_ga4_content_evaluation_settings.sql` で削除される）
...
| **D9** | **UI 用語を GA4 の名前へ戻した件をクライアントへ差し戻し、合意を得る** | 2026-08-17 受領の評価エンジン仕様 §08 は「ツール内でGA4用語を全面禁止」「用語言い換え表をUI全体に適用（GA4用語をコード検索して残存ゼロを確認）」と定めており、§10.7 はそれを取り込んで承認まで通していた。**2026-08-22 のユーザー決定でこれを反転した。**判断根拠: 利用者は GA4 を連携しており GA4 の画面をすでに見ているため、ツール内で別名に置き換えると同じ数字が 2 つの名前を持ち、突合できず混乱する。§10.3 が「生の数値は GA4 の画面と一字一句一致させる」と定めていることとも、値だけ一致させて名前を変える旧方針は整合しない。<br>**提示時に明示する条件**: (a) 反転対象は **GA4 から取得した生の指標のみ**（セッション／ユーザー・エンゲージメント率・エンゲージのあったセッション・平均エンゲージメント時間・ランディングページ・キーイベント・表示回数）、(b) **GrowMate が算出する派生指標は据え置き**（読了率・読了スコア・読み始めスコア・コンテンツ力スコア・資産価値スコア・実効スコア。GA4 に対応する数字が存在しないため）、(c) 既存機能名の「検索順位」も据え置き（GSC 正式名は掲載順位だが `検索順位評価サイクル` 等として定着済み）、(d) 工数は §3.4 の「UI用語言い換え」+4〜6h の範囲内（置換の向きが変わっただけ） | クライアント合意（実装は先行済み。合意が得られない場合は差し戻す） | 開発チーム → クライアント | 次回定例 | **未共有**。2026-08-22 に実装とドキュメントを先行反映済み（`ui-text.md` 用語辞書の反転、コード 14 ファイル、§10.7 全面書き換え） |
...
| R-13 | scroll 90% イベントが実プロパティに存在せず完読率が取れない | 完読率併用診断（流し読み型等）と人数ファネルの「最後まで」が出せない。設計上は完読率なしで評価が成立する（任意指標） | 中 | **イベント名は自動解決し、取れない場合は NULL（未計測）として完読率併用を行わない**（2026-08-19 実装）。表示は §10.3 の率表現フォールバックで、`data_quality_json` に `scroll_90_event_count` 欠損として記録する | 開発チーム | **対策済み（2026-08-19）**。完読率が取れないこと自体は残るが、誤診断は起きない |
...
- **`spec-to-pr` の実行ゲート（フェーズ3。2026-08-19）**: **全ゲート解消。実行できる。** D6 は共有・合意済み（見積 58〜90h、提示条件(a)–(d)）、Q-J / Q-K は同日のユーザー指示から確定、**D8 は (b) で確定（開発側決定）**、Kill Switch は削除済み（§8.2。旧 D9 は commit `80eb80a2` で完了）。**前提のフェーズ2は本番出荷済み**（§14 手順11 まで完了）。サイクル設定は概要タブ、評価結果・操作は独立タブのまま（§10.1 / §10.8）。<br>**未確認のまま残るもの（フェーズ3の着手ブロッカーではない）**: Q-H（GA4 画面突合先の数値確認）・Q-I（scroll イベント名）。いずれもフェーズ2の実データ検証の項目で、Q-I が取れない場合も完読率なしで評価は成立する（§6.2.1）。**spec-review の再実行は未実施**（第7回の指摘24件は反映済み。続く run は provider 障害で ABORT し、ユーザー判断で保留）。
...
§19 の記録は **計141行**（第1表 26行・第1回 audit 対応表 37行・第2回 audit 対応表 31行・第3回 audit 対応表 7行・第4回 audit 対応表 4行・第5回 audit 対応表 10行・第6回 audit 対応表 2行・**第7回 audit 対応表 24行**）。`finding_id` は行単位で一意に管理し、同一 ID を複数行に併記しない。

残置は次の4件。それ以外はすべて修正、または §15.2 の確認質問・§15.3 の開発側確定事項へ隔離した。**第3回 audit（new 3・persists 1・🟢 3）および第4回 audit（new 2・🟢 2）はいずれも修正済みで、残置に追加はない。第5回 audit（🟡6・🟢4）は 9件を修正し、`ARCH-NEW-spec-L367` のみ下表の残置（部分対応）とした。第6回 audit（🟡1・🟢1）は2件とも修正済みで、残置に追加はない。第7回 audit（🔴2・🟡18・🟢4）は 23件を修正し、`ARCH-NEW-spec-L1169` のみ下表の残置（部分対応。D8 へ隔離）とした。**

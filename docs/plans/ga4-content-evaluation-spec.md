...
|---|---|---|---|---|---|---|
| **一覧の状態チップの文言を「評価未開始」のまま据え置く**（2026-08-26。D12 の付随論点） | (a) `評価未開始`（現行＝develop と同一） / (b) `評価未登録` / (c) `評価未設定` / (d) `評価サイクル未設定` | **(a) を採用。**理由は2つ。**(1) develop の一覧と一字一句同じであること自体が D12 の提示条件 (a)「クライアントが現に見ている画面は変わらない」の根拠になっている。**改名するとこの条件を取り下げることになり、承認を得たい相手に「知らないうちに画面の文言も変わっている」という論点を新たに渡す。**(2) 本スコープが一覧に追加する発見手段は正味ゼロ（§2.3）であり、文言だけを変えると「何かが増えた」という誤った印象を与える。 | **(b)「未登録」の指摘自体は正しい。**チップの条件は実際には「サイクル行が無い」であり、「未開始」は**サイクル登録済みで初回評価がまだ走っていない記事**（D12 が対象外にした集合）にも日本語としては当てはまるため、ラベル単体では境界が読み取れない。ただしこの曖昧さは**改名では解消しない**（(b)(c)(d) のいずれでも、初回計測待ちの記事が絞り込めない事実は変わらない）。解消するなら §3.2 が示すとおり**評価状態列での絞り込みを別途設計する**のが筋で、ラベルの言い換えは対症療法にとどまる。<br>**なお改名するなら (b) より (c)**: 同機能の状態表現は記事詳細で既に「まだ評価サイクルが**設定されていません**」（`EvaluationSettings.tsx:491`）であり、`登録` は利用者向け文言に一度も出てこない。`開始` はボタン（「評価サイクルを開始」）の動詞で、状態語ではない。 | 一覧（`/analytics`）のチップと解除ボタンの `title`、`ui-text.md` の辞書。**据え置きのためコード変更なし。** | **D12 がクライアント合意に至らず (d)「評価状態列での絞り込み」を別途設計する場合**、状態列フィルタと並ぶことになるため、そのタイミングで両者の呼び分けを設計しなおす（そのとき (c)「評価未設定」を第一候補とする） | ユーザー提案 → 開発チーム / 2026-08-26 |

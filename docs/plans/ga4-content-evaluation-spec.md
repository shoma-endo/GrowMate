...
- `ga4_content_evaluation_settings`: Kill Switchを管理する単一行設定。`id smallint`（常に1）、`enabled boolean`（デフォルトfalse）、`updated_at`、`updated_by uuid nullable`を持つ。**2026-08-19 に管理画面 `/admin/ga4-evaluation` を追加**し、admin が現在値の確認と切り替えを行えるようにした（当初は「MVPでは設定画面を追加せず運用手順で更新」としていたが、本番DBへ直接SQLを打つ運用を避けるため方針変更）。読み取り・更新の Server Action は両方で `role === 'admin'` を検証し、更新時は `updated_by` に実行者IDを記録する。
...
| `/admin/ga4-evaluation` | Kill Switch（`ga4_content_evaluation_settings.enabled`）の確認と切り替え（admin 専用） | **新規**（2026-08-19）。現在値・最終更新日時・切り替えボタン。停止時に何が止まり何が残るかを画面に明記する。管理者ダッシュボード（`/admin`）に導線カードを追加 |
...
| 2026-08-19 | **Kill Switch の管理画面 `/admin/ga4-evaluation` を追加（開発側決定）。** §7.2 は当初「MVPでは設定画面を追加せず、許可された運用手順または管理者専用経路から更新する」としていたが、`ga4_content_evaluation_settings` に書き込む経路がコード上に一切なく、有効化に本番DBへの直接SQLが必要だったため方針変更。本リポジトリには `middleware.ts` が無く `/admin` 配下の認可はサーバーアクション側だけで担保されるため、読み取り・更新の両方で `role === 'admin'` を検証する。§7.2 / §10.1 を同期 | BR-05 / AC-06 の要件自体は不変。操作経路のみ追加 |

...
| 評価種別由来（設定操作ではない） | 初回計測あり（プレビュー3枠）。サイクル日数を取込期間に使う | **初回計測あり（プレビュー3枠。登録時にベースラインを取得。D10確定 2026-08-24）。評価期間は90日固定で、サイクル日数は取込期間に使わない**（§6.6.2 / §6.6.3） | ~~ダイアログのプレビュー3枠はGSCと揃える（D10）。~~ **ダイアログのプレビュー・状態カードとも常に2枠/2枚へ簡略化（2026-08-25。§10.8参照）。理由はGSCとの差異ではなく、GA4側の3枠目（初回計測日／状態カードの3枚目）がフロント独自計算の不誠実な推測値だったため**。プレビューは`COALESCE(cycle.lastEvaluatedOn, dateStr) + cycleDays`（DBの`next_evaluation_date`生成列と同じ式）で計算し、変更ダイアログで既に評価が回っているサイクルを開いても実態と無関係な日付を出さないようにした。評価対象期間の扱いはGSCと異なる（コンテンツ評価は単発完結のため90日固定のまま）。ベースラインは`ga4_content_evaluation_cycles`にのみ保持し、`ga4_content_evaluation_history`には残さない（§7.7） |
...
- ~~**GSC との違いは「初回計測」の概念を持たないこと。**~~ **D10（2026-08-24）で反転。GSC と同じくベースライン（初回計測）を持つ。**サイクル登録時（`onRegister`）に即座に評価を1回実行してベースラインを取得し、**基準日から1サイクル後が初回評価**となる（GSC は基準日から2サイクル後。§6.6.2の`initialMeasurementDate`/`initialEvaluationDate`計算はGSCと同じ`addDays`連鎖を踏襲するが、起点がサイクル登録時点である点がGSCの「次サイクルを待って計測」と異なる）。<br>**ベースラインの保存先（D10確定。2026-08-24 実装時に訂正）**: ベースラインは**通常の評価実行（`ga4ContentEvaluationService.run()`）をそのまま呼ぶ**。`ga4_content_evaluation_history` にも通常どおり履歴行が作成され、記事詳細「コンテンツ評価」タブの評価状態（`ga4_content_evaluations`）も通常どおり更新される。ベースライン評価は「ユーザーが登録直後に手動で評価を実行した場合」と完全に同一の正当な評価であり、特別扱いしない（前回差分・メディア全体スコアも通常どおり機能する）。<br>**あわせて `ga4_content_evaluation_cycles.last_seen_content_score`（§7.7）に結果をキャッシュする。**理由: GSCの`last_seen_position`と同じく、サイクル設定画面（§10.8）が「ベースライン計測済みか」を判定する際に、履歴テーブルへの追加クエリなしで安価に判定するため（`EvaluationSettings.tsx`の`hasCompletedInitialMeasurement`判定と同型）。<br>**訂正の経緯**: 当初「履歴には残さない」としていたが、`run()`は`startRun`→スコア計算→`finishRun`（履歴行の作成・現在状態の更新を含むRPC）が一体不可分であり、履歴を作らない専用の計測経路を新設すると評価コンテキスト構築・スコアリング・LLM文章化の全パイプラインを複製することになり、二重実装のドリフトリスクを負う。実装着手時にこの構造を確認し、MVP最優先の判断軸（`CLAUDE.md` Core Rules）から「複製しない・通常経路を再利用する」方針へ訂正した。~~画面のスケジュールプレビューはGSCと同じ3枠「基準日／初回計測日／初回評価日」とする（§10.8）。~~ **プレビューは2枠へ簡略化済み（2026-08-25。§10.8・§18参照）。**
...
| `last_seen_content_score` | integer | NULL 可, CHECK 0〜100 | **ベースライン（初回計測）のコンテンツ力スコアのキャッシュ。GSCの`last_seen_position`と同じ役割・同じ用途（D10確定。2026-08-24 実装時に訂正）。「登録済み・未計測」と「計測済み」の判定に使う（`EvaluationSettings.tsx`の`hasCompletedInitialMeasurement`相当）。ベースラインは通常の`run()`実行のため`ga4_content_evaluation_history`にも通常どおり履歴行が残る（§6.6.2）** |
...
| スケジュールプレビュー | ~~GSC と同じ「基準日／初回計測日／初回評価日」の3枠（D10確定。2026-08-24）~~ **2枠「基準日／初回評価予定・次回評価予定」へ簡略化（2026-08-25）**: 新規登録時は「初回計測日」列＝基準日の重複表示だった（D10により登録と同時に測るため）ため削除し、変更ダイアログでは`dateStr`のみの計算が`cycle.lastEvaluatedOn`を無視して実態とズレた日付を出す不具合があったため`COALESCE(cycle.lastEvaluatedOn, dateStr) + cycleDays`へ修正した | ユーザー指摘（2026-08-25）で発覚。登録時にベースラインを取得すること自体は変更なし（§6.6.2） |
| 状態カード | ~~GSC と同じ分岐（初回計測前は3枚、計測後は2枚。`EvaluationSettings.tsx:406-447` と同型。D10確定）~~ **常に2枚（2026-08-25 簡略化。GSCとは意図的に差分あり）**: 「現在の基準日」＋「初回評価予定／次回評価予定」（`hasBaseline`でラベルのみ切替）。旧3枚目「次回評価予定」はDBの権威ある値ではなくフロント側の独自計算（`initialMeasurementDate + cycleDays`）で、リース衝突など`shouldAdvanceCooldown=false`になる稀なケースで実際の次回日とズレうる不誠実な表示だったため削除した（ユーザー指摘） | 「初回計測」の判定は`last_seen_content_score != null`（GSCの`last_seen_position`相当。§7.7）。2枚目のラベル・日付は`cycle.nextEvaluationDate`（DB生成列）をそのまま表示するのみで、`hasBaseline`真偽に関わらず値の算出方法は同一 |
...
```gherkin
  Scenario: サイクルを新規登録する
    ...
    Then ga4_content_evaluation_cycles に status='active' の行が1件作成される
    And 登録と同時にベースライン評価が1回実行される（通常の評価実行と同一経路。D10確定）
    And ga4_content_evaluation_history に通常どおり履歴行が作成され、last_seen_content_score にも結果がキャッシュされる
    And 状態カードに現在の基準日と初回評価予定（基準日 + サイクル日数、指定時刻、日本時間）が表示される
    And スケジュールプレビューには基準日と初回評価予定の2枠が表示される（2026-08-25簡略化。~~GSC と同じ「基準日／初回計測日／初回評価日」の3枠~~。§18）

...
| 注入 budget 対比 | 全文推定 token vs `DEFAULT_KNOWLEDGE_INJECTION_BUDGET_TOKENS`（15,000）。超過時は生成時 trim される旨を表示 |
| step7 リスク | 有効 Doc 合計が大きい場合「step7 で overflow リスク」バッジ |
| 最終フェッチ日時 / エラー | 既存 + **ハード拒否時「最終成功版を使用中」** |

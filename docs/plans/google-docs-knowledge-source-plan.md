# テキスト共通プロンプト 知識ソース機能 実装可否判断 ＆ 段階的ロードマップ

> **Pro 記憶レイヤーの正本（2026-06）**  
> Pro の「考え方・ノウハウ」は **管理者指定の共通プロンプト（テキスト保存）** のみ（`**paid` / `admin` への L1 注入**。下記 §L1 注入の対象ユーザー）。ユーザー別哲学プロファイルは **採用しない**。本書が唯一の実装計画。
>
> **仕様変更（2026-07）**: 当初は Google Docs API で外部 Doc を取得する計画だったが、他のプロンプト管理と同様に **アプリ内でテキストベース保存** する方針へ変更する。共通プロンプトの考え方（管理者のみが管理し、Pro 生成へ毎回 L1 注入する）は維持し、Google Docs API / サービスアカウント / URL フェッチは MVP から外す。
...
| 何を読むか     | 管理者が保存した共通プロンプトテキスト（カオルさんの考え方・ノウハウ）               |
...
- **カオルさん自身の考え方・ノウハウ** をまとめた共通プロンプトテキスト（複数可）。
...
| レイヤー               | 内容                        | 設定者               | 保存・注入                                                    |
| ------------------ | ------------------------- | ----------------- | -------------------------------------------------------- |
| **L1 共通プロンプト** | 考え方・ノウハウ（本機能）             | **管理者**           | `prompt_templates.content`（専用行） → `**paid`/`admin` の chat 系生成**に自動融合 |
...
```
system prompt =
  [L1 共通プロンプトホット層（budget 内）]
  + [L2 選択中テンプレの content（{{company}} {{persona}} 等の事業者変数置換後）]
```

※ L1 の全文は DB（**M② 外部メモリ**）に保持。LLM には **M① コンテキスト内**（budget 内断片）のみ注入。詳細は **共通プロンプトメモリ層** を参照。  
...
**MVP 正体**: `/admin/prompts` に共通プロンプトテキスト管理を統合 → 管理者が本文を保存 → DB 保存 → `**paid`/`admin` の** allowlist 合格 chat 系生成時に L1 Doc + L2 テンプレを自動融合。


| 項目                  | 方針                                                                                             |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| 保存                  | 管理者が `/admin/prompts` で共通プロンプト本文を直接入力・保存                                                       |
...
| 共通プロンプトテキストは **1 専用テンプレート行**               | ✅ 複数の論理 Doc は本文内の見出し・区切りで管理                           |
| URL 登録 + 自動取得                           | ❌ **仕様変更で廃止**                                         |
| UI = 既存プロンプト管理 + 共通プロンプトテキスト保存          | ✅                                                      |
...
運用推奨: 共通プロンプトは **テキスト中心** で記述する。表・画像依存の重要情報は Markdown 化して保存し、長文化が常態化した場合のみ Doc 分割または将来 RAG を検討する。
...
| 残す | chat / canvas / `modelHandlers` / `llmService` の L1 注入配線 | テキスト保存後も生成へ共通プロンプトを注入する本体 |
| 見直す | `getGlobalKnowledgeContent()` 相当の取得処理 | `knowledge_sources` ではなく `prompt_templates.name='global_knowledge_source'` から読む |
| 見直す | `/admin/prompts` の共通プロンプト UI | localStorage ではなく既存プロンプト保存と同様に Server Action 経由で `prompt_templates.content` に保存 |
| 削除 | `knowledge_sources` migration / 型 / repository / mapper / service | 既存 `prompt_templates` を保存先にするため不要 |
| 削除 | `knowledgeSourceOverride.ts` と localStorage 検証 UI / request override 配線 | 共通プロンプトは DB 保存のみ。ブラウザ保存は採用しない |
...
> **混同注意**: 本節の **Memory Taxonomy** は GrowMate 全体の記憶の種類。**M①〜M④**（後述 §共通プロンプトメモリ層）は **L1 Doc レイヤー内** のホット/コールド/cache 分類で、別体系。
...
| **Context Poisoning**（汚染された記憶が正本化）              | 下記 **Memory Write Policy**。**user 入力・LLM 出力を共通プロンプト用 `prompt_templates` 行に書かない**                                  |
...
| ------------------------------- | -------------------------------- | --------- | ---------------------- |
| **Semantic Memory**（意味・ナレッジ）    | `prompt_templates` の共通プロンプト専用行 | **管理者のみ** | L1 hot layer（M①）       |
...
| ルール          | 内容                                                                   |
| ------------ | -------------------------------------------------------------------- |
| 自動書込禁止       | 共通プロンプト用 `prompt_templates` 行へ user 入力・LLM 応答・Web 検索結果を自動保存しない |
| 更新経路         | `prompt_templates.content` の共通プロンプト専用行更新は **admin 操作によるテキスト保存のみ** |
...
1. `/admin/prompts` に **「共通プロンプト」** セクションを追加（`**/admin/prompts` 統合は維持。UI は常時フル展開にしない** — 下記 §管理 UI 実装方針）。
2. 管理者が入力した本文を `prompt_templates.content` の共通プロンプト専用行に保存する。
...
| 層        | 内容                                                   |
| -------- | ---------------------------------------------------- |
| クライアント要件 | 管理者が共通プロンプトを設定できること                                  |
| 実装方針     | `/admin/prompts` に統合（別画面 `/admin/knowledge` は MVP 外） |
| UI 表現    | Doc 管理 UI を **常時フル展開しない**（折りたたみ + コンパクト一覧/編集）        |


> `/admin/prompts` 上部に「共通プロンプト」セクションを追加する。ただし、日常作業であるプロンプト編集を妨げないよう、**初期表示は折りたたみまたはコンパクトサマリー**とする。MVP は専用テンプレート 1 行の本文編集に絞り、最終保存日時・エラー有無・token 詳細を表示する。
...
**MVP 外（将来）**: ページ内タブ（「プロンプト」｜「共通プロンプト」）— Doc 管理頻度が高くなったら検討。更新頻度が低い前提では折りたたみで十分。
...
┌─ ▶ 共通プロンプト（折りたたみ・初期）───────────────┐  ← 日常はここだけ
│   保存済み · 最終保存 2026-06-16 · ⚠ エラー 0             │
└──────────────────────────────────────────────────────────┘

┌─ ▼ 共通プロンプト（展開時）──────────────────────┐
│ 表示名              最終保存      状態        [▼]          │
│ 共通プロンプト   06-16 14:30  ✅ 保存済み  [▼]          │
...
| #   | 作業                                         | 備考                                           |
| --- | ------------------------------------------ | -------------------------------------------- |
| 1   | `prompt_templates` に共通プロンプト専用行を用意          | 既存 RLS（管理者のみ）を利用                           |
| 2   | `/admin/prompts` で共通プロンプトセクションを表示          | 別画面 `/admin/knowledge` は作らない                  |
...
| テキスト保存              | 管理者入力 → `prompt_templates.content` の共通プロンプト専用行へ保存                                      | ✅                                                                      |
...
| DB                    | 既存 `prompt_templates` の共通プロンプト専用行                                                       | ✅                                                                      |
...
```
[初回] prompt_templates に共通プロンプト専用行を用意 → /admin/prompts に共通プロンプトセクションを表示
         ↓
...
有料ユーザー（`paid` / `admin`）: 通常どおり生成
         → allowlist 合格時 [L1 共通プロンプト群] + [L2 テンプレ + 事業者変数] が system prompt に載る
...
| 項目                                                                                | 残作業目安          |
| --------------------------------------------------------------------------------- | --------------- |
| `prompt_templates` 共通プロンプト専用行の seed / 取得処理                                  | 0.5日            |
...
## 共通プロンプトメモリ層（コンテキスト管理・必須）

共通プロンプト が長文化すると **コンテキストオーバーは確実**。対策は **共通プロンプトレイヤーだけ** に閉じ込める。テンプレ・事業者情報・チャット履歴のリファクタは本件スコープ外。
...
| 共通プロンプト | NotebookLM ソース並みに **長文化しやすい**（最大リスク）       |
...
| **M②** 外部メモリ     | `prompt_templates.content` の共通プロンプト専用行 — 管理者が保存した共通プロンプトテキストの **全文**（保存時に切り詰めない） | **採用**     |
| **M③** 再学習       | —                                                                                          | **不採用**    |
...
| 層                | 方針                                                                                                                                                                |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **M② `content`** | 管理者が入力した共通プロンプトテキストを **そのまま全文保存**。NotebookLM 相当の長文を想定し、**保存時 trim は禁止**（M② の意味が崩れるため）                                                                    |
...
function trimKnowledgeForGeneration(
  ...
  // 2) 超過 → 共通プロンプト本文を先頭優先で載せ、見出し/区切り境界で切る
...
```
Block A: L1 共通プロンプトホット層  … cache_control: ephemeral（M④）
...
| 表示    | 一覧行（常時）                    | 行展開（詳細）                              |
| ----- | -------------------------- | ------------------------------------ |
| 共通プロンプト | 表示名・最終保存・状態（✅/⚠/❌） | 文字数 / 推定 token vs 20,000字・150,000字警告 |
...
| Context Poisoning         | **Memory Write Policy** — user/LLM 出力の共通プロンプト専用 `prompt_templates` 行への自動書込禁止                                                                                                                                                                         |
...
| **ページ内タブ**（プロンプト｜共通プロンプト）             | Doc 管理頻度が高く折りたたみでは足りないとき                                                 |
...
```
共通プロンプト専用 `prompt_templates` 行の content
        ↓
...
| `prompt_templates` RLS パターン           | 共通プロンプト専用行の管理者保存にそのまま利用                                                                  |
...
| 共通プロンプトの長文化               | **Doc メモリ層**（budget + trim + UI 警告）。テンプレは切らない                             |
...
| Context Poisoning        | **Memory Write Policy** — 共通プロンプト専用 `prompt_templates` 行への自動書込経路を作らない |
...
1. `prompt_templates` 共通プロンプト専用行 + 取得処理
...
- 共通プロンプト専用行の保存内容が L1 として融合されること
...
- **Memory Write Policy**: user 入力 / LLM 出力 / Web 結果が共通プロンプト専用 `prompt_templates` 行に保存されないこと

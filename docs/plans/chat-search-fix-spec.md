...
RLS によるユーザー本人のアクセス制御が正しく機能していることを確認する。**本検証は必須とする。**

| ロール | 検証内容 | 期待結果 |
| ------ | -------- | -------- |
| 有料契約ユーザー | 検索・セッション一覧を実行 | 自身のデータのみ表示。検索・一覧が正常に動作すること |
...
| テストケース | 手順 | 期待結果 |
| ------------ | ---- | -------- |
| 無効なセッション | ログアウトまたはセッション破棄した状態で検索を実行 | 認証エラーが返ること。「チャットセッションの読み込みに失敗しました。」または認証系のエラーメッセージが表示されること |
| 期限切れセッション | セッション期限切れ後に検索を実行 | 認証エラーまたは再ログインを促すメッセージが返ること |

### 7.6 セキュリティ検証

認証チェックの変更により、**アクセス権のないユーザーが他のユーザーのチャットセッションにアクセスできないこと**を確認する。RPC は認証済みユーザーに紐づくデータのみを返す設計であることを検証する。

| 検証項目 | 手順 | 期待結果 |
| -------- | ---- | -------- |
| 他ユーザーデータの非漏洩 | ユーザー A でログインし、検索・セッション一覧を実行 | ユーザー A のデータのみ表示。ユーザー B のセッションが含まれないこと |
| p_user_id の検証 | RPC は Server Action から `targetUserId`（`auth.userId`）を渡される | `checkAuth` で検証済みのユーザー ID のみが RPC に渡され、本人のデータのみが返ること |

**補足**: RLS ポリシーで `auth.uid()` に紐づく本人データのみを対象にする。`p_user_id` の検証結果と RLS のアクセス境界が一致することを確認すればよい。
...
| ローカル検証 | 1.5h | `db push`、検索・セッション一覧の動作確認、エッジケース、契約状態別確認 |

...
| `serviceName` | Google Ads コンテンツ戦略提案画面で選択中のサービス | 分析対象サービス名 |
| `strength` | Google Ads コンテンツ戦略提案画面で選択中のサービス → `strength` | 選択中サービスの強み |
...
4. **プロンプト変数構築**: `{{persona}}`, `{{serviceName}}`, `{{strength}}`, `{{keywordData}}`, `{{negativeKeywords}}`, `{{dateRange}}`, `{{customerName}}`
...
Google Ads コンテンツ戦略提案画面で選択中のサービス名を渡す。UI では初期表示時に先頭サービスを選択するため通常は未選択にならないが、`serviceId` 未送信・UI 初期化前の送信・選択済みサービス削除後の古い `serviceId` などに備え、サーバー側で `BriefInput.services[]` の先頭サービスに防御的にフォールバックする。
`BriefInput.services[]` が空の場合はエラーを返し、AI分析を実行しない。

#### `{{strength}}` の形式

Google Ads コンテンツ戦略提案画面で選択中のサービスの `strength` フィールドを渡す。選択サービスの強みが未設定の場合は `（事業の強み未設定）` を代入。
`BriefInput.services[]` が空の場合はエラーを返し、AI分析を実行しない。
...
values (
  ...
  '[
    ...
    {"name": "serviceName", "description": "選択中サービス名"},
    {"name": "strength", "description": "選択中サービスの強み"},
...
バックグラウンドで Step 3〜6 を順番に実行・content_annotations に保存
  ※ maxDuration: 300 を設定する長時間 API Route として実装
    ↓

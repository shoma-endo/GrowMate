...
AI 出力には各候補に対し以下を含める: `判定（新規/修正）`、`修正の場合の対象記事（タイトル+URL）`、`判定理由`、`期待される上昇見込み`。
...
MVPメールの各提案ブロックの表示イメージ（既存コンテンツがある場合のみ順位/タイトル/URLを出す。無ければ「既存コンテンツなし＝新規候補」）:
...
> - キーワード↔GSCクエリは完全一致／正規化一致したものだけ順位を出す。一致しないKWは「順位データなし＝新規候補」扱い。
...
3. **プロンプトテンプレート更新（admin/prompts で実施・コード変更なし）**
   ...
   - 分析指示: 「TOP5 各提案について、既存コンテンツとのカニバリ有無と GSC 順位を踏まえ **新規作成 / 既存修正** を判定し、修正の場合は対象記事URLと上昇見込みを明記する」セクションを追加。
...
- 当該KWをカバーする自社記事が**ある**（タイトル/KW/本文で一致、またはGSCでランクイン）→ **既存修正**を推奨し、対象記事のタイトルを判断文に含める。順位が中位帯（概ね4〜30位）なら上昇余地大として優先、上位安定（1〜3位）なら現状維持寄り。
...
- **DEV モード**: `NODE_ENV=development` 分岐（`:110-156`）にサンプル既存コンテンツ＋順位を流し、AI 出力に「新規/修正の判定 + 対象記事 + 理由」が含まれることを確認。

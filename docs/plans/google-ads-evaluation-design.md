...
1. **新規データ取得メソッド（`SupabaseService`）**
   ...
   - `getRankingSnapshotByUserId(userId, limit, dateRangeDays)` — `gsc_query_metrics` を**広告データと同じ分析対象期間（`dateRangeDays`）で集約**して取得する。最新日のみだと GSC のクエリ別データは日次でスパースなため、実際は上位でも順位行が欠落する。
     - **期間集約はDB側RPC `get_gsc_ranking_snapshot` に委譲**（事前の行キャップなし＝窓内全行が対象）。`query_normalized` 単位に **position = インプレッション加重平均**、**impressions/clicks = 合計** で集約し、position 昇順・impressions 降順で上位 N 件（例: 100〜500 件）に制限。窓 = 最新日から `dateRangeDays` 日遡る範囲。
     - **代表ページは `(query, url)` 単位の合計インプレッション最大のページ**を採用する（単一行のインプレッション最大ではない）。
     - **URL/タイトルの突合は代表ページの `content_annotation_id`（FK）経由**で行う（`gsc_query_metrics.content_annotation_id → content_annotations.id`。GSCインポート時に紐付け済み。URL文字列マッチより堅牢）。表示URL = `content_annotations.canonical_url`（あれば）優先、無ければ `normalized_url`。タイトル = `content_annotations.wp_post_title`。`content_annotation_id` が NULL の行（WP未取込ページ）は **URL のみ・タイトル空**でフォールバック。RPC が JOIN まで含めて URL/タイトルを返す。
...
- **修正フローの実体**: MVP は「修正推奨の提示」まで。実体の設計は §17.7 の対象外表のとおり別チケット（判断者: PO）。

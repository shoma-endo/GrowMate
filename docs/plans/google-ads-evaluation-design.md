...
- **メールユーザーの場合**: 以下を表示（管理者・一般ユーザー問わず）
  ...
  - 設定インライン UI（ボタン周辺に配置）
    ...
    - `cron_enabled`: トグル（定期自動送信の有効/無効）
    - 変更時に `updateEvaluationSettings` を呼び出し即時保存
...
| `customerName` | `google_ads_evaluation_settings.customer_name`（null 時は `customer_id` で代替） | Google Ads アカウント名 |
...
分析結果（Markdown）→ HTML 変換 → メール送信
  ↓
設定テーブル UPDATE
  成功時（メール送信まで完了）: last_evaluated_on 更新 + consecutive_error_count リセット
  エラー時（API失敗 / メール送信失敗 いずれも）: last_evaluated_on 更新しない + consecutive_error_count +1
...
- `customer_id` / `customer_name` は `google_ads_credentials` と同期して保持（denormalized）
- **同期タイミングは以下の 2 箇所**:
  1. **OAuth 接続完了時**（初回 row 作成）: `supabaseAdmin.from('google_ads_evaluation_settings').upsert(...)` でデフォルト値（`date_range_days: 30, cron_enabled: false`）で upsert する。再接続時も同じ処理で上書き（`onConflict: 'user_id'`）。
  2. **アカウント選択変更時**（`app/api/google-ads/accounts/select/route.ts`）: `updateGoogleAdsCustomerId` 呼び出し成功後、**別クエリ**で `google_ads_evaluation_settings` の `customer_id` / `customer_name` / `updated_at` を upsert する。`evaluation_settings` の upsert が失敗した場合はエラーをユーザーに返却（HTTP 500）し、アカウント選択をやり直させる（サイレント無視しない）。`customer_name` は選択時に `googleAdsService.getCustomerInfo(customerId, accessToken)` で取得済みの情報から補完する（取得できなかった場合は null）。単一トランザクションは使用しない。
...
| `customer_name` | text | `null` | Google Ads アカウント表示名（OAuth 接続時に取得・保存。null 時はプロンプト変数に `customer_id` を代入） |
...
10. **設定更新**:
    - **成功時**（メール送信まで完了）: `last_evaluated_on` を当日（JST）に更新、`consecutive_error_count` を 0 にリセット
    - **エラー時**（API失敗・メール送信失敗いずれも）: `last_evaluated_on` は**更新しない**、`consecutive_error_count` を +1。エラー詳細はサーバーログに出力。メール失敗時もレポートは保存しないため同日中に手動再実行で再生成可能。
...
// EvaluationSettingsResponse: { dateRangeDays: number; cronEnabled: boolean; lastEvaluatedOn: string | null }
// row が存在しない場合: { dateRangeDays: 30, cronEnabled: false, lastEvaluatedOn: null }
...
export async function GET(request: NextRequest) {
  ...
  // 2. 対象ユーザーを last_evaluated_on ASC NULLS FIRST で最大 BATCH_LIMIT 件取得
  // 3. ユーザーごとに GoogleAdsAiAnalysisService.analyzeAndSend() を実行
  //    経過時間が TIMEOUT_GUARD_MS を超えたらループ打ち切り（残りは次回 cron へ）
  // 4. 結果サマリーを返す
}
...
const BATCH_LIMIT = 10;          // 1 run あたりの最大処理ユーザー数
const TIMEOUT_GUARD_MS = 250_000; // 250秒でループ打ち切り（maxDuration に余裕を持つ）
```

**バッチ処理の仕様**:
- 対象: `google_ads_evaluation_settings` の `cron_enabled = true AND consecutive_error_count < 3` かつ未処理（`last_evaluated_on` が今日未満 or NULL）
- **取得順**: `last_evaluated_on ASC NULLS FIRST`（最も古い/未処理のユーザーを優先）
- **処理上限**: 1 run あたり最大 `BATCH_LIMIT`（10件）。超えた分は次回 cron で自然に拾われる（`last_evaluated_on` が古いままなので再クエリで先頭に来る）
- **時間ガード**: 各ユーザー処理前に経過時間を確認し `TIMEOUT_GUARD_MS` 超過でループ打ち切り
- **カーソル不要**: 未処理ユーザーは `last_evaluated_on` が更新されないため次回 cron で自動的に優先される
- エラー分離: 1ユーザーの失敗が他ユーザーに影響しない
- **スケール上限の目安**: 1ユーザーあたり約 30 秒（Google Ads API × 2 + LLM + メール）× 10件 = 300秒。件数増加時は cron 頻度を上げるか将来的にジョブ分割を検討（Section 16 参照）
...
create table if not exists public.google_ads_evaluation_settings (
  ...
  customer_id text not null,          -- google_ads_credentials から denormalize
  customer_name text,                 -- OAuth 接続時に取得・保存。null 時はプロンプトに customer_id を代入
...
-- Cron 対象ユーザーの取得（古い順・未処理優先、上限 10 件）
...
ORDER BY s.last_evaluated_on ASC NULLS FIRST
LIMIT 10;
...
| `app/google-ads-dashboard/_components/dashboard-content.tsx` | AI分析実行ボタン + 設定インライン UI（`date_range_days` 数値入力・`cron_enabled` トグル）追加 |
...
| `app/api/google-ads/accounts/select/route.ts` | `updateGoogleAdsCustomerId` 呼び出し後、`google_ads_evaluation_settings` の `customer_id` / `customer_name` / `updated_at` を upsert して同期する |
...
`google_ads_evaluation_settings.customer_name` をそのまま渡す。`null` の場合は `customer_id` を代入する。

> **`customer_name` の取得タイミング**: Google Ads OAuth 接続時（既存の接続フロー内）に `CustomerService.getCustomer()` または相当の API コールでアカウント表示名を取得し、`google_ads_evaluation_settings.customer_name` に保存する。既存レコードで `null` のものは次回接続更新時に補完される。
...
|------|---------------------|--------------------------|-----------|
| **成功**（メール送信まで完了） | **当日（JST）に更新** | **0にリセット** | 翌日 |
| **API エラー**（Google Ads / LLM） | **更新しない** | **+1** | 次回実行で自動リトライ |
| **メール送信失敗** | **更新しない** | **+1** | 次回実行で自動リトライ（LLM再生成） |
...
AI分析メールで届いた改善提案を元に、ユーザーがアプリ内でブログ記事作成を開始できる機能。
`content_annotations` テーブルに Step 1〜2 相当のデータを事前保存することで、チャット画面を **Step 3（ユーザーのゴール）からスタート** できる。
...
Claude で分析
    ↓
出力をパース → TOP5レコードを構造化データとして抽出
    ↓
DB（google_ads_blog_suggestions テーブル）に保存
    ↓
メール送信（分析結果のテキストのみ）
...
ユーザーがアプリのダッシュボード or 専用画面で「最新の分析結果」を確認
    ↓
TOP5レコードがカード形式で表示される
    ↓
ユーザーが任意のレコードの [📝 記事作成へ] ボタンをクリック
    ↓
新規チャットセッション作成
    ↓
【自動ステップ実行（Step 1〜6 一気通関）】
  Step 1〜2 データを保存（Google Ads分析結果から）:
    - main_kw / kw / impressions / needs / persona
    ↓
  Step 3 自動実行: persona を入力 → llmChat(blog_creation_step3) → goal 保存
    ↓
  Step 4 自動実行: goal を入力 → llmChat(blog_creation_step4) → prep 保存
    ↓
  Step 5 自動実行: prep を入力 → llmChat(blog_creation_step5) → basic_structure 保存
    ↓
  Step 6 自動実行: basic_structure を入力 → llmChat(blog_creation_step6) → opening_proposal 保存
    ↓
チャット画面（/chat）を Step 7（本文作成）で表示
```

### 16.3 content_annotations へのマッピング

自動実行により Step 1〜6 の全フィールドを保存する。ユーザーのスタート地点は **Step 7（本文作成）**。

| フェーズ | content_annotations カラム | データソース |
|---|---|---|
| Google Ads分析結果から | `main_kw` | CampaignH1用キーワード |
| Google Ads分析結果から | `kw` | 参照KW + サブKW + ピラーKW |
| Google Ads分析結果から | `impressions` | 表示回数推計 |
| Google Ads分析結果から | `needs` | 検索意図詳細 + ファネル分類 |
| Google Ads分析結果から | `persona` | ペルソナ表（Markdown テーブル形式） |
| Step 3 自動実行（AI生成） | `goal` | persona → llmChat(blog_creation_step3) |
| Step 4 自動実行（AI生成） | `prep` | goal → llmChat(blog_creation_step4) |
| Step 5 自動実行（AI生成） | `basic_structure` | prep → llmChat(blog_creation_step5) |
| Step 6 自動実行（AI生成） | `opening_proposal` | basic_structure → llmChat(blog_creation_step6) |

Step 7（`本文作成`）からユーザーが操作を開始する。

### 16.4 新規テーブル: google_ads_blog_suggestions

AI出力からパースしたTOP5レコードを保存する一時テーブル。
...
create table if not exists public.google_ads_blog_suggestions (
  ...
  -- AI出力からパースした構造化データ（TOP5分）
  suggestions jsonb not null default '[]',
  -- 各要素の構造:
  -- {
  --   rank: number,           // 優先度順位（1〜5）
  --   main_kw: string,        // CampaignH1用キーワード
  --   kw: string,             // 参照KW + サブKW + ピラーKW
  --   impressions: string,    // 表示回数推計
  --   needs: string,          // 検索意図詳細 + ファネル分類
  --   persona: string,        // ペルソナ表
  --   search_intent: string,  // 検索意図分類（情報収集/比較検討/指名/最終/購買意図）
  --   match_quality: string,  // 一致度（高/中/低）
  --   seo_difficulty: string  // SEO難易度（高/中/低）
  -- }
...
**RLS**: SELECT は `get_accessible_user_ids(auth.uid())` パターンを使用（`google_ads_evaluation_settings` と統一）。代理店スタッフが事業者の代わりにブログ作成する場合のアクセスを考慮。INSERT/UPDATE は Service Role 経由。
...
-- INSERT/UPDATE は Service Role 経由のため RLS ポリシー不要
-- （RLS は enable だが、supabaseAdmin は RLS をバイパスする）
```

### 16.5 チャット画面の変更

- Step 3 スタート時、画面上部に引き継ぎバナーを表示：
...
- `StepActionBar` は `step3` から表示。バック操作で Step 1/2 に戻ることも可能（データは既に保存済み）。

### 16.6 実装工数見積もり（フェーズ2）

| コンポーネント | 内容 | 工数 |
|---|---|---|
| DB マイグレーション | `google_ads_blog_suggestions` テーブル + RLS | 0.5日 |
| AI出力パーサー | Claude出力からTOP5を構造化抽出（出力形式が不安定な場合のフォールバック含む） | 1.5日 |
| Step 3〜6 自動実行サービス | llmChat 4回連続呼び出し・各ステップのエラーハンドリング・部分成功時のフォールバック状態管理 | 2.5日 |
| API Route + ポーリング | `/api/blog-suggestions/prepare`（maxDuration:300）+ クライアント側ポーリング（タイムアウト・中断・エラー伝播） | 2日 |
| アプリ内表示画面 | ダッシュボードへのカード追加 or 専用ページ | 1日 |
| ChatClient 改修 | Step 7 スタート対応 + 引き継ぎバナー表示 | 1日 |
| E2Eテスト・確認 | カード→自動生成→Step7動作確認（部分失敗ケース含む） | 1.5日 |
| **合計** | | **約 10日（2週間）** |

### 16.7 前提・注意点

- このフェーズはフェーズ1（AI分析メール送信）の完成後に着手する。
- Claude へのプロンプトの出力形式を**構造化（JSON or 明確な区切り記号）**にする必要がある。プロンプトテンプレート設計時に考慮すること。
- Step 3〜6 の自動実行は既存の `prompt_templates`（`blog_creation_step3` 〜 `blog_creation_step6`）をそのまま利用する。
- 自動実行中にいずれかのステップが失敗した場合、失敗前の最終保存済みステップからチャット画面に遷移する（部分的な引き継ぎ）。
- メール送信側にはリンクを追加しない（操作はアプリ内に集約）。

#### タイムアウト対策

Step 3〜6 の自動実行は Claude API を4回連続呼び出すため、合計 60〜120秒かかる。Server Action のデフォルト制限（Vercel: 10秒）を超えるため、**API Route + バックグラウンド実行**方式を採用する。

```
ユーザーが [📝 記事作成へ] をクリック
    ↓
POST /api/blog-suggestions/prepare を呼び出す（即時 202 Accepted を返却）
    ↓
クライアントが完了ステータスをポーリング
    ↓
バックグラウンドで Step 3〜6 を順番に実行・content_annotations に保存
  ※ maxDuration: 300 を設定（/api/cron/google-ads-evaluate と同じ方式）
    ↓
完了後、クライアントがポーリング完了を検知してチャット画面へ遷移
```

- 既存の `/api/cron/google-ads-evaluate` と同じパターンのため実装コストは低い。
- ポーリング間隔: 3秒。最大待機時間: 150秒（タイムアウト時は失敗前の最終保存済みステップから遷移）。

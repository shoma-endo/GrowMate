...
- **対象ユーザー**: メールアカウント登録済みの全ユーザー（管理者限定ではない）。LINE のみのユーザーは対象外。
...
- **メールユーザーの場合**: 「AI分析を実行してメール送信」ボタンを表示（管理者・一般ユーザー問わず表示）
...
- 認証は `authMiddleware()` を使用する。`authMiddleware` は Supabase Auth セッション（Email認証）のみで `public.users` のユーザーを解決する（LINE 認証は廃止済み）。

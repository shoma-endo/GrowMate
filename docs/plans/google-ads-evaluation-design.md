...
| `customerName` | `GoogleAdsService.getCustomerInfo()`（取得失敗時は `customer_id` で代替） | Google Ads アカウント名 |
...
分析実行時に `GoogleAdsService.getCustomerInfo()` から取得する。取得に失敗した場合は `google_ads_credentials.customer_id` を代入する。

...
- 1 回の Cron で claim するジョブを 3 件から 1 件へ変更し、Anthropic への同時リクエスト数を抑える。
- LLM 個別タイムアウト 220 秒、ジョブ全体 240 秒は維持する。
- 本文リライトは入力本文を全量渡し、出力だけを 16,000 tokens に制限する。
- SDK の Abort は `LLM_TIMEOUT` として構造化ログへ記録する。
...
- 1 回の Cron で claim するジョブを 3 件から 1 件へ減らし、Anthropic への同時リクエスト数を抑える。
- 個別 LLM 呼び出しのタイムアウトを 180 秒から 220 秒へ延長する。
- ジョブ全体の 240 秒制限は維持し、結果の集約と DB 保存に 20 秒を残す。
- 本文リライトは入力本文を全量渡し、出力を 16,000 tokens に制限する。
- テンプレートごとに開始・完了・失敗、stage、provider、model、処理時間をログへ出す。
- 入力本文自体はログへ出さず、入力文字数と出力上限だけを記録する。
...
- claim RPC に 1 件を渡すこと。
- LLM 呼び出しへ 220 秒の timeout と AbortSignal が渡ること。
- SDK の Abort が `LLM_TIMEOUT` として構造化ログへ記録されること。

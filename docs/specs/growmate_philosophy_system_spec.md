...
実装計画：`docs/plans/philosophy-profile-mvp-plan.md`（段階導入・工数）

---

## 0. 実装段階（クライアント合意）

本書は **フル実装時の正** である。ただし **先行してパイロット（P0）** を実施し、効果を確認してから本書の DB・オンボーディング・学習ループへ進む。

### P0 パイロット（先行・軽量）

| 項目 | 内容 |
|------|------|
| 方針 | まず軽めの実装。`strength` 流用で足りればそのまま運用。別保存が必要になった時点で仕組みを構築する |
| 診断 | **GrowMate 外**（専用 GPTs 等）。10問対話後、貼り付け用テキストを出力 |
| 保存 | 事業者情報 `サービス内容（5W2H）` の **`strength`**（UI 表示名：`強み+毎回反映したい内容`）。DB カラム・JSON キーは変更しない |
| 注入 | 既存 `{{strength}}` 経路（`PromptService`）。哲学専用の profile 注入は **P0 では行わない** |
| スコープ外（P0） | `philosophy_profile` テーブル、GrowMate 内オンボーディング、edit_log、再生成、追い質問、Layer3 |

**P0 の位置づけ**: 本書 §5（オンボーディング）・§6（2層モデル）・§7（学習ループ）の **サブセット相当**（保存＋注入の最小形のみ。構造化・継続更新は未実装）。

### P0 後の分岐

1. **`strength` 1欄で十分** → フル実装は据え置きまたは長期バックログ
2. **強みと哲学の分離が必要** → `briefs` に別フィールド追加＋プロンプト変数（小規模）
3. **構造化プロファイル・学習ループが必要** → 本書どおり Phase 0〜5（計画書参照）

...
Service / Action / Route 層には Supabase・外部 API をモックしたテストが既存分として存在する。新たに書く場合は「[過剰なテストを書かない](#過剰なテストを書かない2026-09-26)」に従い、配線の代表ケースと、AC・回帰に紐づく性質に限る。
...
### 過剰なテストを書かない（2026-09-26）

2026-09-26 の監査で、モックを多用したテスト 36 ファイルの約 15% が過剰だった（`chore/test-slimming` で削減）。同じ形を増やさないため、テストを書く・直す・レビューするときは次に従う。

**書かない**

1. 下位層で固定済みの分類・集計・ログを、上位層（Service / Action / Route）で再検証しない。上位層は配線を代表ケース 1 本で確認する。
   - 例: OAuth の status 行列は `tests/unit/domain/errors/google-oauth-error-handlers.test.ts`、`batch_failed` ログは `tests/unit/server/lib/cron-observability.test.ts` の `runBatch`、GA4 集計値は `tests/unit/server/lib/ga4-aggregation.test.ts` が固定している
2. 同じ分岐を入力値だけ変えて別の `it` にしない。`it.each` 1 本にする
3. 同じ fixture を 2 回以上手書きしない。`buildX(overrides)` の factory にする
4. モックが返した値をそのまま assert しない（素通しの確認）
5. レビュー指摘の回帰は、既存テストへの assert 追加で済むなら新しい `it` を作らない

**消さない（過剰に見えても残す）**

削減時に「定数の写し」「素通し」と誤判定した 7 件が、実際には fix の回帰テストだった。次に当たるテストは残し、残す理由をテスト名かコメントに書く。

- 定数値そのものが fix の結果（例: claim の `p_limit: 1`、LLM 呼び出しの `timeoutMs`）。コミットハッシュを添える
- モックでは再現しない実環境の前提（例: `.select('id')` が無いと実環境で data が常に null になる、`user_id` でのスコープ、打ち切り時の並び順の固定）
- 二値分岐の片側だけを残さない（「不可」側だけ残すと、常に `false` を返す退行を見逃す）
- 認可・データ消失・課金・冪等性

判定する前に、値の由来を `git log -S '<値>' -- <本番ファイル>` で確かめる。「他のテストで検証済み」とするときは、同じ層を検証しているかまで確かめる（純関数の集計テストは、Service 側の写像を守らない）。

**Non-goal**

- 機械検出（重複 fixture の検出、ファイルごとの `it` 数上限など）は入れない。新規依存が要り、lint では「同じ分岐か」を判定できないため。次の監査で同じパターンが再発したら検討する
- 定期的な監査ジョブは作らない
...
上記の「別仕様として合意する」に対応する記録。`vitest.config.ts` の `coverage.thresholds` を `npm run verify` と CI（`.github/workflows/ci.yml` の test ジョブ）で強制する。

- 分母は `src/**` と `app/**` の全 TS ファイル（`src/types/**` と `*.d.ts` を除く）。従来の「テストが import したファイルのみ」の集計（lines 48.8%）は未テストのファイルを見えなくするため採らない
- 初期値は develop の全ファイル基準の実測（2026-09-08: lines 17.48% / statements 17.5% / functions 18.83% / branches 13.09%、テスト 703 件）から **1 ポイントの余白を引いた床**（lines 16 / statements 16 / functions 17 / branches 12）。低い数字をそのまま出発点にし、上げるためのテストは書かない
- ラチェット運用: `autoUpdate` は `floor(実測) - 1` で書き換える。**`floor(実測)` にしない理由**は、それだと床までの余白が実測の小数部そのままになり、余白が偶然小さいときに通常サイズの PR で落ちるため（2026-09-08 実測の branches 13.09% は床 13 だと未カバー分岐 128 個で割る＝機能 1 本ぶん）。閾値割れの規定動作が ABORT（下記）である以上、床は誤発火しない幅を持たせる。上がった差分はその PR でコミットする。閾値を下げる変更は本節を更新して理由を残す

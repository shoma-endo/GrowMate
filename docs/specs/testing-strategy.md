...
- 初期値は develop の全ファイル基準の実測（2026-09-03: lines 15.46% / statements 15.51% / functions 16.73% / branches 11.52%、テスト 560 件）の切り捨て。低い数字をそのまま出発点にし、上げるためのテストは書かない。main はテストが多く 1 ポイント高いが、ゲートは develop 起点の PR が最初に踏むので低い方に合わせる

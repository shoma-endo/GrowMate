...
- ページ送り・タブ切替のリンクに期間（`start` / `end`）も引き継ぐようにした（期間を URL で指定しているとき、または並べ替え中）。従来は期間を落としており、GA4 の列で並べたまま2ページ目へ進むと既定の期間（直近30日）で並び直され、行が重複・欠落するため。並べ替え中は既定の期間でも書くのは、日付をまたいで「直近30日」がずれるのを防ぐため。副作用として、並べ替え中に一度ページ送り等をすると期間が URL に固定され、並べ替えを解除しても「直近30日」へは自動で追従しない（期間の入力で変えられる。MVP では許容）。

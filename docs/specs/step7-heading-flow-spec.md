...
- 生成対象見出し: Step5 で保存された構成案テキストから抽出した `h2`（H2）、`h3`（H3）および `h4`（H4）行。
- 見出しレベル: 各生成対象見出しのレベル（H2/H3/H4）。保存時に H2=2 / H3=3 / H4=4 へ正規化して保持する。
...
- 生成対象:
  - `h2`（H2）、`h3`（H3）と `h4`（H4）の行を生成対象見出しとして抽出する（直後の半角/全角コロン・半角/全角スペースの有無は不問。例: `h2: 見出し` / `h2： 見出し` / `h2:: 見出し` も有効）
  - 生成順はテキスト内の出現順とする
  - 抽出した見出しの `heading_level` は保存時に H2=2 / H3=3 / H4=4 へ正規化する
- 非対象:
  - Markdown 記法の `#`, `##`, `###`, `####`, `#####`, `######` は生成対象見出しとしては扱わない

補足:
- 箇条書き（`-` / `*`）は見出しとして扱わない。
- 空見出しは除外する。
- リッチテキスト保存によって長い見出しの末尾だけが次行へ折り返された場合は、見出しが20文字以上・次行がインデント付き20文字以下・その直後が空行、の全条件を満たすときだけ次行を見出し末尾として連結する。
- セクション本文範囲は「対象見出し（`h2` / `h3` / `h4`）の直下から、次の対象見出し（`h2` / `h3` / `h4`）の直前まで」。
...
1. Step7 初回開始時（および書き出し案送信で見出しセクション削除後）の見出し抽出元は**メモ・補足情報の「基本構成」**（`content_annotations.basic_structure`）のみとする。
   ...
   - basic_structure が空、または `h2`/`h3`/`h4` が見出しとして抽出できない場合は、ユーザーに「基本構成」入力を促す導線を表示する。
   以降の再開時は再抽出せず、`session_heading_sections` を正本として読み込む。
   詳細は 5.2.1 を参照。
2. 見出し配列の先頭から順に「1見出し+本文」を生成する。
   - 構成案の `H2: 見出し` / `H3: 見出し` / `H4: 見出し` は、生成・Canvas 表示・完成形結合では `## 見出し` / `### 見出し` / `#### 見出し` に変換する。ラベル文字列は本文に残さない。
   - H2 を生成する場合も、配下の H3/H4 見出しおよびその本文は含めない。
   - LLM が後続の正規見出しと一致する H2/H3/H4 を出力した場合、保存時にその見出し以降を除外する。通常本文の `H2 直下…` 等は除外しない。
   - Canvas 付与見出しと LLM 応答内の対象見出しが冒頭で重複した場合、冒頭 lookahead 内で最後に一致する対象見出しまでを除外し、本文のみ保存する。
...
#### データ形式
- Markdown 形式のテキスト
- Step5 構成案と同じ形式（`h2` で大見出し、`h3` で中見出し、`h4` で小見出し。`extractHeadingsFromMarkdown` で抽出）

#### 読み取り時のエラーハンドリング
- `content_annotations` が存在しない、または `session_id` に紐づくレコードがない場合: 基本構成入力導線を表示
- `basic_structure` が null または空文字列の場合: 基本構成入力導線を表示
- 見出し抽出結果が 0 件（`h2`/`h3`/`h4` が含まれない）の場合: 基本構成入力導線を表示
...
| `src/lib/heading-extractor.ts` | `extractHeadingsFromMarkdown(markdown)` で `h2`/`h3`/`h4` から見出し抽出（basic_structure も step5 も同一ロジック） |
...
| `heading_level` | `smallint` | Yes | アプリ保存時に H2=2 / H3=3 / H4=4 へ正規化して保持（DB制約は `2,3,4` のみ許容） |
...
補足:
- `heading_level` は本フローの保存処理で必ず H2/H3/H4（2/3/4）へ正規化し、DB 制約も 2/3/4 以外は保存しない。
...
12. Step7 の生成対象見出しが `h2`（H2）、`h3`（H3）、`h4`（H4）である
...
15. H2 の見出し単位生成・保存・完成形作成に、配下の H3/H4 が重複して含まれない
16. 既存の見出し確定本文に後続 H2/H3/H4 が混入している場合も、次回の本文生成では最初の対象外見出し以降が除外される
17. 見出し単位生成・Canvas 表示・完成形の見出しが `## ...` / `### ...` / `#### ...` 形式となり、`H2:` / `H3:` / `H4:` ラベルが残らない
...
3. 見出し抽出元（5.2 参照）に従い、基本構成（basic_structure）から `h2`/`h3`/`h4` を再抽出する。

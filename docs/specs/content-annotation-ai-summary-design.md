...
> 改訂履歴
> - 2026-08-24: 実装済みのため正本を `docs/plans/` から `docs/specs/content-annotation-ai-summary-design.md` へ移動（`update-docs` の分類に従う）。一覧一括は引き続き `docs/specs/content-annotation-bulk-ai-summary-spec.md`。
...
> - 2026-07-13: コンテンツ一覧の編集モーダルにも要約導線を追加。右パネルとモーダルの要約ボタンを、既存の「タイトル・説明文生成」と同じ紫系のAI操作色・アイコン・左配置で保存操作から視覚的に分離した。ボタン・上書き説明Tooltip・実行状態・toast・認証競合処理は`ContentAnnotationSummaryAction.tsx`へ共通化した。
> - 2026-07-14: WordPress一括インポート由来の`session_id`を持たない記事にも対応するため、要約対象を`sessionId`または`annotationId`で指定可能に変更。コンテンツ一覧は`annotationId`を使用し、認証ユーザー本人のコンテンツを直接更新する。編集保存では未変更のURLと解決済み`wp_post_id`を再利用し、不要なWordPress再検索を行わない。セルフホストWordPressのGET処理には`rest_route`形式のフォールバックを追加した。
> - 2026-07-14: `opening_proposal`をAI要約から機械抽出へ変更。WordPress生HTMLの本文冒頭から最初の`h2`直前までにある空でない`p`要素の原文を、段落間の空行を維持して保存する。最初の`h2`または対象段落がない場合は空値とする。
> - 2026-07-14: チャット右パネルで要約成功直後に保存すると、親状態からローカルフォームへの`useEffect`同期前の旧値で要約結果を上書きできる競合を修正。成功コールバック内でローカルフォームを即時置換してから親状態を更新する。
> - 2026-07-15: `opening_proposal`の機械抽出対象を、冒頭の`p`に加えて`ul`・`ol`のリスト項目へ拡張。`ul`は中黒、`ol`は連番で整形し、ハイライトボックス等に含まれる導入部の要点も原文から保存する。
...
| `goal` | ゴール | Claude |
| `prep` | PREP | Claude |
| `opening_proposal` | 書き出し案 | **機械抽出**（本文冒頭から最初の`h2`直前までにある`p`と`ul`/`ol`のリスト項目。Claude出力は使用しない） |
...
- `app/chat/components/AnnotationPanel.tsx`（`AnnotationFormFields` 使用、chat画面の右パネル）
- `src/components/AnalyticsTable.tsx`（コンテンツ一覧の「コンテンツ情報を編集」モーダル）
...
- コンテンツ一覧からの AI 要約一括実行（トリガー・対象選定・時間予算）。**別仕様** `docs/specs/content-annotation-bulk-ai-summary-spec.md` で扱う。本設計は単記事要約の実装正本のままとする。
...
配置: `AnnotationFormFields` 内ではなく、各画面のアクション行に配置する。

- `AnnotationPanel.tsx`: 「キャンセル」「保存」と同じ`border-t`で区切られた行
- `AnalyticsTable.tsx`: 「コンテンツ情報を編集」モーダルの`DialogFooter`

配置をこの階層にする理由:
- 両画面とも`wp_post_id`・`canonical_url`を含む`AnnotationRecord`を直接保持しているため、活性化条件の判定に`AnnotationFormFields`への新規prop受け渡しは行わない。チャット右パネルは`sessionId`、コンテンツ一覧は一括インポート由来で`session_id`がない記事も扱えるよう`annotationId`を要約対象の識別子として使用する。
- アクション行に置くことで発見しやすくなる。

**「保存」ボタンとの見た目の差別化が必須**: 要約ボタンは「保存」とは性質が異なる（2.3のとおり、現在フォームに入力中の未保存内容を無視してWordPress本文から生成し直し、確認ダイアログなしで即DB上書きする）。同じ行・同じ見た目で並べると「AIで保存する」の一種と誤解され、未保存の手入力が黙って消えるリスクがある。対策として、既存の「タイトル・説明文生成」と同じ紫系の背景色・枠線、lucide-reactの`Sparkles`アイコン、「AIで要約」のラベルを使用する。要約ボタンは行の左側へ配置し、右側の「キャンセル」「保存」から視覚的に分離する。

条件:
- `wp_post_id` または `canonical_url` のいずれかが解決できる（WordPress連携済み）場合のみ活性化。
- 未連携時はボタンを disabled にし、「WordPress投稿URLを保存すると利用できます」と次の操作を説明する。
- チャット右パネルでは、初期データに加えて保存成功レスポンスの`canonical_url`・`wp_post_id`から連携状態を更新する。入力中の未保存URLだけでは活性化せず、連携保存または連携解除の成功直後にボタン状態を同期する。

### 3.2 状態表示（`growmate-ui-ux` AI連携UIの鉄則に準拠）

- **上書き説明**: ボタン横にCanvasパネルと同じ`Info`アイコンを配置し、「WordPress本文から生成し直します。現在入力中の未保存内容も含めて上書きされます」とTooltipで表示する。マウスホバーだけでなくキーボードフォーカスでも確認できるようにする。
...
- チャット右パネルでは、成功レスポンスを`useAnnotationForm`のローカルフォームへ即時反映してから親の`annotationData`を更新する。親propsからの`useEffect`同期だけに依存せず、要約直後の保存でも旧値へ巻き戻さない。
- コンテンツ一覧の編集モーダルでは、成功後もモーダルを閉じず、返却された最新値をフォームへ即時反映する。
- 失敗時: toast でエラーメッセージを表示し、原因に応じた次アクションを示す（後述 9. エッジケース）。**失敗時は既存の`content_annotations`の値を変更しない**（Memory Write Policy: 失敗時に正本を消さない）。
- モーダル on モーダルは発生しない（編集モーダル内から追加モーダルは出さない）。
...
「要約」ボタンクリック
  ↓
Server Action: summarizeContentAnnotation({ sessionId } | { annotationId })
  ↓
認証・本人所有チェック
  ↓
content_annotations 取得（チャット右パネルはsession_id、コンテンツ一覧はannotation id起点。認証ユーザーのuser_idで制限し、wp_post_id / canonical_url / impressionsを含めて取得）
  ↓
WordPress本文取得（1回のWordPress REST API呼び出しから2種類を得る。8.1参照）
  ├─ contentText: HTMLタグ除去済みプレーンテキスト（従来どおり wp_content_text としてキャッシュ）
  └─ contentHtml: 生HTML（DBには保存しない。このリクエスト内でのみ使用しbasic_structure・opening_proposal抽出に渡す）
  ↓
本文サイズガード（上限超過ならAI呼び出し前にエラー終了。9.参照）
  ↓
┌─────────────────────────┬─────────────────────────────┐
│ basic_structure・書き出し（機械抽出）│ 他6項目（Claude JSON出力）      │
│ contentHtmlの見出しと冒頭pタグを    │ contentText（プレーンテキスト）  │
│ パースして原文から決定              │ を prompt_templates経由でllmChat│
...
- 生成結果は `extractHeadingsFromMarkdown`（`src/lib/heading-extractor.ts`、既存関数）で検証する。0件の場合はエラーにせず `basic_structure` を空文字のまま保存する（元記事に見出しがない場合は正当な結果のため）。他の項目の保存は独立して継続する。

### 6.1 opening_proposalの機械抽出仕様

- 入力: `content.rendered`（生HTML）。
- 範囲: 本文冒頭から最初の`<h2>`直前まで。
- 対象: 範囲内の空でない`<p>`要素と、`<ul>`/`<ol>`直下の`<li>`要素。その他の要素に含まれる文字列は対象外とする。
- 処理: インラインHTMLを除去し、パーサーでHTMLエンティティをデコードした原文テキストを取得する。`ul`の項目には`・`、`ol`の項目には1始まりの連番を付け、同一リスト内は改行、段落・リストのブロック間は空行で連結する。
- 最初の`h2`がない、または範囲内に有効な対象要素がない場合は空値を保存する。
- 現行プロンプトとの後方互換のためAI応答の`opening_proposal`は受理するが、保存値には使用しない。書き出し案の正本は生HTMLからの機械抽出結果とする。
...
| ファイル | 責務 |
|---|---|
| `src/server/actions/contentAnnotationSummary.actions.ts` | `'use server'`。認証・本人所有チェック・全体オーケストレーション |
...
当初はチャット右パネルだけを対象として`sessionId`起点・`upsertContentAnnotationBySession`で設計したが、コンテンツ一覧にはWordPress一括インポート由来で`session_id`を持たない記事が存在する。要約のためだけにチャットセッションを作成すると不要なデータと機能間結合が増えるため、対象識別子と書き込み方式を次のように変更する。
...
export async function summarizeContentAnnotation(
  target: { sessionId: string } | { annotationId: string }
): Promise<ServerActionResult<AnnotationRecord>>
```

1. 認証（既存 `withAuth` パターン）
2. `sessionId`または`annotationId`のいずれか一方をzodで検証
3. `content_annotations`を指定された識別子と認証ユーザーの`user_id`で取得する
...
12. パース結果の7項目と手順7の`basic_structure`を統合し、対象`annotationId`・認証ユーザーの`user_id`を条件に8項目のみ部分更新する。`impressions`は更新対象に含めず既存値を維持する
...
| `canonical_url`のみ保存され`wp_post_id`がない記事 | URLスラッグで投稿を検索する。API境界の`WordPressService`で、セルフホストREST APIの`id`またはWordPress.com形式の`ID`を必須の`id`へ正規化し、以降の内部処理は`id`だけを参照して本文を取得する |
| WordPress.comの保存トークン更新に失敗し、有効なブラウザCookieがある | Cookieトークンを優先して本文を取得する。Cookieがないバックグラウンド処理では保存トークンの更新結果を使用する |
...
11. 要約ボタン横の`Info`アイコンへホバーまたはキーボードフォーカスすると、上書き・未保存内容破棄の説明が表示される
12. `admin/prompts` 画面で `content_annotation_ai_summary` が新設の「コンテンツ情報要約」タブに表示され、「AIチャット・生成」「GSC改善提案」タブには重複表示されない
13. チャット画面の右パネルとコンテンツ一覧の編集モーダルの両方に要約ボタンが表示される
14. 要約ボタンはアイコン・強調色・左配置により、右側の「キャンセル」「保存」と視覚的に区別できる
15. コンテンツ一覧の編集モーダルで要約に成功した場合、モーダルを閉じずに生成結果が各フォームへ反映される
16. WordPress一括インポート由来で`session_id`がない記事でも、コンテンツ一覧からAI要約を実行できる
17. 保存済みURLが変更されておらず`wp_post_id`がある場合、他フィールドの保存でWordPress投稿検索を再実行しない
18. 本人以外が所有する記事は保存・要約できない
19. セルフホストWordPressで`/wp-json/`形式のGETが拒否された場合、`index.php?rest_route=`形式を試行する
20. チャット右パネルでWordPress投稿URLを保存・解除した直後、パネルを開き直さなくてもAI要約ボタンの活性状態が同期される

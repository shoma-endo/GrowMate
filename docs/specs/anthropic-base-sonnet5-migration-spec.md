...
| 指標 | 現状 | 目標 | 測定方法 | 測定時期 |
| --- | --- | --- | --- | --- |
| §8「出力品質・評価基準」の合否条件を満たすグループ数 | — （移行前は未計測） | 7/7 | §13 の手動確認。判定条件は §8「出力品質・評価基準」の絶対条件 | §14 手順6 完了時 |
...
| 思考トークンの混入がないこと | 該当なし（`claude-sonnet-4-6` は明示指定しない限り思考しない） | 18エントリすべてで `thinking` がリクエストに載っている | **AC-M02b**（AC-M02 は共有定数の記述しか検証しないため、この指標は測れない）。`[LLMService] output truncated at max_tokens` / `[Ga4EvaluationLlm] structured output validation failed` が出ないこと（§13） | §14 手順6 完了時 |
...
- **移行前ベースラインの採取**: G1〜G7 を移行前にもう1周実行して出力を保存する手順は置かない。理由は3つ。(1) §8「出力品質・評価基準」の7条件のうち**6条件は移行後の1回で判定できる絶対条件**（途中切れの有無・`JSON.parse` の成否・構造化出力バリデーションの成否・ストリーミングの完了）で、比較対象を必要としない。(2) 残る G2 / G6 も「応答が返り画面に項目が表示される」という絶対条件に書き換えられ、そうした。(3) **LLM 出力は非決定的**（`seed` は Anthropic の `params` に載らない。§10）なので、n=1 の前後比較では文体・粒度の差がモデル差かサンプリングの揺らぎか区別できず、判断材料にならない。**取り直しが必要になっても移行前のコミットへ戻せば再取得できる**ため、失われるのは検証可能性ではなく実費だけ。`CLAUDE.md` Core Rules（MVP 最優先 / 要件に無いものを先回りで作らない）に従い置かない
...
- 含めるもの: 共有定数の変更、`thinking` の詰め替え配線、7グループの手動確認、回帰の切り分け
...
幅の理由: RISK-M01 は 2026-09-22 実機確認で解消。回帰が何グループで出るかが実行するまで分からないため、工数幅の主因は手動確認側に残る。
...
| 7グループの動作確認 | §13 のテスト方針に沿って各グループ1回以上実行する | 4〜7 |
...
シナリオ: AC-M02b 共有定数の thinking が実際に Anthropic リクエストへ届いている
  ...
  かつ G1 step7 と G5 と G7 を実行したとき
       [LLMService] output truncated at max_tokens と
       [Ga4EvaluationLlm] structured output validation failed が出ない
...
| AC-M02b | FR-M02 | コード確認（呼び出し元 8ファイル・11箇所を1つずつ）＋ 外形確認（`stop_reason === 'max_tokens'` 警告と GA4 の構造化出力エラーが出ないこと） |
...
| 観点 | 本仕様での扱い |
| --- | --- |
| **出力品質・評価基準** | **グループごとに、移行後の1回の実行だけで判定できる絶対条件を置く**（移行前ベースラインは採らない。理由は §4 Non-goals）。**G1**: step7 の応答で `stop_reason` が `max_tokens` にならない（`[LLMService] output truncated at max_tokens` が出ない）。**G2 / G6**: 応答が返り、画面に項目が表示される。**G3**: 32000 トークン枠内で完結し、上記の切り詰め警告が出ない。**G4**: 4種とも応答が返り、本文提案のストリーミングが完了する。**G5**: 末尾 JSON ブロックが `JSON.parse` に通る。**G7**: `[Ga4EvaluationLlm] structured output validation failed` が出ず、`llm_output_invalid` にならない。**新しい計測基盤・スコアリングは作らない**（既存ログと画面表示で判定する） |
...
| ID | リスク | 影響 | 対応 |
| --- | --- | --- | --- |
| RISK-M01 | `app/api/chat/anthropic/stream/route.ts:281` の Web検索ツールが旧バリアント `web_search_20250305` を使っている。**2026-09-22、PR #559 プレビューで人間が Web検索付きブログ生成を実機確認し、Web検索経路で特に問題なし（§14 手順4 通過確定）。** 公式のモデル対応表は依然として無い（§16） | ブログ生成の Web検索経路が 400 で落ちる可能性 | **解消（実機確認済み）。§14 手順5（`web_search_20260209` への更新）は不要。** 将来フォールバックが必要になった場合のみ、その1箇所を `web_search_20260209` へ上げ、**あわせて `allowed_callers: ['direct']` を明示する**（§16 verbatim・確認日 2026-09-07） |
...
| RISK-M06 | 入力トークン推定器（`src/lib/knowledgeBudget.ts:2`）が旧トークナイザで校正されており、Sonnet 5 では実トークンを約30%過小に見積もる。推定器自身が宣言する許容幅 ±15% を超える | step7 のナレッジ注入ガード（40,000 で予算縮小・60,000 でスキップ）が設計より約30%外側で発火し、設計意図より多い入力が 64000 の出力枠へ進む。RISK-M02 の発生確率が想定より高くなる | **係数は本仕様で変えない**（§10 技術前提）。実害は step7 の途中切れとして現れるため、§13 G1 の `[LLMService] output truncated at max_tokens` が出ないことで足りる（ログの前後比較はしない。§4 Non-goals）。係数の変更可否は OPEN-M05 |
...
**合否はすべて移行後の1回の実行だけで判定できる絶対条件**とし、移行前ベースラインの採取は行わない（理由は §4 Non-goals）。合否条件は §8「出力品質・評価基準」と揃えてある。

| グループ | 確認手順 | 見るところ |
| --- | --- | --- |
| G1 ブログ生成 | `/chat` で step1〜step7 を通し、見出し単位モードも1回実行する | 各ステップが応答すること。**step7 の本文が途中で切れないこと**＝`[LLMService] output truncated at max_tokens` が出ないこと（RISK-M02）。Web検索が有効な経路で 400 が出ないこと（RISK-M01）。**途中切れが起きた場合、既存の続き生成導線（`isContinuation` 経路）が従来どおり働き、続きが元メッセージへ上書き保存されるか**（RISK-M02） |
| G2 広告コピー | `/chat` で広告コピー生成を1回 | 応答が返り、画面に広告コピーが表示されるか |
...
| G6 Google Ads 除外KW | 1回 | キーワードが画面に出るか |
...
| 手順4完了時 | Web検索経路が 400 にならない | 手順5を実施する | 人間 | **通過確定（2026-09-22）** — 人間実行。環境: PR #559 プレビュー。Web検索付きブログ生成を実機確認し、Web検索経路で特に問題なし。**RISK-M01 解消。§14 手順5は不要** |
| 手順6完了時 | G1〜G7 すべてで §8「出力品質・評価基準」の合否条件を満たす | 手順7へ | 人間 | **G1 通過（2026-09-22、PR #559 プレビュー、人間実行・特に問題なし）／G2〜G7 未確認** |
...
- G1〜G7 の手動確認が完了し、§8「出力品質・評価基準」の合否条件を満たしている
- RISK-M01（Web検索ツール型）の可否が確定している（**2026-09-22 実機確認・手順5不要**。§14 手順4）。`web_search_20260209` へ上げた場合のみ `allowed_callers: ['direct']` が明示されている
...
- 🟡 **ベースライン採取手順が無い**: AC-M04 / §13 が「移行前と同じ構造」を要求しながら採取手順を持たず、手順1で共有定数を変えた後は比較対象が存在しなかった。§14 に手順0 を追加し、§8 に「出力品質・評価基準」行（グループごとの機械判定条件）を足した（→ **2026-09-08 に手順0 を撤回し、§8 の合否条件を移行後1回で判定できる絶対条件へ統一した**。7条件中6条件がもともと絶対判定で比較対象を必要とせず、非決定的な出力の n=1 比較に判断材料としての価値が無いため。§4 Non-goals「移行前ベースラインの採取」。**§8 の合否条件そのものはこの指摘の成果として残っている**）
...
- **`web_search_20250305` × `claude-sonnet-5` の明示的なモデル対応表は公式に存在しない**（Web検索ツールのページと Tool reference は相互参照で終わる）。**実機可否は 2026-09-22 に PR #559 プレビューで人間確認済み**（§14 手順4 通過確定・RISK-M01 解消・手順5不要）。公式表が無いこと自体は `.agents/skills/spec-review/external-services.md:66` の規約どおり未確認のまま残す
...
| 日付 | 変更内容 | 理由 | 変更者 |
| --- | --- | --- | --- |
| 2026-09-22 | **RISK-M01 を実機確認で解消、§14 手順4 を通過確定、手順5不要を明記。** PR #559 プレビューで人間が Web検索付きブログ生成を実行し特に問題なし。§12 RISK-M01・§14 チェックポイント「手順4完了時」・§15 完了条件・§16 未確認項目・§5 工数幅の理由を更新 | 移行検証の記録依頼 | 人間実行結果の反映 |
| 2026-09-08 | **§14 手順0（移行前ベースラインの採取）を撤回し、§4 Non-goals に理由付きで記録した。**あわせて §8「出力品質・評価基準」の G2 / G6 を「応答が返り画面に項目が表示される」という絶対条件へ書き換え、§1 成功指標・§7 AC-M02b / AC-M04・§13 テスト方針とリリース方針・§14 手順とチェックポイント・§15 完了条件から比較前提を外した。RISK-M06 のログ前後比較も、実害が step7 の途中切れとして現れるため G1 の絶対条件に寄せた。工数は 11.5〜22 時間 → **10.5〜20 時間**（準備フェーズ 1〜2 時間を削除）。**§8 の合否条件そのものと、他の spec-review 指摘の反映は一切変更していない** | `CLAUDE.md` Core Rules「GrowMate は MVP 開発を最優先とする」「要件に無い機能を先回りで作らない」に照らして過剰と判断したため。7条件中6条件はもともと移行後1回で判定できる絶対条件で比較対象を必要とせず、残る G2 / G6 も絶対条件に書き換えられた。加えて `seed` が Anthropic の `params` に載らず出力が非決定的なため、n=1 の前後比較では差分がモデル由来かサンプリングの揺らぎか区別できない。「手順1以降では比較対象が失われる」という手順0 の前提も、移行前のコミットへ戻せば再取得できるため正確でなかった | Claude（ローカルセッション） |

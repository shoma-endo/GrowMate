...
- 生成対象見出し: Step5 で保存された構成案テキストから抽出した `h3`（H3）および `h4`（H4）行。
...
- 生成対象:
  - `h3`（H3）と `h4`（H4）の行を生成対象見出しとして抽出する（直後の半角/全角スペース有無は不問）
  - 生成順はテキスト内の出現順とする
...
- 箇条書き（`-` / `*`）は見出しとして扱わない。
- 空見出しは除外する。
- セクション本文範囲は「対象見出し（`h3` or `h4`）の直下から、次の対象見出し（`h3` or `h4`）の直前まで」。
...
1. Step7 初回開始時（および書き出し案送信で見出しセクション削除後）の見出し抽出元は**メモ・補足情報の「基本構成」**（`content_annotations.basic_structure`）のみとする。
   ...
   - basic_structure が空、または `h3`/`h4` が見出しとして抽出できない場合は、ユーザーに「基本構成」入力を促す導線を表示する。
...
| `src/lib/heading-extractor.ts` | `extractHeadingsFromMarkdown(markdown)` で `h3`/`h4` から見出し抽出（basic_structure も step5 も同一ロジック） |
...
12. Step7 の生成対象見出しが `h3`（H3）と `h4`（H4）である
...
3. 見出し抽出元（5.2 参照）に従い、基本構成（basic_structure）から `h3`/`h4` を再抽出する。

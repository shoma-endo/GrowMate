...
- 完成形用: **`getCombinedContentForStep7`**（`src/server/actions/heading-flow.actions.ts`）で結合テキストを取得し、Stream API に `step7FullBodyGeneration: true` で送信。AI（`blog_creation_step7`、maxTokens: 20000 / `src/lib/constants.ts`）が完成形を生成し、Stream API 内で `headingFlowService.saveCombinedContentSnapshot` により `session_combined_contents` に保存。長文出力のため maxTokens は 20000 に設定済み。

### 12.1 書き出し案の保存・取得仕様

#### データフロー（要約）

- **完成形作成のトリガー**: StepActionBar の「本文生成」ボタンのみ。`getCombinedContentForStep7` で結合テキストを取得し、Stream API に送信。サーバー側で 12.1 の優先順で書き出しを取得した上で結合する。
...
完成形作成は StepActionBar の「本文生成」ボタンで行われる。`getCombinedContentForStep7` 内で `headingFlowService.getCombinedContentForPrompt` が以下を実行する。
...
- **`getCombinedContentForStep7`**（`src/server/actions/heading-flow.actions.ts`）: パラメータ `{ sessionId, liffAccessToken }`。`headingFlowService.getCombinedContentForPrompt` で上記優先順で取得した結合テキストを返す（DB 保存なし）。

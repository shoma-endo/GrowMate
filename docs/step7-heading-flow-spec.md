...
- **完成形の書き出し**: 完成形の先頭に置くテキスト。`getStep7UserLead` で `chat_messages`（model=blog_creation_step7_lead）の最新1件を取得。ない場合は空文字列。
...
- **保存先の一貫性**: Step6→7 または見出し再スタート時に `chat_messages`（model=blog_creation_step7_lead）に保存した書き出し案を、完成形作成時にサーバー側で `getStep7UserLead` により取得。
...
- **lead**: `getStep7UserLead` で取得（`chat_messages` の role=user, model=blog_creation_step7_lead の最新1件）。ユーザープロンプトに注入。
...
| `src/server/services/headingFlowService.ts` | `getCombinedContentForPrompt`: 書き出しを getStep7UserLead で取得し見出し本文と結合。`initializeHeadingSections`、`resetHeadingSections`（Step7 初期化用） |

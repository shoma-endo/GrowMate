...
6. **全見出し確定後**、送信内容により分岐する（8.3・8.7 参照）:
   - **空送信**: 保存済み書き出し案＋各見出し本文を結合して `session_combined_contents` に完成形として保存（AI 不使用）
   - **書き出し案を入力して送信**: 見出し再スタート。完成形保存は行わず、見出し1からやり直す
...
- Step7 の完成形（`session_combined_contents`）:
  - 完成形フェーズで**空送信**したときに保存（保存済み書き出し案＋見出し確定本文を連結）
  - 書き出し案を入力して送信した場合は見出し再スタート（8.7）となり、完成形保存は行わない
  - **ユーザー入力の書き出し**（空送信時は保存済み）＋**見出し確定本文**を連結（AI 不使用）
...
7. 4〜6 を最終見出しまで繰り返し
8. 全見出し確定後、**空送信**で完成形を保存（Step6→7 で保存済みの書き出し案を使用）。やり直す場合は**書き出し案を入力して送信**で見出し1から再スタート（8.7）
...
全見出し確定後、プレースホルダーが「書き出し案を入力して送信すると、見出し1から始まります。」に変わり、見出し生成ボタンは非表示。送信内容により動作が分かれる。

| 送信内容 | 動作 |
|----------|------|
| **空送信** | 保存済み書き出し案（Step6→7 または chat_messages）＋各見出し本文を結合し、`session_combined_contents` に保存（AI 不使用）。完成形を確定する。 |
| **書き出し案を入力して送信** | 見出し再スタート（8.7 参照）。完成形は保存せず、`session_heading_sections` を削除して見出し1からやり直す。入力した書き出し案は `chat_messages` に保存される。 |
...
完成形フェーズ（8.3）でユーザーが**書き出し案を入力して**送信すると、以下の動作が行われ、見出し1から再スタートできる。**空送信は完成形保存（8.3）となり、本動作にはならない。**
...
- **完成形作成のトリガー**: 完成形フェーズで**空送信**したときのみ `buildCombinedContentWithUserLead` が呼ばれる。書き出し案を入力して送信した場合は見出し再スタート（8.7）となり、完成形作成は行わない。
- **保存先の一貫性**: Step6→7 または見出し再スタート時に `chat_messages` に保存した書き出し案を、完成形作成時（空送信時）にサーバー側で取得して使用する。
- **取得ロジックの分担**: InputArea の空送信時は `userProvidedLead` に空文字列を渡す。サーバー側 `buildCombinedContentWithUserLead` は、受け取った値が空のとき `chat_messages` から取得する。
...
- 見出し再スタート時（8.7）は `resetHeadingSections` を `preserveStep7Lead: true` で呼ぶため、直前に保存した書き出し案は保持される。次回の完成形作成時（空送信）では `getStep7UserLead` が最新1件を取得するため、その書き出し案が使用される

#### 完成形作成時の取得（空送信時のみ）

完成形作成は**空送信**時のみ行われる。書き出し案を入力して送信した場合は見出し再スタートとなるため、本取得は実行されない。
...
1. ユーザーが完成形フェーズで**書き出し案を入力して**送信する（空送信は完成形保存となり本動作にはならない）。

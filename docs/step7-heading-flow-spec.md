...
3. Step7 で StepActionBar の「見出し生成」をクリック → AI が1見出し分をチャットに生成
4. チャットで確認し、必要ならタイルクリックで Canvas を開いて修正指示
5. StepActionBar の「保存」をクリック → 次見出しへ進む
6. 3〜5 を最終見出しまで繰り返し
7. 全見出し確定後、プレースホルダー「書き出し案を入力してください、本文を出力します。」に従い、書き出し案を入力して送信 → 完成形として保存（AI 不使用）
8. 必要に応じて Canvas で修正指示
9. 最終生成（最新完成形を入力に）
...
表示内容:
- `構成案から抽出した見出し {currentIndex}/{totalCount}` ＋ 現在見出し名（省略表示）
...
- 完成形の再結合時は、ユーザー入力の書き出しを先頭に置く。未入力の場合は空文字列として扱い、見出し確定本文のみを連結する。
- ユーザー入力の書き出しは、全見出し確定後にユーザーが InputArea で入力して送信したテキストを、完成形用 Server Action で受け取る。
  - 実装: `buildCombinedContentWithUserLead` in `src/server/actions/heading-flow.actions.ts`
  - パラメータ: `{ sessionId, userProvidedLead, liffAccessToken }`（userProvidedLead は空白のみでないこと。refine で検証）
  - 戻り値: `{ success: boolean, error?: string }`
  - 責務: userProvidedLead と見出し確定本文を結合し、`session_combined_contents` に新バージョンとして保存

...
Canvas は修正指示専用。完成形は書き出し（12.1 参照）＋各見出しの結合で出力（AI 不使用）。
...
- **完成形の書き出し**: 完成形の先頭に置くテキスト。取得優先順は 12.1 に従う。1) 呼び出し元が渡した `userProvidedLead`、2) `chat_messages`（model=blog_creation_step7_lead）の最新1件、3) Step6 チャット（blog_creation_step6 の assistant 最新）。いずれもない場合は空文字列。
- 完成形: Step7 時点で、**完成形の書き出し**（上記）＋**見出し確定本文（order_index 順）**を連結した全体本文。**一度生成されたら `session_combined_contents` に永続的に保存され、バージョン管理される。通常の編集フローでは削除されない。**（ユーザーによるデータ削除要求やデータ保持ポリシーに基づく削除は本仕様のスコープ外。2.スコープ参照）
...
6. **全見出し確定後**（8.3・8.7 参照）:
   ...
   - **完成形の保存**: StepActionBar の「本文生成」ボタンで実行。保存済み書き出し案＋各見出し本文を結合して `session_combined_contents` に保存（AI 不使用）
...
- Step7 の完成形（`session_combined_contents`）:
  - StepActionBar の「本文生成」ボタンで保存。書き出しは 12.1 の優先順でサーバー側取得し、見出し確定本文と連結（AI 不使用）
...
- **完成形の保存**: StepActionBar の「本文生成」ボタンで実行。保存済み書き出し案（Step6→7 または chat_messages）＋各見出し本文を結合し、`session_combined_contents` に保存する（AI 不使用）。
...
- **完成形作成のトリガー**: StepActionBar の「本文生成」ボタンのみ。`buildCombinedContentWithUserLead` は `userProvidedLead` を省略または空で呼ぶ。サーバー側で上記「完成形作成時の取得」の優先順に従い取得。
- **保存先の一貫性**: Step6→7 または見出し再スタート時に `chat_messages`（model=blog_creation_step7_lead）に保存した書き出し案を、完成形作成時にサーバー側で第1候補として取得。ない場合は Step6 チャットをフォールバック。
...
完成形作成は StepActionBar の「本文生成」ボタンで行われる。`userProvidedLead` を渡さない（または空）ため、サーバー側 `headingFlowService.combineSections` で以下を実行する。

1. **userProvidedLead** が渡されていればそれを優先（本ボタン経由では空）
2. **getStep7UserLead**: `chat_messages` の role=user, model=blog_creation_step7_lead の最新1件
3. **getStep6Lead**: `chat_messages` の role=assistant, model=blog_creation_step6 の最新1件（フォールバック）
4. いずれもない場合は空文字列。見出し確定本文のみを連結

#### 実装

- **`buildCombinedContentWithUserLead`**（既存。`src/server/actions/heading-flow.actions.ts`）: パラメータ `{ sessionId, userProvidedLead?: string, liffAccessToken }`。`userProvidedLead` は省略可。空/未指定時にサーバー側で上記優先順で取得。
...
| `src/server/services/headingFlowService.ts` | `combineSections`: 書き出しを getStep7UserLead→getStep6Lead の優先順で取得し見出し本文と結合。`initializeHeadingSections`、`resetHeadingSections`（Step7 初期化用） |
...
| `src/server/actions/heading-flow.actions.ts` | `resetHeadingSections` アクション、`buildCombinedContentWithUserLead`（完成形作成: 12.1 の取得ロジックで書き出しを取得し、見出し本文を結合して `session_combined_contents` に保存。実装は `headingFlowService.combineSections` を呼ぶ） |

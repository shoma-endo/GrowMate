...
- 完成形: Step7 時点で、**ユーザー入力の書き出し**（8.3参照）＋**見出し確定本文（order_index 順）**を連結した全体本文。Step6 書き出し案は使用しない。
...
7. 4〜6 を最終見出しまで繰り返し
8. 全見出し確定後、プレースホルダー「書き出し案を入力してください、本文を出力します。」に従い、書き出し案を入力して送信 **または空で送信**（Step6→7 で保存済みを使用）→ 完成形として保存（AI 不使用）
...
- 全見出し確定後、プレースホルダーが「書き出し案を入力してください、本文を出力します。」に変わり、見出し生成ボタンは非表示
...
目的:
- **Step7** の「完成形履歴」を蓄積する
- 完成形 = **ユーザー入力の書き出し** + 見出し確定本文（order_index 順で連結）
...
| `content` | `text` | Yes | 当該バージョンの完成形全文（ユーザー入力の書き出し＋見出し本文） |
...
- 未保存状態での遷移:
  - 確認ダイアログで破棄/継続を選択
- ユーザーが書き出しを入力していない場合:
  - **空または空白のみの入力**で送信した場合: バリデーションエラー「書き出し案を入力してください」をトースト表示し、完成形の作成は行わない（送信前に弾く）。
  - 何らかの経路で空文字列が渡された場合（例: 他モジュールからの呼び出し）: 空文字列として扱い、完成形の先頭に何も付与せず、見出し確定本文のみを連結する。確認ダイアログや警告は表示しない。
...
- 完成形用 Server Action: `buildCombinedContentWithUserLead` in `src/server/actions/heading-flow.actions.ts`
  - パラメータ: `{ sessionId, userProvidedLead?: string, liffAccessToken }`（userProvidedLead は省略可。12.1 参照）
...
- **ケース2**: ユーザーが空で送信した場合  
  ...
  → 取得できない場合は getStep6Lead（blog_creation_step6）へフォールバック  
  → いずれもない場合は空文字列として扱い、見出し確定本文のみを連結
...
- `combineSections(sessionId, userId, userProvidedLead?)`: userProvidedLead が空/未指定時、内部で getStep7UserLead → getStep6Lead の順に取得

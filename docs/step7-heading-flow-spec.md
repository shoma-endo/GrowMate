...
Canvas は修正指示専用。完成形はユーザー入力の書き出し＋各見出しの結合で出力（AI 不使用）。
...
- **ユーザー入力の書き出し**: 全見出し確定後、完成形フェーズ（8.3）の InputArea でユーザーが入力して送信したテキスト。空送信の場合は `chat_messages`（model=blog_creation_step7_lead）から取得し、なければ空文字列。
- 完成形: Step7 時点で、**ユーザー入力の書き出し**（上記）＋**見出し確定本文（order_index 順）**を連結した全体本文。Step6 書き出し案は参照しない（パターンA）。**一度生成されたら `session_combined_contents` に永続的に保存され、バージョン管理される。通常の編集フローでは削除されない。**（ユーザーによるデータ削除要求やデータ保持ポリシーに基づく削除は本仕様のスコープ外。2.スコープ参照）
...
1. Step7 初回開始時（および構成リセット後）の見出し抽出元は**メモ・補足情報の「基本構成」**（`content_annotations.basic_structure`）のみとする。
...
6. **全見出し確定後、ユーザーが書き出し案を入力して送信すると、書き出し＋各見出しを結合して完成形として保存する（AI 不使用）。**
...
- Step7 開始後に基本構成を変更した場合:
  ...
  - 見出し構成を更新したい場合は、ユーザー明示操作で「構成リセット」を実行して再開始する（UI仕様は 8.6 を参照）
- 理由:
...
- Step7 の完成形（`session_combined_contents`）:
  - ユーザーが書き出し案を入力して送信したときに保存
  - **ユーザー入力の書き出し**＋**見出し確定本文**を連結（AI 不使用）
...
3. **Step6→Step7 遷移**: 書き出し案を入力して送信 → **保存のみ**（AI 呼び出しなし）。`chat_messages` に user メッセージ（model=blog_creation_step7_lead）として保存
...
7. 4〜6 を最終見出しまで繰り返し
8. 全見出し確定後、プレースホルダー「書き出し案を入力して送信すると、見出し1から始まります。」に従い、書き出し案を入力して送信 **または空で送信**（Step6→7 で保存済みを使用）→ 完成形として保存（AI 不使用）
...
- ユーザーが書き出し案を入力して送信すると、書き出し＋各見出しを結合して `session_combined_contents` に保存（AI 不使用）
- **Step6→7 で保存した書き出し案がある場合**: 空入力で送信するとその書き出し案を使って完成形を保存
- 完成形は Canvas で開いて修正指示できる

#### プレースホルダーの動的表示

- **保存済み書き出し案がない場合**: 「書き出し案を入力して送信すると、見出し1から始まります。」
- **保存済み書き出し案がある場合**: 「書き出し案を入力して送信すると、見出し1から始まります。空送信で保存済みの書き出し案を使用します。」
...
| 項目 | 仕様 |
|------|------|
| ボタン配置 | StepActionBar の三点リーダードロップダウン内。`戻る` ボタンの近くなど、目立ちすぎない位置。 |
| 表示条件 | **Step7 表示中のみ**表示可。Step6 では表示しない。 |
| ボタン文言 | `構成リセット` |
...
- 文言例:
  ...
  ```
- 選択肢: `キャンセル` / `OK`（`window.confirm` を使用）

#### 初期化後の遷移先

- **Step7 の先頭見出し（order_index 0）**に遷移する。
- 基本構成（basic_structure）から見出しを再抽出し、`session_heading_sections` を全削除後に新規投入する。
- `session_combined_contents` は保持する（構成リセット後も完成形履歴は参照可能）。
- ユーザーは新しい見出しリストの1件目から再開する。Step5 に戻すことはしない（Step7 内で完結）。

#### 可逆性

- **元に戻せない（見出し単位の編集データの初期化）**。`session_heading_sections` の該当レコードは物理削除する。`session_combined_contents`（完成形履歴）は保持される。
...
目的:
- **Step7** の「完成形履歴」を蓄積する
- 完成形 = **ユーザー入力の書き出し**（3.用語定義参照）＋見出し確定本文（order_index 順で連結）
...
| `content` | `text` | Yes | 当該バージョンの完成形全文（ユーザー入力の書き出し＋見出し本文。用語定義3参照） |
...
完成形の**先頭**には **ユーザー入力の書き出し**（全見出し確定後にユーザーが入力して送信したテキスト）を置き、その後に見出し確定本文を `order_index` 順で連結する。ユーザーが未入力で送信した場合は空文字列を先頭に置く（見出し確定本文のみの連結）。
...
- 未保存状態での遷移:
  - 確認ダイアログで破棄/継続を選択
- **空入力・空白のみの入力**で送信した場合:
  - バリデーションエラーは表示しない。完成形の作成を実行する。
  - `chat_messages`（model=blog_creation_step7_lead）から取得して使用。取得できない場合は空文字列として扱い、見出し確定本文のみを連結する。
  - 確認ダイアログや警告は表示しない。見出しのみの出力を許可する。
...
5. 完成形の先頭にユーザー入力の書き出し（未入力時は空文字列）が入る
6. 再編集時に対象見出し以外が変更されない
7. Step7 完了後は全文 Canvas 修正が可能
8. 最後の見出し保存後、ユーザー入力の書き出し＋見出し本文が Canvas に表示される
...
13. Step7 中に「構成リセット」を実行すると、確認ダイアログ後に `session_heading_sections` が削除され、基本構成（basic_structure）から見出しを再抽出して先頭見出しから再開できる（`session_combined_contents` は保持）
14. 既存の step6 見出し生成済みデータ（レガシー）は特別扱いしない。Step6 では構成リセットを表示せず、Step7 に進んでから構成リセットを使うか、Step6 を再生成すればよい

## 12. 実装メモ

- Step7 見出し保存は「ユーザー操作で確定」が前提。自動確定は行わない。
- 完成形の再結合時は、ユーザー入力の書き出しを先頭に置く。未入力の場合は 12.1 のフォールバックに従う。
- 完成形用 Server Action: **`buildCombinedContentWithUserLead`**（既存。`src/server/actions/heading-flow.actions.ts` に実装）
  - パラメータ: `{ sessionId, userProvidedLead?: string, liffAccessToken }`（userProvidedLead は省略可。空/未指定時に 12.1 の取得ロジックでサーバー側で取得）
  - 戻り値: `{ success: boolean, error?: string }`
  - 責務: 書き出し（12.1 の優先順）と見出し確定本文を結合し、`session_combined_contents` に新バージョンとして保存

### 12.1 書き出し案の保存・取得仕様

#### データフロー（要約）

- **保存先の一貫性**: Step6→7 で `chat_messages` に保存した書き出し案を、完成形作成時にサーバー側で取得して使用する（userProvidedLead が空の場合）。
- **二重入力の扱い**: 完成形作成時にユーザーが新しい書き出し案を入力した場合は**そちらを優先**し、Step6→7 の保存内容は使用しない。上書きではなく「入力あり＝それを採用」の優先順位。
- **取得ロジックの分担**: InputArea は `userProvidedLead` を常に渡す（入力ありならその文字列、空送信なら空文字列）。サーバー側 `buildCombinedContentWithUserLead` は、受け取った値が空のとき `chat_messages` から取得する。
...
- 構成リセット時は `resetHeadingSections` で当該メッセージも削除し、次 run で古い lead が使われないようにする

#### 完成形作成時の取得

- **ケース1**: ユーザーが InputArea で新しい書き出し案を入力して送信した場合  
  → その入力を使用（Step6→7 の保存内容は使用しない）
- **ケース2**: ユーザーが空で送信した場合  
  → `chat_messages` から最新の model=blog_creation_step7_lead を取得して使用  
  → 取得できない場合は空文字列として扱い、見出し確定本文のみを連結  
  → Step6 書き出し案は参照しない（パターンA）

#### 実装

- `buildCombinedContentWithUserLead`: パラメータ `userProvidedLead` は省略可。空/未指定時にサーバー側で chat_messages から取得するため、`useSavedLead` などの追加パラメータは不要。
- `combineSections(sessionId, userId, userProvidedLead?)`: userProvidedLead が空/未指定時、内部で getStep7UserLead のみ取得（Step6 は参照しない）
- `getStep7UserLead`: chat_messages の role=user, model=blog_creation_step7_lead の最新1件から content を取得
...
| 担当 | 役割 |
|------|------|
| `useHeadingCanvasState` | `viewingHeadingIndex`、`handleSaveHeadingSection`、`setViewingHeadingIndex` などを提供。見出し表示インデックスと保存のラッパー。 |
...
interface UseHeadingCanvasStateReturn {
  ...
  currentHeading: SessionHeadingSection | null;
  sections: SessionHeadingSection[];
  isSaving: boolean;
  handleSaveHeadingSection: (content: string) => Promise<void>;
  handleResetHeadingConfiguration: () => Promise<boolean>;
}
```
...
| `src/server/services/headingFlowService.ts` | `combineSections(sessionId, userId, userProvidedLead?)` でユーザー入力の書き出し＋見出し本文を結合、`initializeHeadingSections` の UNIQUE 制約違反時エラー返却、`resetHeadingSections`（Step7 初期化用） |
...
| `app/chat/components/StepActionBar.tsx` | 見出し0件時の基本構成導線、**Step7 時の三点リーダードロップダウン内に「構成リセット」**（8.7） |
| `src/server/actions/heading-flow.actions.ts` | `resetHeadingSections` アクション、`buildCombinedContentWithUserLead`（完成形作成: ユーザー入力の書き出し＋見出し本文を結合して `session_combined_contents` に保存） |
| `app/chat/components/MessageArea.tsx` | step7 メッセージ照合、タイルラベル |
| `app/chat/components/InputArea.tsx` | step7 プレースホルダー（8.3 の動的表示: 保存済み書き出し案の有無で文言切り替え） |
| `src/lib/prompts.ts` | 見出し生成プロンプトの step7 前提（該当箇所のみ） |

### 14.4 既存データ（Step6 見出し生成済みセッション）の扱い

#### 方針: Step6 レガシーは特別扱いしない。構成リセットは Step7 のみ

既に step6 で見出し単位生成済みのセッション（レガシーデータ）であっても、**step6 は通常の書き出し案フローとして扱う**。  
見出しフロー（1見出し+本文）は **step7 のみ**とする。

**Step6 レガシーへの導線は設けない**。`session_heading_sections` に古い見出しが残っている Step6 セッションでも、構成リセットボタンは Step6 では表示しない。ユーザーは Step7 に進んでから三点リーダー内の「構成リセット」を使うか、Step6 を再度生成すればよい。
...
| Step7 への移行 | Step7 表示中に三点リーダー内の `構成リセット` を実行した場合、見出し抽出元（5.2 参照）に従い再抽出して step7 を開始する。Step6 では構成リセットは表示されない。 |
| タイル→Canvas 紐づけ | `blog_creation_step6` は常に step6 タイルとして扱う。step7 として解釈しない。 |

#### 構成リセット時の動作（Step7 で実行した場合）
...
- Step6 表示時には構成リセットが三点リーダー内に表示されないこと。
- `blog_creation_step6` タイルをクリックしても step7 へ自動解釈されないこと。
- Step7 表示時に `構成リセット` 実行後にのみ step7 見出しフローが開始されること。

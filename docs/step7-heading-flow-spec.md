...
- Step7 の完成形:
  - 見出し保存時には保存しない（全文編集または明示的な完成形更新時のみ保存）
...
### 8.1 現在見出し表示（CanvasPanel）

表示場所:
- `app/chat/components/CanvasPanel.tsx` のヘッダー内（「完成形」/「見出し単位生成」タイトルの横）

表示条件:
- **Step7** の見出し単位生成フロー中（`showHeadingUnitProgressAndActions` が true のとき）のみ表示
- 基本構成エラー時・構成リセット前の過去見出し表示時は非表示
- 見出し配列が取得できない場合（totalHeadings=0）は非表示

表示内容:
- 進捗: `進捗: {currentIndex}/{totalCount}`（1-based で表示）
- 見出し名: 現在の見出しテキスト（truncate で省略表示、ホバーで全文）

表示例:
- `進捗: 3/8` ＋ `「大寒卵の定義」`

フォールバック:
- 見出し名が空の場合は表示しない（区切り線のみ）
- `totalCount=0` の場合は StepActionBar で基本構成入力導線を優先表示
...
| 担当 | 役割 |
|------|------|
| `useHeadingCanvasState` | `viewingHeadingIndex`、`handlePrevHeading`、`handleNextHeading`、`handleSaveHeadingSection`、`setViewingHeadingIndex` などを提供。見出し間ナビゲーションと保存のラッパー。 |
| `useHeadingFlow` | `headingSections`、`activeHeadingIndex`、`handleRetryHeadingInit`、`handleRebuildCombinedContent`、`combinedContentVersions` など。見出しデータ・初期化・完成形管理。 |

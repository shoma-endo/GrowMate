...
/**
 * PROVISIONAL: supabase/migrations/20260921000000_add_instagram_engagement_rate_and_followers_count.sql
 *
 * 管理者がマイグレーションを適用し `npm run supabase:types` を実行した後、
 * このブロックを削除し、呼び出し側を生成済みのテーブル型へ切り替える
 * （`.agents/skills/supabase/service-usage.md` §6）。
 */
export type InstagramEngagementDatabase = Omit<Database, 'public'> & {
  public: Omit<Database['public'], 'Tables'> & {
    Tables: Omit<Database['public']['Tables'], 'instagram_media' | 'instagram_credentials'> & {
      instagram_media: {
        Row: Omit<Database['public']['Tables']['instagram_media']['Row'], 'engagement_rate'> & {
          engagement_rate: number | null;
        };
        Insert: Omit<Database['public']['Tables']['instagram_media']['Insert'], 'engagement_rate'> & {
          engagement_rate?: never;
        };
        Update: Omit<Database['public']['Tables']['instagram_media']['Update'], 'engagement_rate'> & {
          engagement_rate?: never;
        };
        Relationships: Database['public']['Tables']['instagram_media']['Relationships'];
      };
      instagram_credentials: {
        Row: Database['public']['Tables']['instagram_credentials']['Row'] & {
          followers_count: number | null;
          followers_count_synced_at: string | null;
        };
        Insert: Database['public']['Tables']['instagram_credentials']['Insert'] & {
          followers_count?: number | null;
          followers_count_synced_at?: string | null;
        };
        Update: Database['public']['Tables']['instagram_credentials']['Update'] & {
          followers_count?: number | null;
          followers_count_synced_at?: string | null;
        };
        Relationships: Database['public']['Tables']['instagram_credentials']['Relationships'];
      };
    };
  };
};

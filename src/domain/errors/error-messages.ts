...
export const ERROR_MESSAGES = {
  ...
  WORDPRESS: {
     ...
     *
     * 仕様側も UI 用語辞書の契約に合わせ、「要約を実行」を含まない文面へ統一している。
     * `scripts/check-ui-text.sh` の検査対象なので、現行文面を変更しない。
     */
   ...
   */
  HOME: {
    /** マイホームで連携状態・提案件数の一部が取得できなかった場合 */
    PARTIAL_FETCH_FAILED: '一部の情報を取得できませんでした。ページを再読み込みしてください。',
  },
  ...
  ERROR_BOUNDARY: {
    ...
    /** デプロイ更新により古いバンドルの Server Action が見つからない場合 */
    DEPLOYMENT_MISMATCH: 'アプリが更新されました。ページを再読み込みしてください。',
   ...
   */
  FIELD_CONFIG: {
    /** 一覧テーブルのフィールド構成（表示列・並び順）の保存に失敗した場合 */
    SAVE_FAILED: 'フィールド構成の保存に失敗しました。画面を再読み込みしてからお試しください。',
  },
  ...
  GOOGLE_ADS: {
    ...
    NEGATIVE_KEYWORDS_SUGGESTION_PROMPT_NOT_FOUND:
      'Google Ads 除外キーワード提案プロンプトが見つかりません。管理画面でテンプレートを確認してください',

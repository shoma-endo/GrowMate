...
- `.takt/workflows/grill-to-gherkin.yaml` uses TAKT standard `grill-me` as the default interactive mode, normalizes its Markdown + Gherkin instruction into a report, records recommended reuse/extend implementation options against existing code (without inventing requirements), and requires human approval for both the Gherkin and the rough-estimate go/no-go decision before any spec or code edit.

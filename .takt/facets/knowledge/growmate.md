...
- Follow `AGENTS.md` Core Rules for MVP-first and safety-machinery scope. Do not add a looser exception list here.
...
- Follow `AGENTS.md` Core Rules for default roles (`admin` / `paid`) and server-side authorization. Specs must still document the role matrix and unauthorized behavior.
...
- `.takt/workflows/spec-review.yaml` reviews specifications before implementation (local TAKT). When the user explicitly requests Cursor Cloud, the unattended path is `.agents/skills/cloud-agent-unattended/` (no `takt` CLI).

...
- 結合は必ず `python3 scripts/spec-html.py build` に4ビューすべてと `--source <対象仕様書>` を渡して行う。安全検査で落ちた場合は違反箇所を直して再実行する。
...
- 生成結果を headless Chrome の `--dump-dom` で確認し、パネル数が 4、`hidden` が 3 であることを検証する。

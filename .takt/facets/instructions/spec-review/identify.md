...
7. `spec-review-scope.md` 冒頭に `基準ブランチ: origin/develop@<SHA> / HEAD: <branch>@<SHA>` を書く（`git rev-parse --short origin/develop HEAD` で取る。ゲート自体は prepare の `takt-check-base-branch.sh` が持つ）。

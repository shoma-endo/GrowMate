下記に全文添付された `plan.md` と、その `# タスク計画` 直後の `対象仕様書:` 行のパスの現行ファイルに従い、GrowMate の既存パターンに合わせて最小差分で実装してください。要件にない画面・UI・機能・改善を追加せず、ついで修正や将来対応は別タスクへ切り出してください。`docs/plans/` を列挙・推測して対象を決めてはならない。

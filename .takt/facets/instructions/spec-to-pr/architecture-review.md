...
- 追加・変更されたテストが `docs/specs/testing-strategy.md`「過剰なテストを書かない」 の「書かない」1〜4（下位層で固定済みの内容の上位層での再検証、同じ分岐の別 `it`、fixture の手書き重複、モックの戻り値の素通し assert）に当たる場合は `needs_fix` にする。差分外の既存テストは対象にしない。同節の「消さない」に当たるテストが削除されている場合も `needs_fix` にする。

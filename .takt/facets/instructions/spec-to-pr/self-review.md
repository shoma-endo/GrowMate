...
- 変更差分が対象仕様書と `plan.md` の `対象スコープ:` 内であること。機密情報の露出がないこと。混入があれば `needs_fix` とする。
...
- `needs_fix` は、このステップ直後の `self_review_fix` がコード変更で閉じられる指摘だけに使う。手動ブラウザ確認・管理者マイグレーション作業・残置合意・仕様がテスト追加不要と明示する論点は `needs_fix` にせず、未確認／残タスクとして記録したうえでコード上問題なければ `pass` とする。
- コード修正が必要な指摘には短い ID（例: `SR-1`）を付け、後段 fix が突合できるようにする。

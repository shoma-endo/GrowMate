 ...
 * ステータス別の分類そのものは google-oauth-error-handlers.test.ts で固定済みのため、
 * ここでは action が分類結果を 再認証・一時失敗・DB保存失敗 の各1件で正しく返すかを見る。
 */
...
describe('getGoogleAdsConnectionStatus のリフレッシュ失敗分類', () => {
  ...
  it.each([
    {
      status: '400',
      error: authExpired400,
      needsReauth: true,
      message: ERROR_MESSAGES.GOOGLE_ADS.AUTH_EXPIRED_OR_REVOKED,
    },
    {
      status: '429',
      error: rateLimited429,
      needsReauth: false,
      message: ERROR_MESSAGES.GOOGLE_ADS.TOKEN_REFRESH_TEMPORARY_FAILURE,
    },
  ])('refresh失敗 status $status は needsReauth=$needsReauth', async ({ error, needsReauth, message }) => {
    mocks.refreshAccessToken.mockRejectedValue(error);

    const result = await getGoogleAdsConnectionStatus();

    expect(result.needsReauth).toBe(needsReauth);
    expect(result.connected).toBe(true);
...
describe.each([
  { name: 'fetchKeywordMetrics', action: fetchKeywordMetrics },
  { name: 'fetchCampaignMetrics', action: fetchCampaignMetrics },
])('$name のリフレッシュ失敗分類', ({ action }) => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.authMiddleware.mockResolvedValue({ userId: USER_ID, userDetails: { role: 'paid' } });
    mocks.getGoogleAdsCredential.mockResolvedValue(CREDENTIAL);
  ...
  it('DB保存失敗は一時的失敗メッセージ', async () => {
    ...
    const result = await action('2026-01-01', '2026-01-31');
  ...
  it.each([
    { status: '400', error: authExpired400, message: ERROR_MESSAGES.GOOGLE_ADS.AUTH_EXPIRED_OR_REVOKED },
    {
      status: '429',
      error: rateLimited429,
      message: ERROR_MESSAGES.GOOGLE_ADS.TOKEN_REFRESH_TEMPORARY_FAILURE,
    },
  ])('status $status は対応するメッセージ', async ({ error, message }) => {
    mocks.refreshAccessToken.mockRejectedValue(error);

...
### サブスクリプションと権限
- Stripe v17.7 で Checkout / Billing Portal / Subscription 状態確認を実装
- `trial`（チャット1日3回まで）/ `paid`（無制限）/ `admin` / `unavailable` の4ロールで機能制御
...
| サービス | 取得先 | 主な設定変数 |
|---------|--------|------------|
| Stripe | [Stripe Dashboard](https://dashboard.stripe.com/) → API キー・商品 Price ID | `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID` |
...
**Stripe を無効化する場合**: `STRIPE_ENABLED=false` を設定し、`STRIPE_SECRET_KEY` と `STRIPE_PRICE_ID` にはダミー値（例: `sk_test_dummy`）を設定してください。
...
│   │   ├── services/        # 統合層（Stripe / WordPress / Supabase / LLM / GSC / GA4 / Google Ads など）

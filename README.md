...
LINE LIFF またはメール OTP を入り口に、業界特化のマーケティングコンテンツを一括生成・管理する SaaS アプリケーションです。Next.js 15 App Router を基盤に、マルチベンダー AI、WordPress 連携、Supabase による堅牢なデータ管理を統合しています。

## 🧭 プロダクト概要

- LIFF でログインしたユーザー向けに、広告／LP／ブログ制作を支援する AI ワークスペースを提供
- Anthropic Claude Sonnet 4.6 と OpenAI の Fine-tuned モデル `ft:gpt-4.1-nano-2025-04-14:personal::BZeCVPK2` を用途に応じて切り替え
...
### 認証とユーザー管理
- **LINE LIFF 認証**: LIFF v2.25 を利用したシームレスな LINE ログインと自動トークンリフレッシュ
- **メール OTP 認証**: Supabase Auth を利用した 6 桁 OTP によるメールログイン（`NEXT_PUBLIC_EMAIL_AUTH_ENABLED` で制御）
...
- `search_chat_sessions` RPC（`pg_trgm` 全文検索）でオーナー/スタッフ共有アクセスに対応
...
- `canRunBulkImport` による一括インポートの実行可否制御（許可: `owner` / `paid(owner)` / `admin` / `trial`）
...
- **Anthropic**: Claude Sonnet 4.6 (SSE ストリーミング)
...
| Client | `NEXT_PUBLIC_EMAIL_AUTH_ENABLED`     | 任意（デフォルト: 有効）           | メール OTP ログイン機能の on/off（`false` を設定すると無効）                           |
...
各サービスの OAuth クライアント・API キーを取得し `.env.local` に設定します。
...
| エンドポイント                      | メソッド | 概要                                                          | 認証                           |
| ----------------------------------- | -------- | ------------------------------------------------------------- | ------------------------------ |
| `/api/chat/anthropic/stream`        | POST     | Claude とのチャット SSE ストリーム                            | `Authorization: Bearer <LIFF>` または Email Cookie |
| `/api/chat/canvas/stream`           | POST     | Canvas 編集リクエスト（選択範囲差し替え）                     | `Authorization: Bearer <LIFF>` または Email Cookie |
...
| `/api/wordpress/settings`           | GET/POST | WordPress 設定の取得・保存                                    | Cookie                         |
...
| `/api/gsc/evaluations/register`     | POST     | GSC評価対象の登録                                             | Cookie                         |
| `/api/gsc/evaluations/update`       | POST     | GSC評価設定の更新                                             | Cookie                         |
| `/api/gsc/dashboard/[annotationId]` | GET      | 注釈別GSCダッシュボードデータ取得                             | Cookie                         |
| `/api/cron/gsc-evaluate`            | POST     | GSC記事評価の定期実行（Bearer 認証）                          | Authorization ヘッダー         |
...
| `/api/google-ads/accounts/clients`  | GET      | Google Ads クライアントアカウント一覧取得                     | Cookie + admin ロール          |
| `/api/google-ads/keywords`          | GET      | Google Ads キーワード指標取得                                 | Cookie + admin ロール          |
| `/api/ga4/sync`                     | POST     | GA4日次データ同期                                             | Cookie                         |
| `/api/ga4/settings`                 | GET/POST | GA4設定の取得・保存                                           | Cookie                         |
| `/api/ga4/properties`               | GET      | GA4プロパティ一覧取得                                         | Cookie                         |
| `/api/ga4/key-events`               | GET      | GA4キーイベント一覧取得                                       | Cookie                         |
| `/api/employee`                     | GET/POST | スタッフ招待リンク一覧取得・招待発行                          | Cookie                         |
| `/api/employee/invite`              | POST     | 招待リンク発行                                                | Cookie                         |
| `/api/employee/invite/status`       | GET      | 招待リンク状態取得                                            | Cookie                         |
| `/api/employee/invite/[token]`      | GET/POST | 招待トークン検証・受け付け                                    | 公開（トークン検証あり）       |

**GSC 評価バッチ**: `CRON_SECRET` を設定し、`Authorization: Bearer <CRON_SECRET>` ヘッダー付きで `/api/cron/gsc-evaluate` を呼び出します。
...
- **Keepalive**: 月1回 `.github/workflows/keepalive.yml` で自動停止を防止（public リポジトリは60日無活動で停止）

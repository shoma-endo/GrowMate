...
graph TB
  ...
  subgraph Data["Supabase PostgreSQL"]
    UsersTable["users"]
    EmployeeInvitations["employee_invitations"]
...
- 直近のマイグレーション概要:
  ...
  - `ga4_page_metrics_daily` に impressions / clicks / search_clicks カラムを追加
  - `google_ads_credentials` テーブル追加・`customer_id` カラム追加
  - `employee_invitations` テーブル追加・スタッフ招待 RPC（`accept_employee_invitation`, `delete_employee_and_restore_owner`）
...
| `/api/employee`                     | GET      | スタッフ情報取得（オーナーのみ）                              | Bearer（オーナーロール） |
| `/api/employee`                     | DELETE   | スタッフ削除・オーナーロール復元                              | Bearer（オーナーロール） |
...
| `auth.actions.ts` | `sendOtpEmail`, `verifyOtp` | メール OTP 認証 |
| `login.actions.ts` | `logout` | ログアウト |

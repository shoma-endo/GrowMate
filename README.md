...
- **認証・ユーザー管理**: メール OTP（Supabase Auth）、`authMiddleware`、ロール管理（`trial` / `paid` / `admin` / `unavailable`）、パス別 middleware ゲート
...
graph TB
  ...
  subgraph Data["Supabase PostgreSQL（テーブル詳細は database.types.ts）"]
    ...
    ContentData["briefs / content_annotations / headings / combined_contents"]
...
| 変数名 | 必須 | 用途 |
| ------ | ---- | ---- |
| `CRON_SECRET` | 任意（`/api/cron/*` バッチを使う場合は必須） | Cron バッチの Bearer 認証（`gsc-evaluate` / `google-ads-negative-keywords-suggestion`） |

...
### スタッフ管理
- オーナーは `GET /api/employee` でスタッフ情報を取得、`DELETE /api/employee` でスタッフを削除してオーナーロールに復元
- オーナーのスタッフ閲覧は Cookie 付与と `authMiddleware` 判定で実現
- **注**: 招待リンク発行・受け付け機能（`InviteDialog`, `useEmployeeInvitation`）は削除済み。`employee_invitations` テーブルは DB に残存するが、アプリケーション側から新規書き込みは行われない

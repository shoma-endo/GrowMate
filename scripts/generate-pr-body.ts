...
# 見出し構成（この6つを順に使う。該当なしの節は見出しごと省略してよい）
...
## 受け入れテスト手順
番号付きで書く。ユーザーが画面上から実際に動かす手順に限定する。画面遷移と操作と確認ポイントを含める。diff から読み取れる範囲だけを書く。UI に触れない変更しか無い場合は見出しごと省略してよい。

## 検証観点
箇条書きで書く。境界値、回帰、権限、異常系のうち diff から読み取れるものに限定する。UI に触れない変更しか無い場合は見出しごと省略してよい。

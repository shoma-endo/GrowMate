...
| `warn` 原本の改訂が直接効く concept | 改訂された章を `source_refs` で根拠にしている concept。**どこを直すかの名指し** | 更新モードでその concept の `summary` / `detail` を見直す（`id` は変えない） |
| `info` 1ホップで波及しうる concept | 上の concept と `relations` で繋がっている concept。直接は改訂されていない | 経路（`A --depends_on--> B`）を見て、前提が動いたぶん記述が古くないか確かめる。当たらないこともある |
...
## 更新モードで fail / warn が出たときの手順

1. コンソールの fail 行が指す参照 id（`r_xxx`）を控える。
2. 原本の該当章を読み、`core.yaml` の `source_refs` の `lines` を実際の行番号に直す。
3. **`原本の改訂が直接効く concept` の warn が名指しした concept**（`c_xxx`）の記述を見直す。
   ここは推測しなくてよい ── `source_refs` の持ち主を辿って機械が特定している。
4. **`1ホップで波及しうる concept` の info** を見る。経路（`c_gate --blocks--> c_status`）が
   示す前提が動いているので、その concept の記述が古くないか確かめる。当たらないこともあるので、
   確かめた上で変更不要ならそのままでよい（2ホップ以上は追わない ── 辿るほど当たりが増えて
   結局全部読み直すことになる）。
5. `build` を再実行し、fail が消えることを確認する（消えなければ 2 に戻る）。

`refresh` の陳腐化警告にも `直すべき concept: c_status` の形で名前が出る。

...
一気通貫のレビューは TAKT `.takt/workflows/spec-review.yaml` を使う（レビュー → 仕様書修正 → 再レビュー → docs ブランチ push、auto-pr 委譲）。レビュー済み仕様書の合意（確認質問への回答・PR マージ）後に `.takt/workflows/spec-to-pr.yaml` で実装へ進む。

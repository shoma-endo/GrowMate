...
- [ ] 主要な設計・スコープ判断について、採用前に既存コード・同種仕様・（外部なら公式ドキュメント）でより良い案を再調査した痕跡があるか。「検討不要」だけで範囲も結論も無い場合は指摘する。レビュー側で別案に差し替えない。小改修で主要な設計判断なしと明記されていれば対象外
...
| 画面・UI コンポーネント                                                               | `growmate-ui-ux`（正本優先順位、迷ったら既存の再利用・拡張、AI 連携 UI の鉄則）                                                                                         |

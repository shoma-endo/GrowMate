...
2. **着手前 — 調査**: 同種画面（`/setup/*`, ダッシュボード等）の既存パターンを grep で確認する。**ヒットした箇所は全件開き、表にしてから判断する**（1件見つけた時点で止めない）。レイアウト・variant・配置など判断軸が複数ある場合は、軸ごとに全件を数える。多数派と違う形を採る場合は、その理由を仕様書または PR に書く。**再利用または拡張で足りるか**を先に判断し、足りるなら踏襲する。新規パターンは既存で足りない理由があるときだけ。

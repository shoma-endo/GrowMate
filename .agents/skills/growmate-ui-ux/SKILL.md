...
3. **マークアップとクラスを1文字も変えずに写す**（1・2 ができない理由を PR に書く）

写すとき・再利用するときに、**自分の判断で次を変えない**: 見出しの語、文言、色クラス、`title` / `aria-label`、アイコンの有無、余白、枠。変える場合は仕様書の明示か利用者の指示を根拠として PR に書く。

**優先順位**: 「同種の既存 UI と一致すること」は、下の「デザインシステム」の「生 Tailwind 色は避けトークンを使う」より優先する。既存が生の色を使っているなら、写す側も同じ生の色を使い、`eslint --suppress-rule shadcn/no-raw-colors <file>` で既存と同じく抑制する。トークン化は既存側と同時にやる別作業であり、写す側だけトークンへ置き換えて見た目をずらさない（2026-09-23、Instagram タブで写す側だけトークン化・見出し変更・`title` 削除をして、ブログ一覧と見た目がずれた）。
...
|---|---|---|---|
| 例: 状態フィルターの行 | `src/components/CategoryFilter.tsx:103-134` | 写す | なし |

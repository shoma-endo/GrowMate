...
**優先順位**: 「同種の既存 UI と一致すること」は、下の「デザインシステム」の「生 Tailwind 色は避けトークンを使う」より優先する。既存が生の色を使っているなら、写す側だけトークンへ置き換えて見た目をずらさず、上の 2（既存ファイル内での共通化）で同じ見た目を保つ。トークン化は既存側と同時にやる別作業である（2026-09-23、Instagram タブで写す側だけトークン化・見出し変更・`title` 削除をして、ブログ一覧と見た目がずれた）。

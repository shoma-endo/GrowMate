...
export default function PromptsClient({ initialTemplates, initialError }: PromptsClientProps) {
  ...
  const loadTemplates = useCallback(async (): Promise<PromptTemplate[] | null> => {
    try {
      setIsLoading(true);
      ...
      if (res?.success && res?.data) {
        ...
        // 再取得時に選択中テンプレートを同期する
        // ※未保存の編集がある場合はエディタ内容のみ保持し、selectedTemplate 自体は常に最新版に更新する
        setSelectedTemplate(prevSelected => {
          if (!prevSelected) {
            return prevSelected;
          }
          const currentEditedContent = editedContentRef.current;
          const updated = nextTemplates.find(t => t.id === prevSelected.id) ?? null;
          if (!updated) {
            // 対象テンプレートが削除されている場合は選択状態と編集中内容をクリアする
            setEditedContent('');
            return null;
          }

          const hasUnsavedChanges = currentEditedContent !== prevSelected.content;

          // 未保存の編集がある場合はエディタ内容を保持しつつ、selectedTemplate 自体は最新版に更新
          if (hasUnsavedChanges) {
            return updated;
          }

          // 未保存の編集がない場合はエディタもサーバー内容で同期する
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        // templates 配列も更新（一覧から再選択時の巻き戻しを防止）
        setTemplates(prevTemplates =>
          prevTemplates.map(t => (t.id === selectedTemplate.id ? savedTemplate : t))
        );

        // サーバーから最新データを取得してupdated_atなどを正確にする
        ...
        if (freshTemplates) {
          ...
          if (updatedTemplate) {
            ...
            // テンプレートが見つからない場合（保存は成功しているのでローカル状態は維持）
            setError('保存後のデータ取得に失敗しました');
            toast.error('保存は成功しましたが、最新データの取得に失敗しました');
          }
        } else {
          // loadTemplates が失敗した場合（保存は成功しているのでローカル状態は既に更新済み）
          setError('保存後のデータ取得に失敗しました');
          toast.error('保存は成功しましたが、最新データの取得に失敗しました');
        }
      } else {
